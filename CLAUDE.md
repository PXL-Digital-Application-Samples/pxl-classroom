...
- **Whatever `publish` switches on, something has to switch off.** `INVITE_ENABLED` went `true` at publish and was never set back, so every broker an org had ever published still booted a runner for acceptance attempts — on a finished exam, five days after its deadline, for a run the hub refuses as `rejected:past-deadline`. The nightly closes it when it finalizes (`scripts/close-acceptance.mjs`). Ask the same question of every other flag a publish sets.

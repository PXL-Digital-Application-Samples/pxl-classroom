...
- **Fail closed.** An unrecognised `roster_mode` is `enforced`; an absent nonce rejects; an absent `lock_down_enabled` is `true`. Never relax one of these to match a form default. Same for cohorts: once an assignment names `class_groups`, a student with no `class_group` is **not** admitted — "I did not say" is not "any". The one deliberate exception is the field's own absence, which means *every* group, because every assignment predating it has no list and reading that as "nobody" would lock out every existing cohort.
- **The roster is org-wide; the cohort is per assignment.** One `students/roster.yml` per org, and `class_groups` on an assignment narrows it — `lib/class-groups.mjs` decides, nothing else. It is GitHub Classroom's "classroom" stored as a column rather than a folder, chosen over a second roster file so a student exists once. `students/roster.yml` itself is `ROSTER_PATH`, never re-spelled (`tests/roster-path.test.mjs`).

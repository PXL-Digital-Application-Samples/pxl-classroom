...
- **Dispatch-and-watch.** SPA workflow triggers are never fire-and-forget: each polls for the artifact/status the workflow produces (publish → broker repo, daily-activity → `reports/<id>.json`, usage → `usage-latest.json` compared against a `generated_at` baseline, and retry-acceptance → polls for the student repository to appear with a 2-minute timeout and a link to the workflow run). Follow this pattern for any workflow trigger.
- **Lecturer validations & Student gating.** Lecturer actions (granting extensions, retrying acceptances) validate student logins beforehand (checking roster, records, reports, and GitHub user API, returning specific error warnings for typo check vs. not on roster). Extension operations are append-only to preserve overrides history. The student acceptance card is gated on `state === 'published'` and `opens_at <= now` (showing closed/early warnings instead of Accept button, keeping provisioned repo access if already accepted).
...
- **CLI destructive ops confirm.** `roster import` prompts before committing removals (TTY) or requires `--force` (non-TTY); `feedback open` separates newly created vs adopted pre-existing PR counts and exits non-zero on partial failure; `--concurrency` parses to positive integers only.

...
- **`pxl-classroom` CLI is the parallel lecturer surface** (in `cli/`) — same App, same device-flow auth, same schemas as the SPA. Owns the workflows that scale poorly through clicks: CSV roster import, install audits, feedback-PR orchestration, bulk submission download, lecturer-side autograding. The SPA links to it via "Copy CLI command" affordances; never duplicate a CLI flow as an in-SPA button (autograder/bulk-clone are intentionally not in the SPA).

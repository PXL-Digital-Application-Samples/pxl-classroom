...
- **Live Status invariants (AssignmentDetailView):** a post-deadline commit never downgrades a student who has an on-time submission on record (it sets `first_late_sha`, not a `late` status — nightly semantics), a partial refresh is never persisted (all students refreshed, or nothing is committed to the control repo), and grading sync results with 0 check-runs are logged under failed with reason `"no CI run at preserved SHA"`. CSV export is generated client-side from the on-screen report, never fetched from the control repo.

...
# cohort: [num:0123456, login:ella-dev] # optional; absent = every roster student.
                                       ...
                                       # the gate.
# cohort_groups: [3A]                   # optional; a LABEL for the dashboard card,
...
**`class_group` is a property of a student, and a filter.** It gates nothing. It was the gate until 2026-09-05, as a rule the acceptance path re-evaluated - which meant the cohort was never written down and could only slice one way, so a resit for "3A plus these four" had no expression at all. `lib/class-groups.mjs` now holds only what a picker needs: the distinct groups on a roster and their counts, so the chips read `3A · 20` and **No group** is a filter of its own rather than a set of students the old rule refused outright. `cohort_groups` records which groups a selection was made from, as a **label** no code path reads - it is what lets the dashboard say "3A" instead of leaving a cohort of 22 out of 40 unexplained, and it can go stale when a student changes group, so the copy beside it names it as how the list was made and not as who is in it.

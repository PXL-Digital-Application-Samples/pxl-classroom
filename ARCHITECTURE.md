...
| Repository: Checks | Read | Yes | Reading a grade out of CI — `GET /repos/{o}/{r}/commits/{ref}/check-runs` and the annotations behind it are both gated by this. Undeclared until 2026-08-26, which 403'd every grading sync (§11.6) |
...
The *form* default is `open` (2026-08-24). WS1 had set it to `enforced` because the broker repo is public, so the roster was the only thing between any GitHub account and a provisioned repository. Signed invitations took that job over (§4.3.2): the broker verifies a signature at the edge before a credential is minted - the student's P-256 acceptance assertion on a migrated assignment, the token's own Ed25519 signature on one still on the legacy format - so someone without the link gets nothing whatever `roster_mode` says. Requiring a CSV import before a single student could accept therefore bought nothing, while making every new org's first assignment depend on one. `enforced` is one dropdown away; `open` requires `max_acceptances`, which `emptyForm()` supplies. Existing assignments are untouched - the default governs new ones only.
...
- **Roster-gated acceptance, available per assignment.** Under `roster_mode: enforced` students must be registered on the course roster (`students/roster.yml`) before they can accept the assignment and get a repo, which prevents arbitrary users from spawning repositories and using template resources. Mitigations: `opens_at..deadline_at` window, `max_acceptances` cap, idempotency, roster gating.
  ...
  **`roster_mode: org_member` was withdrawn on 2026-08-27, having never run a cohort.** It gated on an ACTIVE organization membership, to serve the case where a lecturer holds student **email addresses** and not GitHub usernames: an installation token can never read a user's email addresses and anything the browser sends is a claim rather than a credential, so the lecturer invited the addresses, GitHub performed the email-to-account binding, and `accept.mjs` read `GET /orgs/{org}/memberships/{login}` for the result. The problem it solved is real and remains; the solution made enrolment depend on organization membership, which is a heavy and permanent thing to require for a semester's assignment — an org member is subject to `default_repository_permission` (§11.6) and appears in the org's People list indefinitely. The claim flow binds the address directly instead, without putting students in the organization at all. Removal was safe to verify rather than assume: across the twelve participating orgs, every assignment ever written is `enforced` (12) or `open` (10), and none has ever been `org_member`. A control repository still holding the value now reads as `enforced`, because `normalizeRosterMode` fails **closed** — which is why removing a value from `ROSTER_MODES` is safe in a way adding one is not. The App's `members` permission is deliberately **kept**: it is approved on eleven orgs, costs nothing to hold, and re-acquiring an org-approved permission is a slow round (§11.6).
...
Preserved submissions are per assignment (§11.3.1), so retiring a finished cohort is its student repositories and its `pxl-classroom-archive-<id>`, one gesture - RUNBOOK §8a.

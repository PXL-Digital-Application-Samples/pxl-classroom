...
- **The claim under `open` enrolment.** The claim mechanism (§15) is built and runs under `roster_mode: claim`, where the roster is the gate. Extending it to `open` was decided as wanted and is not built.

  What it would buy is **not** a gate - `open` stays genuinely open, and its guardrails remain the cap, the window and the link not leaking. It buys two things `open` has never had: **detection**, because an address outside `claim_domains` is visible in the report even though nothing refuses it, and **one-address-one-repo accounting**, because the uniqueness check that makes a second claim on a taken address fail is the only thing that would stop one person quietly taking several exam repositories. Both are review aids, and the copy must say so - describing a domain check on `open` as if it prevented anything is the C4 failure this document keeps naming.

  The pieces already exist: `lib/claim.mjs` for the seal and the domain resolution, the attempt counter, and the uniqueness scan in `accept.mjs`. What is missing is the branch that runs them under `open` without the roster match that currently follows, and a decision about what a claim binds to when there is no roster entry to bind it to.

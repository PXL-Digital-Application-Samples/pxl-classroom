...
`acceptance/accept.mjs` step 7 then asks the question with both halves in hand - this student, this name - after the `already-accepted` branch has excluded a retry and `isFirstMember` has excluded a group member joining a teammate's repository. One `GET /repos/{org}/{repo}`; on a 200, one `GET /repos/{org}/{repo}/rulesets`, which returns the repository's own rulesets *and* the organization ones that apply. A repository frozen by any `pxl-classroom-deadline*` (`isSubmissionLockName`) is `rejected:repo-frozen` **whatever the policy says** - there is no assignment for which handing a student a repository they cannot push to is the wanted outcome. An ordinary one follows the policy: reused, recorded as `reused_existing_repo` on the acceptance, or `rejected:repo-exists`. Either read failing is `rejected:repo-exists` too: a failed read is not evidence the name is free. It is in `accept.mjs` rather than `provision.mjs`, which makes the same `GET` a moment later, because refusing there would be after the acceptance record was written and a slot of `max_acceptances` spent - and because a student turned away on purpose is a `rejected:*` outcome that exits 0 and reaches the lecturer's detail view, not a `fail:*` that paints the hub's Actions tab red. `lib/existing-repo.mjs`, `tests/existing-repo.test.mjs`, `tests/accept.test.mjs`.

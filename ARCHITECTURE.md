...
**An existing repository is reported here and decided at acceptance, and the split is about where the question is answerable.** This screen holds the pattern and the organization's repository listing; who will *accept* is not knowable on it, so "a repository this pattern would produce exists" and "a student in this cohort would be handed one" are different statements. Refusing on the first cost a real course its assignment name over 300 portfolios from previous years, of which perhaps two belonged to a student who would accept, with "delete 300 students' repositories" offered as one of two ways forward. So the form **notes** the count - saying *in this organization*, because the number is the organization's - and asks the one question a lecturer can answer and the system cannot: `existing_repo_policy`, `reuse` (what provisioning has always done, right for a portfolio carried across years) or `refuse`. Absent means `reuse`, so nothing predating the field is reinterpreted, and the schema carries **no default** for it because Ajv's `useDefaults` would write one in (§ the tri-state rule, `tests/schema-defaults-are-writers.test.mjs`).

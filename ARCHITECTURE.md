...
| `claim_address_format` | The form the part before the @ must take (`pattern`, shown to students as `example`) - `firstname.lastname` at PXL, so `12345678@student.pxl.be` is not accepted. An assignment switches it off with `claim_address_format: false`. `lib/claim.mjs` `resolveAddressFormat` / `addressFormatAllowed`; `lib/deployment.mjs` refuses a pattern that does not compile |
...
- **Public broker compromise.** A broker workflow mints a token for the `pxl-classroom`-scoped installation and dispatches into the hub. The *private key* it does that with is bounded too (§4.3.0), so a broker compromise buys the ability to submit an acceptance, and nothing else. **It does not buy a signed one, and the acceptance it submits is not confined to that broker's organization.** The signature is checked on the *broker*, so whoever holds the key skips it by dispatching directly, and `client_payload.org` is bound to nothing: one key sits on every broker in every course org, and the hub mints a token for whatever org the payload names. What that is worth, and why it is accepted rather than fixed, is OPEN-ITEMS §10. §4.3.1 is a hard rule rather than a style preference: it is what keeps attacker code out of the job in the first place.
...
**A second residual, also accepted, and larger than it reads.** That key is the *same* key on every broker in every course org, and an acceptance dispatch names its own target organization. So a holder - anyone with admin on one course org, who can push a workflow to a broker there and read it out - can submit an acceptance into *any other* participating org. `lib/broker-issue-target.mjs` confines only which issue the hub reads and writes; `acceptance/accept.mjs` never sees `broker_repo` at all. Accepted because the principal is a lecturer with far more destructive options inside their own org, and the gain is one student repository from a peer course's template plus a slot off its cap. OPEN-ITEMS §10 carries the shape of the fix.
...
student_permission: maintain          # pull|triage|push|maintain|admin. Read at
                                       ...
                                       # past their deadline, never to a locked
                                       # repository whatever the deadline says now,
                                       # never to a revoked one, never after the
                                       # sentinel stopped writes (its timeline is
                                       # the lock until finalize writes one) -
                                       # planned again at the click
...
# cohort: [num:0123456, login:ella-dev] # optional; absent = every roster student.
                                       # Who this assignment is for, picked from
                                       # the roster. Only read when the roster is
                                       # the gate.
...
**`repository_name_pattern` is the collision key, not `id`.** Provisioning is idempotent on repository *existence*, so an assignment whose pattern produces a name that already exists hands that student the old repository instead of a fresh one - and skips generate-from-template, so they get none of this assignment's starter code. Two ids can point at one namespace and one id can be reused over a fresh one, so the id answers neither question. `lib/assignment-collision.mjs` is the judge, isomorphic and evidence-in/verdict-out, and the Admin Panel refuses a create that lands on another live assignment's pattern (the invariant `lib/seed-teams.mjs` has always stated and nothing enforced, §5.6) or on a surviving `pxl-classroom-archive-<id>` - which still holds `refs/heads/preserved/<id>/<login>`, and preservation pushes without `--force` on purpose (§11.3.1), so the collision surfaces at the *next* deadline rather than at the click. The question is always what **exists**: the authoritative org repository listing decides, not the deletion record, so `retired/<id>/` alone never blocks - an assignment opened by mistake and deleted before anybody joined leaves one, and refusing that would refuse starting over. An unreadable answer refuses. An **existing** assignment is checked for one thing only, the pattern clash: its own repositories and its own archive are its own. `tests/assignment-collision.test.mjs`, `tests/e2e/61-assignment-collision.spec.mjs`.

**An existing repository is reported here and decided at acceptance, and the split is about where the question is answerable.** This screen holds the pattern and the organization's repository listing; who will *accept* is not knowable on it, so "a repository this pattern would produce exists" and "a student in this cohort would be handed one" are different statements. Refusing on the first cost a real course its assignment name over 300 portfolios from previous years, of which perhaps two belonged to a student who would accept, with "delete 300 students' repositories" offered as one of two ways forward. So the form says nothing while a lecturer types, and Save opens `ExistingReposModal` once - the count, and the one question a lecturer can answer and the system cannot. **There is no permanent control**, deliberately: one lecturer in the deployment meets this, and a field under the pattern would ask every other one to have an opinion about a case they never see. **Asked once** is the other half - what fires it is "repositories exist", not "a student will hit one", so on the assignment where it does fire it would fire on every later save too; the answer is stored, and a save whose assignment already carries one is silent. A changed pattern asks again, being a different question about a different set of repositories. `buildAssignmentDoc` carries the value through, or the next unrelated edit would delete it.

**Absent resolves by what the name embeds, and this is the security-relevant half.** An individual pattern carries `{github_login}`, so `portfolio-PXL-AnnDeWit` can only be Ann's and reuse is safe. A team pattern carries `{team_slug}` - and the repository name carries the slug **without the assignment id**, so `grp-team-a` is the same name for every assignment that ever had a `team-a`, whose members were different students. Reuse there hands this year's team another cohort's repository with their work in it, which provisioning has always done silently. `normalizeExistingRepoPolicy` therefore resolves an absent policy to `reuse` for individual and `refuse` for team; it is **derived, never written**, so an explicit `reuse` from a lecturer is still obeyed, and the dialog is not shown on a team assignment at all because there is no choice to offer. The schema carries **no default** because Ajv's `useDefaults` would write one in (`tests/schema-defaults-are-writers.test.mjs`).

The student's side of a refusal says *who*, never *why*: the hub's only channel to that page is a label on their own public acceptance issue (`lib/acceptance-labels.mjs`, two values and no more), and a per-reason label would be a one-click public list of which named students are not enrolled. The rejected state names the attempt instead - assignment title, account, time - so a lecturer can find the refusal that the private control repository does explain.

`acceptance/accept.mjs` step 7 then asks the question with both halves in hand - this student, this name - after the `already-accepted` branch has excluded a retry and `isFirstMember` has excluded a group member joining a teammate's repository. One `GET /repos/{org}/{repo}`; on a 200, one `GET /repos/{org}/{repo}/rulesets`, which returns the repository's own rulesets *and* the organization ones that apply. A repository frozen by any `pxl-classroom-deadline*` (`isSubmissionLockName`) is `rejected:repo-frozen` **whatever the policy says** - there is no assignment for which handing a student a repository they cannot push to is the wanted outcome. An ordinary one follows the policy: reused, recorded as `reused_existing_repo` on the acceptance, or `rejected:repo-exists`. Either read failing is `rejected:repo-unreadable`, which is its **own** outcome and not one of the other two: a failed read establishes nothing about what is at the name, and the way to reach it - an absent token, an App that lost `administration` - refuses the whole cohort at once, so the message has to be about the fault rather than about the students.

**A 403 on the rulesets read is the plan gate, and it means not frozen.** Measured 2026-09-09: Team answers `200 []`, a free organization answers `403 Upgrade to GitHub Pro` for a *private* repository (`200 []` for a public one). Reading it as unreadable refused every student on a free organization whose repository already existed. The feature being unavailable is an answer - no ruleset can be enforcing - and the lock would have been written by this same App with this same token, so one that cannot read rulesets there cannot have created one. `frozenFromRulesets` takes the status rather than a boolean so that distinction is expressible; a 401 or a 500 is still `null`. It is in `accept.mjs` rather than `provision.mjs`, which makes the same `GET` a moment later, because refusing there would be after the acceptance record was written and a slot of `max_acceptances` spent - and because a student turned away on purpose is a `rejected:*` outcome that exits 0 and reaches the lecturer's detail view, not a `fail:*` that paints the hub's Actions tab red. `lib/existing-repo.mjs`, `tests/existing-repo.test.mjs`, `tests/accept.test.mjs`.
...
**The join runs both ways, and for a long time it ran only one.** The paragraph above describes the roster supplying an address and the system learning a **login**: `byEmail.get(entry.email)`. A row promoted from an acceptance has no address at all, so that lookup missed every one of them - while the claim sitting there carried the `github_id` and the `github_login` that row is keyed by. The one kind of row that could not be identified any other way was the one kind a claim could not reach. It is matched on the id first (immutable, where a login is the one thing a student can change between accepting and being folded) and the address is written in. Two more cases are held rather than written, alongside the three above: an address **another row already holds**, because `email` is what this join reads and two rows sharing one would break it for good; and an address **outside `claim_domains`**, because GitHub verifying that the account owns it does not make it the institutional address the column is for. Both surface in the same *N need your decision* list. `tests/claim-identifies-promoted.test.mjs`, `tests/e2e/63-roster-identify.spec.mjs`.
...
**`cohort` names *who*, and the roster stays org-wide.** One organization runs several sections of a course, and one `students/roster.yml` gated all of them: change it for one section's assignment and you changed it for the other's. An assignment carries the students it is for, picked from that roster when it was created - `lib/cohort.mjs` is the only judge, isomorphic so the picker and the gate cannot differ about who is admitted. A per-assignment roster *file* was rejected for the reason this codebase keeps meeting: a second copy lets one student exist twice and disagree with themselves. The cohort holds **references**, never names or numbers copied out of the roster.

Each entry names a roster row as `num:<student_number>` or `login:<github_login>`, and matching accepts **either identity a row carries**. That is not defensive: a row promoted from an acceptance has only a login, and if a later CSV import gives that person a student number, a single canonical key would change and silently stop matching a cohort they are deliberately in. **Absent or empty means every student on the roster** - what every assignment written before the field means, and what an untouched picker means, which is why the form says so on screen rather than letting a lecturer untick their way to admitting the whole course. It is a **snapshot**: a student imported afterwards is not in it until someone adds them, so the cohort is a record of who the assignment was for rather than a rule that re-evaluates. Only consulted when the roster is the gate - `enforced` (`rejected:not-in-cohort`) and `claim` alike, where an address from another section is validly registered but not admitted; under `open` the roster decides nothing, so the Admin Panel offers no picker there. The report narrows only its *roster*-derived population, never a row for someone who actually accepted: a cohort of 40 must not read as half-finished because 20 were never invited, but a row that should not exist is exactly the one to keep. `tests/cohort.test.mjs`, `tests/e2e/49-class-groups.spec.mjs`.

**A refusal reaches the lecturer in the app, not only on GitHub.** Every `rejected:*` is announced by `acceptance-handler.yml` as a comment on the control repo's instructor tracking issue, and that was the only surface it had - so an assignment refusing exactly the students it was configured to refuse looked identical, on the detail view, to a broken invitation link. The detail view reads that issue and shows who was turned away and why (`lib/rejection-notice.mjs`), in plain language rather than as `rejected:` slugs, with the raw outcome as the title so a lecturer chasing one student can match it against the workflow log. **Parsed from the dedup key, not the prose:** every rejection comment carries `<!-- pxl-dedup:reject-<assignment>-<login>-<outcome>-->`, which the notifier already depends on byte for byte, so it is the one part that cannot drift silently - and the key is *built* by the same module, with `tests/rejection-notice.test.mjs` checking the workflow's literal against it. Deduping means one comment per student per reason, so counting comments counts distinct refusals. **An unreadable tracking issue is not zero:** a failed lookup or a capped comment walk says so, because a silent "nobody was turned away" is the same mistake as a green check over a grading run nobody could read. The panel is inline in the report, never an overlay - the help drawer owns that layer (DESIGN.md §7).

**Seeding checks the cohort, not the organization.** A team manifest names logins and a cohort names roster identities, so `lib/seed-teams.mjs` bridges the two through `rosterEntryForLogin` - and the bridge is the interesting part. The roster's own `github_login` column is only half of it: under `claim` a properly enrolled student's login lives in their claim record until the nightly folds it in, and a check reading the column alone told the lecturer that student "is not on the roster and will be rejected when they accept", about somebody who will be admitted. `extraLogins` carries the claim bindings in; `lib/cohort.mjs` takes it as a parameter rather than importing `lib/claim-bindings.mjs`, so it stays dependency-free for the acceptance gate. **Two warnings, not one** - `not-on-roster` and `not-in-cohort` send a lecturer to do different things. And `unplaced` counts the cohort: it was roster-relative, so seeding a 22-person cohort in a 200-student org reported 178 unplaced students who the assignment was never for.

**`class_group` is a property of a student, and a filter.** It gates nothing. It was the gate until 2026-09-05, as a rule the acceptance path re-evaluated - which meant the cohort was never written down and could only slice one way, so a resit for "3A plus these four" had no expression at all. `lib/class-groups.mjs` now holds only what a picker needs: the distinct groups on a roster and their counts, so the chips read `3A · 20` and **No group** is a filter of its own rather than a set of students the old rule refused outright. A `cohort_groups` label was built and then removed: it recorded the groups *represented* in a selection, so three students who happened to share one made a card read "3A" for an assignment that was for three people out of twenty, and the honest version - only when a whole class is taken - goes stale the moment somebody joins that class. The count is what the card shows.
...
**One column list, two exports.** `lib/report-csv.mjs` holds `REPORT_ROW_COLUMNS` - what both the nightly `reports/<id>.csv` and the Assignment detail view's **Export CSV** write - plus `RENDER_JOIN_COLUMNS`, the five the SPA appends because they are not report-row fields at all (grades joined from `grading/<id>/summary.json`, and the feedback pull-request lookup), and `EXCLUDED_ROW_COLUMNS`, the row fields deliberately left out with the reason each. Both surfaces kept their own 35-column list and they were not the same 35: the SPA silently dropped `claimed_email`, `claim_verified`, `claim_domain_allowed`, `archive_repo` and `archive_ref`, and ten more row fields were in neither with nothing recording that as a choice. `tests/report-csv-columns.test.mjs` is **exhaustive rather than comparative** - every field the schema declares on a student row must be exported or excluded-with-a-reason, so a new field fails until somebody says which. `commit_date`, `author_name` and `author_email` are exported now: `collect.mjs` has gathered them on every scheduled run for months, and they are what a disputed submission time is settled with.

**One CSV cell, written and read back by the same module.** `lib/csv-cell.mjs` holds `csvCell` and `stripFormulaGuard` as a pair: a value beginning `=`, `+`, `-` or `@` is written with a leading apostrophe, so a spreadsheet renders it rather than executing it, and `coerceCell` in `lib/roster-csv.mjs` removes that apostrophe on the way back in. They are inverses because the round trip is a documented procedure - the Roster tab tells a lecturer to export, edit and re-import - and for months only the outward half existed, in three byte-identical copies (`RosterTab.vue`, `AssignmentDetailView.vue`, `report.mjs`) none of which owned the other direction. The inverse undoes its own rule and no more: `'t Hooft` is a Flemish surname, not a guarded formula, and the exporter never touched it. `tests/csv-cell.test.mjs` asserts the pair over a table.

**A roster is still worth having under `open`.** It stops being a gate, not a record: a student who accepts and is on the roster carries their student number, name and class group into the report and the CSV export. **But under `open` it is not the population.** `report.mjs` builds that from the union of acceptances, repositories, observations and - under `enforced` and `claim` only - the roster, so a gated assignment lists every roster student before they accept ("1 of 22"), while an open one lists only the students who did. The roster is org-wide and keeps every student ever promoted into it, repeaters included, so on an open assignment it would list previous years' students as "Not accepted" who were never invited. Under `open` a student who is not on the roster still accepts - their row simply carries the GitHub login and nothing else until the lecturer reconciles it.
...
- **Manifests:** Each team is stored as a JSON document in `teams/<id>/<team-slug>.json` conforming to `team.schema.json`. **They are the only pre-assignment acceptance reads.** It used to fall back to the roster's own columns, and both halves of that fallback were wrong. `students[].teams` was a per-assignment map that *nothing wrote* - not the CSV import, not quick add, not promotion, not seeding - a schema field with a reader and no writer, which is the shape that invites somebody to add a writer in the wrong place; it is gone. `students[].team_slug` was worse because it worked: it carries **no assignment**, one slug per student for the whole organization, so a student seeded into `team-alpha` in September was silently pre-assigned to `team-alpha` by December's assignment. It is now what RUNBOOK always claimed it was - a seed `lib/seed-teams.mjs` reads once. The consequence is real and belongs in the runbook: **a `pre-assigned` assignment must be seeded before students accept**, or they meet `rejected:no-assigned-team` (or `unassigned_fallback`, if the lecturer allowed it).
...
- **The student's records move in the same commit.** Moving, adding or removing a member also rewrites that student's `repositories/<id>/<login>.json` and `acceptances/<id>/<login>.json` (`lib/team-member-records.mjs`), because the collector and lockdown read the repository record, never the manifest. A record that would name a repository the student no longer has is deleted rather than left for lockdown to demote them on. A stored acceptance counts as "already accepted" only into the team it names.
- **Team capacity is the assignment's `max_team_size` as it is now.** A manifest's `max_members` is a snapshot from when the team was created and is never read for capacity, by the gate, the public teams file or the student page.
...
- `acceptance-handler.yml` after a student accepts (so the public Pages assignments list is rebuilt, and real-time interim reports are generated to keep `reports/dashboard.json` updated). A rejected acceptance does not trigger it.
- `daily-activity.yml` after the nightly run (so the dashboard reflects new observations and finalized state).
- `publish-assignment.yml` at the end of a publish or republish.
- The SPA, after any write that changes what a student's page shows: saving a published assignment whose broker exists (where it does not, Save publishes instead and the publish regenerates), a state change into or out of `published` or `closed`, a cap change on the cohort page, and every team edit or seeding. `frontend/src/lib/student-pages.js` is the one dispatcher; `frontend/src/lib/publish.js` decides when. The hub enforces the stored document from the commit on while the page shows the card from the last run, so without this the two disagree until the next nightly, and for about two minutes with it.
- Manual `workflow_dispatch` (for repair).

It also **reconciles** `reports/dashboard.json` against the assignments that still exist, in a step of its own (`scripts/prune-dashboard.mjs`, `if: always()`). That reconciliation used to live inside `report.mjs`, which made it a side effect of generating some other assignment's report - and `generate-interim-reports.mjs` generates reports for `published` and `closed` assignments only, so an organization whose remaining assignments were all draft or archived never reconciled at all and kept a deleted assignment's card indefinitely. The decision itself is `pruneMissingAssignments` in `lib/dashboard-aggregate.mjs`, shared by both callers. An **unreadable** listing prunes nothing - it is not evidence that every assignment is gone. An **empty** one is a different answer and does prune, which is what covers the organization that deleted its *last* assignment: with none left there is no other assignment's run to carry the reconciliation. The two are told apart by the scaffold's `.gitkeep` (`SCAFFOLD_KEEPFILE`), which `scripts/scaffold-control-repo.mjs` writes into every directory in §5.1, so the function takes the raw listing rather than a set of ids - a listing carrying neither an assignment nor the marker established nothing and prunes nothing. `lib/control-layout.mjs` owns `assignmentIdFromFile`, the inverse of `assignmentPath`, so the two callers no longer spell the filename rule themselves. The prune does not restamp `generated_at`, which records when the numbers were computed. The Admin Panel's delete is unaffected: it removes the entry in the same commit as the rest of the deletion, and this is the net under everything that does not go through it.
...
| `daily-activity.yml` | `cron 0 0 * * *` + `workflow_dispatch` | Nightly: collect, finalize finalizable assignments, disable self when idle. **Disabled when no class active.** Collect and the finalizable scan are per-org matrices, and one org's failure isolates to that org: every job after them runs with `always()` and checks the result it needs, so the other orgs still finalize. `scripts/aggregate-finalizable.mjs` merges the scan and says `complete` only when every org reported a count; the nightly disables itself only on a complete, unscoped count of zero. Within finalize, every step after collect runs past an earlier failure: lockdown unless the run was cancelled, preservation and everything after it always. |
...
**Dependency updates:** `.github/dependabot.yml` has Dependabot propose updates every Monday to the hub's npm packages (`/`, `/cli`, `/frontend`) and to every pinned action (`.github/workflows` and each composite action's directory). They are the only pull requests the hub takes, and their branches are created through the Dependabot app's bypass on the `Block ad-hoc branch creation` ruleset (INSTALL.md §3.4). Listing `/acceptance` also covers the broker template, whose pins `tests/dependabot-config.test.mjs` requires to equal the hub's. Merging one is [ADMIN.md §8](ADMIN.md).
...
8. acceptance-handler.yml in the hub:
   ...
   c. Runs ./acceptance - validates payload; under enforced and claim checks the
      ...
      opens_at..deadline_at and max_acceptances; asks the org whether the target
      repository name is already taken and whether it is frozen (§5.4); writes
...
Vue 3 SPA, built with Vite, deployed as static files to GitHub Pages from the hub. No server runtime. Auth state is kept in memory and in localStorage until GitHub's 8-hour expiry or a sign-out, so a new tab or a browser restart does not ask for another device code; `frontend/src/lib/auth-storage.js` is the one module that reads and writes it. The token is never refreshed and never kept past its expiry. Every tab follows the stored sign-in: a sign-in, sign-out or account switch in one tab reloads the others (`followSignInAcrossTabs` in `api.js`), except a tab with unsaved edits, which keeps them and is told what changed. What that costs: on a shared computer the next person is still signed in until expiry, and Sign out removes the token from the browser without revoking it at GitHub, because revoking needs the App's client secret (github.com/settings/applications revokes it).
...
- **Lecturer dashboard:** the lecturer's own token reads the per-org control repo's `reports/dashboard.json` directly via Contents API. One fetch - not N per-student calls. When that file is absent (a newly onboarded org, before the first publish or nightly), the view falls back to listing `assignments/` - and **counts drafts by reading each YAML's own `state`**, never by counting files. What is missing on that branch is the report, not the publish, so counting files tells a lecturer who has just published two assignments that they have two drafts and should *"publish to track them here"*. The listing carries names only, so each file is fetched (6-way pool, fallback path only) and anything unreadable or unparseable is left out of the count rather than assumed to be a draft. With nothing in draft the copy says what is actually pending instead.
  **When the control repo answers 404**, the answer depends on who is asking, because GitHub returns 404 for a private repository you cannot see. `frontend/src/lib/control-repo-access.js` is the one judge, for the dashboard and the Admin Panel alike. An organization **owner** sees every repository, so their 404 is absence and they get the setup card. An account that is not an owner and has no write on the hub gets the lecturer-view refusal. Write on the hub means only that the account can *run* Setup Organization, so for a non-owner who has it the hub registry decides: `participating-orgs.yml`, read from the public hub through `lib/org-registry.mjs`, the same lookup both diagnostics use. Listed means *set up, but not for this account* (no Lecturer tag, no Set up button, the budget owner named); unlisted means the setup card; an unreadable registry is its own state and offers neither. A non-owner who does run Setup cannot see the control repository appear, so the wait ends on the registry listing the org, which is Setup's last step. The Admin Panel disables **New assignment** in all of these, since the assignment would be saved into that repository.
...
- **Refresh / Live Status & Student Hover Tooltips (AssignmentDetailView).** The per-assignment detail view exposes a "Refresh" button that re-queries the tip of the assignment's **submission branch** (`/repos/<org>/<repo>/commits?sha=<branch>&per_page=1`, as the collector does) for each provisioned student (concurrency 6) and recomputes `submission_status` against `effective_deadline_at` with nightly semantics: a post-deadline commit never downgrades a student who has an on-time submission on record (it records `first_late_sha`, not a `late` status). Refresh also captures `author_name` and `author_email` from commit objects. Hovering over a student's username renders a smart tooltip resolving identity across a 4-tier hierarchy: (1) institutional roster (`students/roster.yml`), (2) Git commit author email/name (prioritizing real email, suppressing noreply addresses and bot names), (3) GitHub public user profile (`GET /users/{login}`, batched in the background without blocking render), and (4) clean fallback. The updated `reports/<id>.json` is committed back to the control repo with `live_refreshed_at` + `live_refreshed_by` set, **in one commit with an observation per refreshed student** (`observations/<id>/<login>/<ts>.json`, `collection_type: manual`, built by `lib/observation.mjs` - the same builder the collector uses). A rebuild of the report reads observations and nothing else, so a Refresh stored in the report alone was undone by the next rebuild any Admin Panel save triggers; with the observation it is evidence, and the report's own rules classify it. A row with no `repo_id` gets no observation (the schema requires one) - only when every student refreshed successfully; a partial refresh (rate limit, transient errors) is surfaced and not persisted. Backend `collect/collect.mjs` also gathers `commit_count`, `commit_date`, `author_name`, and `author_email` during scheduled runs so static reports populate automatically. The view's CSV export is generated client-side from the report currently on screen, matching the table.
...
**An empty result and an unanswered request are never rendered the same way.** Two places in the Admin Panel turn a count into a claim a lecturer acts on, and both distinguish "none" from "we could not find out": `listOrgTemplates` passes `{ failFast: true }` to its `listOrgRepos` fallback (that leg runs only because the `template:true` search could not be read in full - it failed, or it came back demonstrably partial - so an empty list there is no evidence rather than evidence of none), and `RosterTab` exposes `studentCount: null` when the roster read failed. A confident zero in either place sends the lecturer to create something they already have. `RosterTab` also exposes `linkedCount`, because `github_login` is optional in the roster CSV and is the only field `accept.mjs` matches on - a roster with no usernames is a roster nobody can accept against.

**A template in another organization must be public, and the form is not the authority on that.** `POST /repos/{template_owner}/{template_repo}/generate` is one call needing read on the template and create in the course org, served by an App installation token minted for the course org - and a token is minted per installation. Measured 2026-09-07 on the live testbed: a **public** template in another org provisions normally (ownership is irrelevant, since the token does not authenticate as the lecturer), a **private** one is `HTTP 404` even where the App is installed on that other org too. `lib/template-source.mjs` is the single judge. The Admin Panel consults it while the lecturer types and blocks the save - but it probes with the **lecturer's** token, which can see their own private repository elsewhere, so its verdict is advisory by construction. `scripts/check-publish-preflight.mjs`, run from `publish-assignment.yml`, re-asks with the org's App installation token: the first surface holding the credential that actually has to answer, and the last moment before a student can accept. It **refuses** an unusable template - including one nobody ticked as a template, which until then also only surfaced per student, and one with no commits, which `GET /repos` cannot show (it names a default branch that does not exist yet) and `GET .../commits` answers with 409 (`templateHasCommits`) - and **warns**, from a live read of `plan.name`, when a free organization will blunt this assignment's deadline (`assignmentFreezePlanFinding`; `lock_down_enabled` defaults to `true`, so most assignments freeze without saying so). It covers `late_policy: block` as well, and that is the case it originally missed: blocking with no demotion is what a lecturer picks to stop late pushes *without* losing Actions and secrets - the form unticks the demotion box for them - and on free the ruleset cannot be created, so §11.2.1's degradation demotes anyway. The finding gated on `lock_down_enabled`, which is false in exactly that configuration, so the one setting chosen to protect the toolchain was the one nothing warned about. The plan verdict is stored nowhere: GitHub Team is free for verified educators, and a recorded "this cannot freeze properly" would outlive the fact.

**The template is pinned by its immutable repository id.** `template.repository_id` is taken on first use and compared afterwards by `resolveTemplatePin`. A **rename** is deliberately invisible - GitHub redirects a renamed repository and the id does not change - so what the pin catches is the case the name cannot: the template deleted and recreated under the same name, or transferred away and the name reused, which would put students who accept later on different starter code with nothing recording it. The pin is stored inside `template`, beside the owner and repository it was taken from, so pointing an assignment at a *different* template is a new pin rather than a mismatch. Three surfaces read it: the Admin Panel **warns** (only the lecturer knows whether the new repository is the starter code they meant, and saving is how they accept it), the publish preflight **refuses**, and `provisioning/provision.mjs` **refuses** with `fail:template-replaced` - the last one because publishing does not happen again while a cohort is accepting, so it is the only check between a replaced template and half a cohort on different code.

**The submission branch is the template's default branch.** Provisioning generates with `include_all_branches: false`, so a student repository has the template's default branch and no other, while the collector, the lockdown ruleset and the autograding trigger all read `submission_ref`. `submissionBranchProvisioned` in `lib/template-source.mjs` is the judge: the Admin Panel fills a **new** assignment's ref from the probed default branch until the lecturer types one, and otherwise **warns** under the template (never writing into an existing assignment, which would make opening it look edited); the publish preflight **refuses**, reading `default_branch` with the org's installation token. An unreadable default branch is `unknown`, never a guessed `main`. The collector, where a live assignment already disagrees, reports `branch <name> does not exist (the repository's default branch is <name>)` rather than a bare `HTTP 404`.

**The template wall links out; it never creates the course's template on the lecturer's behalf.** Considered and rejected: an org with no template repository is the one moment a "create one for me" button is most tempting, and the App holds `administration: write`, so it could. It does not, for two reasons. A template is course material with a lifetime measured in years - the system would be creating something it then has no opinion about and never touches again - and `is_template` is a *setting on an existing repository*, so the common case is not an absent repository at all but one whose checkbox is unticked. The wall therefore explains what a template repository is, names the Settings checkbox that is the actual reason the list is empty, and links to GitHub's own new-repository page. The combobox stays beside it: typing `owner/repo` is the only way to name a template in another org, and an org with none of its own is exactly when someone reaches for one.

**The one repository the panel does create is the blank starter, and it is not course material.** "The students start from nothing" is an ordinary assignment and GitHub Classroom answered it by making the template optional; here it cannot be, because `generate` refuses a repository with no commits. That difference is acted on by lecturers, which is the half the paragraph above did not cover: it reasons about a template that exists to be filled with starter code, and this one exists to hold none. Under the picker, *Nothing to start from?* -> **Create a blank starter** makes `starter-<slug>` in the course org with `auto_init` and `is_template` set in the single `POST /orgs/{org}/repos` call - the commit and the checkbox are exactly what a lecturer doing it by hand gets wrong - then writes the full name into `form.template`, where the ordinary probe takes the pin and fills `submission_ref` from the real default branch. `frontend/src/lib/blank-starter.js` holds the live readings and judges the reply: a 403 is the organization's own *Members can create repositories* setting and leaves the manual route above untouched, and a name that already exists is **refused, never adopted** - `starter-<slug>` pins the slug rather than the assignment id, and last year's `portfolio` assignment produces the same name (the general rule is in `lib/existing-repo.mjs`). New assignments only: on one that already exists this would repoint a template students may hold repositories generated from.
...
"Ruleset" is whichever rung held the repository - the organization ruleset by default, a repository ruleset where the assignment opts out or the organization one could not be created - and Phase 4 demotes on top of **either**. It is decided per repository by `demotesAfterStop` in `lib/lock-scope.mjs`: demote unless Phase 1 already demoted that one. The condition used to compare the run's method with `ruleset`, so when organization scope became the default the bottom row silently lost its demotion. Written the other way round, a rung added later inherits the demotion rather than dropping it. The row records `demoted: true` beside its `lock_method`, and the reopen (§11.2.4) undoes both.
...
**Organization scope closes both, and it is the DEFAULT under `late_policy: block`.** One ruleset named `pxl-classroom-deadline-<assignment-id>` covers the whole cohort, each student's repository lists it as `source_type: "Organization"` - visible to them, manageable only by an org owner - and one `PUT` flips all of them regardless of cohort size.

`lib/lock-scope.mjs` decides. **Absent means organization scope**, which reinterprets documents written before 2026-09-09 and is the one place this system does that: the configuration the flag exists for is *"the deadline is final and the student keeps their Actions and secrets"*, and that is exactly the case where the student is still admin of the repository holding the ruleset that stops them. `org_scoped_lock: false` is the explicit opt-out, `lib/assignment-doc.mjs` carries it through a save because the form offers no control that could set it again, and the run log names which of the three cases applied - a default that reinterprets a document nobody edited has to be legible afterwards.

**Targeted by `repository_id`, never by a name pattern.** Name globs collide in real data - `test-groepsopdracht-{team_slug}`, `test-groepsopdracht-2-{team_slug}` and `test-groepsopdracht-vervolg-{team_slug}` all live in one organization, all on `late_policy: block`, and `test-groepsopdracht-*` matches all three cohorts. The id also survives a rename, which is why §13 already calls it the primary external identifier. The list is derived from `planTargets`' own output, so a deferred student and a reopened repository are absent by construction rather than by a loop remembering to skip them.

Measured against a live Team organization on 2026-09-08, because GitHub documents none of it: the `repository_ids` cap is at least **301** (above the 250-student target, so no chunking), `PUT` **replaces** conditions rather than merging them (which is what makes removing one repository work), a ruleset name is accepted at 252 characters and refused at 262 (assignment ids are capped at 100, so no truncation is needed), and **a create carrying a repository that has been deleted is refused 422 while an update carrying one is accepted**. That asymmetry is what `ensureOrgSubmissionLock` handles: a student can delete their own repository and the control-repo record survives it, so the create is attempted and a 422 triggers one re-derivation of the live ids and one retry, naming who was dropped. Verifying every repository first would be N reads before the one time-critical call, which is the entire point of the change.

**Three rungs, each a degradation rather than a failure:** organization ruleset, then repository ruleset, then demotion. A target whose record carries no `repo_id` falls to the repository path individually rather than being dropped from the list - an id missing from an organization ruleset is a student nothing locks and nothing reports. `lock_method` is per row and carries which rung held each repository, so `lib/repo-unlock.mjs` applies the matching inverse: under org scope that is removing one id from an object covering everybody else, not flipping a repository ruleset that is not there. During a migration a cohort can hold both, and the unlock covers **both directions** - an `org-ruleset` row releases any leftover repository ruleset it finds, and a `ruleset` row asks whether an organization ruleset is covering that repository before reporting success. The second is the one that arises on its own: a migration writes the rows it changed, but that write is not committed until its run ends, so a run that dies in between leaves rows naming a rung that is no longer the lock.

Deleting the assignment takes the organization ruleset with it (`org_ruleset_removed` on the retired manifest). Repository rulesets need no such cleanup - they live inside student repositories, which delete never touches, so they go when those go.

`scripts/migrate-org-lock.mjs` moves an existing cohort: create the organization ruleset, **verify it reads back active**, write the control repo, and only then disable the per-repository ones - never the reverse, because deleting first leaves a window with no lock. The write-back is `org_scoped_lock: true` on the assignment and `lock_method: "org-ruleset"` on the rows it moved, and it sits before the disable because that is the recoverable order: rows naming the organization ruleset while a repository one is still active are released by both halves of the unlock, and the opposite is a green reopen over a student who cannot push. A repository the create dropped as deleted keeps its row, because nothing covers it. It runs from the hub with the App token, so whoever dispatches it need not own the organization being migrated, and its dry run is the default.

Free organizations are unaffected by setting it: no ruleset applies to a private repository there, so the ladder degrades to demotion exactly as it does today - and `assignmentFreezePlanFinding` warns at publish.

This is also why `organization_administration` is deliberately **not** narrowed back to `read` (ADMIN.md §3.2): it is what the organization ruleset needs, a reduction is instant, and restoring one needs every participating org owner to approve. All 13 readable installations grant it (checked 2026-09-08); five participating organizations are unreadable without owner access, so that is a floor rather than a total.
...
**`assignment_ids` is where a sentinel starts, not what it stops.** The matrix is fixed when the job is queued, and an exam publishes its groups at different times: a group published afterwards with the same deadline arms another job in the same concurrency group, which can only *wait* behind the running one and reaches the instant after it has gone. Nor is the queue a fallback, because GitHub keeps **one** pending job per group and the newest arrival cancels the one before it - so the list that survives in the queue is whichever arrived last, not whichever knows the most. The running sentinel therefore re-reads the control repo every poll and stops **every** assignment whose deadline is its instant, asking `sentinelInstant` (`lib/sentinel-window.mjs`), which is the same predicate arming used - one rule, so a running watch and an arm queued behind it cannot disagree about who is due. It re-reads the **repository records** on the same poll for the same reason: a sentinel armed at publish is running before anybody has accepted, and one that read them once watched an empty cohort for a whole exam. Both reads are one directory listing plus a blob for whatever changed, so waiting costs a call per poll rather than a call per assignment.

**A watch outlives its credential, so it runs in phases.** A GitHub App installation token lives for at most an hour and this job waits for up to 4h45m: once it expires every poll answers 401, the credential the control checkout persisted is dead, and the `git pull` *at* the instant fails, which skips Stop writes entirely. Three real deadlines on `PXL-Automation-II` went red that way having stopped nothing (runs 35220220960, 35181092340, 35120380004) - every watch armed more than an hour ahead, which is most of them. So the job polls for 45 minutes on its first token, **idles holding no credential at all**, then mints a fresh token 35 minutes before the instant for the last stretch, the stop, the timeline push and the finalize dispatch. A sentinel armed inside that last stretch reaches the instant on its first token and there is no second phase. `SENTINEL_PHASE_MS` says when to hand over and `SENTINEL_STATE_FILE` is what is handed: the samples taken and the group as it stood, so **one watch writes one timeline** however many tokens it took. The idle is deliberate: there is nothing to poll with, and the push record that settles a dispute is the one around the instant. `SENTINEL_ARMED_AT` keeps the two instants apart in a later phase - it waits for the deadline as last read, while the group it is responsible for is still the instant it was armed for, which is the concurrency key.

A duplicate sentinel is still ordinary - a publish, an edit and the cron each arm one - and it re-stops the same group idempotently after the instant. It writes its **own** timeline: `sentinel-<key>.json` is kept by whoever wrote it first and a later one lands beside it as `sentinel-<key>-<run>-<attempt>.json`, because the duplicate's single poll after the deadline used to overwrite the record of the sentinel that actually stopped the cohort. `lockdown.mjs` reads every `sentinel-*.json` and credits the earliest that fired.
...
| Two sentinels overlap | The concurrency group queues the second; it re-stops the same group idempotently and keeps its own timeline |
| A group is published after the sentinel was armed | The running sentinel finds it at its next poll and stops it at the instant with the rest |
| One assignment's stop fails | The others are still stopped; the step fails afterwards naming it, and the nightly locks what it missed |
| The watch outlasts its App token | It hands the evidence over, idles without a credential, and mints a fresh token for the last 35 minutes |
...
#### 11.2.4 Reopening one repository

A deadline is a rule about a cohort; a repository belongs to a person. The two come apart often enough - a certificate that arrives after the freeze, an appeal upheld, a resit in the same repository, a repository frozen by a run that should not have included it - that the honest answer is not "no way back". The alternative to a control is an organization owner clicking in the GitHub UI, which records nothing at all, and a grade dispute months later asks whether this student could have pushed after the deadline.

**Unlocking is not deleting the ruleset.** `enforcement` is a flag (§11.2.1): `releaseSubmissionLock` sends `{ enforcement: "disabled" }` and nothing else, so the rules and the App's bypass survive and re-locking later is one more flip. A deleted ruleset re-created without the App in `bypass_actors` would lock this system out of the repository along with the student. It never *creates* one either: an absent ruleset means nothing is locked, and answering that by creating a disabled one would leave a ruleset on a repository nobody asked to have one, reported as a successful unlock.

**Two lock methods, two inverses.** `lockdown.mjs` prefers a ruleset and degrades to demoting the student per repository, so one assignment can hold both. `lock_method` on the lockdown record's row says which happened for *this* repository and the inverse is read from it - undoing a demotion with a ruleset flip would report success over a student who still cannot push. Restoring a demotion restores the assignment's own `student_permission`, because the lockdown record stores `permission_after` alone and never says what the student had before; `admin` is the fallback, matching the schema's default, so an assignment predating the field is not silently left read-only.

**A ruleset row can carry a demotion as well.** Under `lock_down_enabled` Phase 4 demotes on top of the ruleset and records `demoted: true` on the row, so that row takes both inverses: the ruleset is released first, then the permission restored. That order is the one a retry survives - an id already out of the organization ruleset and an already-disabled repository ruleset both answer `already`, so a restore that failed is retried from the same row - and a failure after the release says the student is still read-only rather than reporting a reopen. `unlockability` returns a non-null `permission` whenever there is a demotion to undo, whatever the method.

**Preservation first, always.** `lib/repo-unlock.mjs` refuses while the deadline snapshot is `pending` - reopening then lets the work being graded move before it is safely in the archive - and refuses differently when preservation `failed`, because waiting will not fix that and there would be nothing to grade against. `not-required` permits it: nothing was preserved because there was nothing to preserve. An absent status refuses; unreadable is not evidence.

**It is written down.** `lockdowns/<id>/unlocked/<login>.json` (`schemas/unlock-record.schema.json`) records who, when, why, which method was undone, and the preserved SHA that was already in the archive at the time - one file per student, *beside* the lockdown record rather than inside it, because the record is re-written whole by the next finalize attempt and would erase it. The reason is required and never defaulted: this document exists to be read by somebody who was not there. It is written **after** the unlock succeeds, and a write that fails still reports the repository as open, because the lecturer needs to know the one document a dispute would rest on is missing.

Editing a repository ruleset needs admin on it, so a lecturer who is hub-writable without owning the organization gets a 403 - reported as such, naming who can, rather than as "unlock failed". `tests/repo-unlock.test.mjs`, `tests/e2e/62-unlock-repository.spec.mjs`.

**And the reopen survives the next run, which it did not at first.** A finalize is not once: `find-finalizable.mjs` re-queues an assignment while preservation is incomplete **and whenever any student's deferred extension expires** (§6.2.2), and `planTargets` builds its list from every repository record. So a second pass re-locked a repository a lecturer had deliberately reopened - triggered by granting somebody *else* more time, two sections away in the same dialog. `lockdown.mjs` reads `lockdowns/<id>/unlocked/` before planning and excludes those repositories exactly as it excludes a deferral: never stopped, never demoted, never touched. Matched over `teamMembers`, because a group repository is one object with one lock on it and reopening it for one member reopened it for the team. An unreadable record counts as reopened - failing the other way locks out a student a lecturer deliberately let back in. Counted as `reopened_count` and named in the run, because a decision honoured silently is indistinguishable from one forgotten. `tests/lockdown-reopened.test.mjs`.
...
**`archive_repo` is read off the report's preserved rows, never composed**, for the reason §5.4 gives about `lib/archive-repo.mjs`: a cohort preserved before per-assignment archives existed is in the org's single legacy `pxl-classroom-archive`, and today's naming rule would name a repository that never held their work. The delete already has the report in hand - it reads it as evidence before removing anything - so this costs no extra call. Beside it, **`preserved_submissions`** records how many were in that archive on the day. The archive deliberately outlives the assignment and the lecturer removes it by hand at the end of the year, which nothing tells this file and nothing can; the count is what makes a later 404 interpretable, separating a deliberate cleanup from a name recorded wrongly. An `archive_deleted_at` was considered and rejected: no surface would ever set it, and a field that is permanently absent means nothing.
...
**2. Student-side (GitHub Actions):** When `execution_environment` is `github_actions`, the tests run automatically on GitHub Actions on every student push. During provisioning, if the template repository already provides its own `.github/workflows/autograding.yml` or `classroom.yml`, it is preserved without overwrite; otherwise, provisioning injects a workflow composed of `classroom-resources/autograding-*-grader` and `classroom-resources/autograding-grading-reporter` actions - **committed to the student's repository, where they can read it**, which is the only arrangement there is; keeping checks out of a student's view is what `lecturer_local` is for. Provisioning waits for GitHub to finish populating the generated repository before it writes, because `POST /generate` returns before that has happened and a write that lands first is either deleted by the population commit or prevents it entirely. **Grades are read at the deadline, by the finalize job.** `scripts/grade-at-deadline.mjs` runs as step 6, after the report and before its commit: the submission is decided by then, the control repository is already checked out and committed, and it costs one Checks read per student inside a job that is running anyway. No new trigger, no new credential, no idle minutes. Grading on the student's *push* is not available and is not an oversight - the grading workflow already runs there on every push; for the hub to react to it, the student's repository would need a credential to dispatch with, which is the incident `close-acceptance.mjs` exists to have ended.

It **never overwrites a summary a person produced**: `graded_by` set, or a `docker`/`host` runner, means a lecturer ran it and the nightly leaves it alone. `graded_by: null` is this path's own signature - the schema's *"the session had no user on hand"* - and is the one thing it may replace, because a finalize run is not once. Every refusal is reported and survived: by then the cohort is locked and its submissions are in the archive, so a non-zero exit would re-queue all of that to fix a score and burn an attempt from the ceiling that stops `find-finalizable` looping.

The same reading is available on demand - the SPA's **Re-grade** for the cohort, **Re-grade this student** on a row, or CLI `pxl-classroom grade`. All of them query the Checks API at each student's preserved or latest observed commit SHA - or at their hand-in commit, where the assignment declares a `submission_marker` - and commit the results to `grading/<id>/summary.json` (validated against `schemas/grading-summary.schema.json`). `lib/grade-cohort.mjs` is the one implementation: which commit to read, what a hand-in message changes, and the three refusals that stop a partial result being written over real grades. It lived inside `AssignmentDetailView.vue`, where the workflow could not reach it and no test could run it.

**A lecturer's decision outranks the rules, and it is STORED, not applied once.** Two override types in `overrides/<id>/<login>.json` (`lib/grade-override.mjs`): `submission_sha` - grade this student on this commit, skipping the deadline, the hand-in cap and which hand-in counts, but still reading that commit's grading run (a commit with none is a named refusal, never a zero) - and `manual_score` - this is the score, read from nothing. Precedence is manual score > chosen commit > the rules; each is append-only and revoked by the same type with `value: null`; a team's decision is written on every member, and a member with none takes the newest teammate's. `gradeStudent` asks it before anything else, so the page's Re-grade, the nightly and `pxl-classroom grade` all honour it - a choice that lived only in the summary would be reverted by the next of them. The summary row records `graded_sha` always and `decided_by` (`kind`, `by`, `at`, `reason`) when a lecturer decided; `score_source: manual` for a score by hand. The CLI's local runner grades the preserved submission only, so for a chosen commit it refuses by name rather than grade something else. The page now refuses to grade whenever the overrides cannot be read, on every assignment rather than only under a cap, since a decision that was not read would be overwritten.

**"Grade this commit now" is a `workflow_dispatch` entry in the grading workflow** (`lib/grade-dispatch.mjs`): input `grade_sha`, a `run-name` that names it, a hand-in gate that lets a dispatch through, and a checkout of that commit. Generated workflows carry it (the starter workflow, provisioning's `buildAutogradingWorkflow`, the shipped templates); existing repositories get it through Sync Starter Code, and a 422 from the dispatch says so. The workflow file is the branch's, so tests written in it are current; test files come from the checked-out commit. A dispatched run belongs to the dispatched branch's **tip**, not to the commit it graded, so two things follow, both measured by `tests/live/grade-dispatch.mjs`. The chosen-commit decision stores the run (`run_id`) and `readRunScore` reads that run's check suite, accepting it only when its event is `workflow_dispatch` and its title names the commit. And every read of "the result at a commit" by the rules drops dispatched runs (`withoutDispatchedRuns`), because their check runs are listed on the tip newest first: without it the tip read the old commit's score. Grading asks whenever there is a check suite (a tip whose only suite is a dispatched one read the old score otherwise); the page's Refresh column asks only when there are two. A 403 on that list keeps every check run: without Actions (read) a lecturer's page token, capped by the App, cannot have dispatched either, and failing closed stopped whole installations grading. A decision whose run no longer exists (deleted, aged out) is a named refusal for that student, never an API failure that blocks the cohort; every unreadable student is named in the refusal. Each dispatch is its own concurrency group, so a second dispatch of the same commit cannot cancel a run a lecturer already recorded. `hasGradeDispatch` requires the whole entry - input, title and checkout - and `findGradingWorkflow` prefers a workflow carrying it, then GitHub Classroom's reporter.

The nightly reads `overrides/` on **every** assignment, capped or not, because a decision lives there too; a score by hand is graded for a student with no repository. On the page a score by hand replaces **Re-grade a commit…** with why a chosen commit would not count, and a decision whose re-read finds no score (back to rules that cannot grade them) replaces the student's summary row with the reason rather than leaving the decided score on record. Granting an extension merges into the student's overrides and refuses when they cannot be read, like every other override write. A single student's score is written the same way (`saveSummaryMerging`): the summary is read with its version, the row merged into THAT, and a conflict re-read and re-merged - merged into the page's copy with a freshly fetched version, a team's second member could replace a row saved a moment before. **Every override write is made against the version it read** (`commitFile`'s `expectedSha`): on a conflict it reads again and appends again, three times, then refuses - fetching the sha afresh just before the write took the other lecturer's and replaced their entry without a conflict. The CSV export carries `graded_sha` and the decision flat (`grade_decision`, `_by`, `_at`, `_reason`, `lib/report-csv.mjs`).

Whether an assignment grades in CI at all is `lib/autograde-source.mjs`'s `gradesInCi`, asked by the workflow and the Admin Panel alike - the permissive answer names every student in a cohort as a grading failure on an assignment that grades nothing. A generated workflow runs on every push; a template's own workflow runs whenever the template says it does.

**The reporter derives the environment variable it reads, so the generator does not get to name it.** `autograding-grading-reporter@v1` looks up `process.env[runner.trim().toUpperCase() + "_RESULTS"]` - upper-cased and nothing else, so a hyphen stays a hyphen, which is what GitHub Classroom's own generated workflows write (`VPC-TEST_RESULTS` for a `vpc-test` runner). `buildAutogradingWorkflow` folded `[^A-Z0-9_]` to `_` instead, guarding against two ids differing only by `-` vs `_` - a collision the schema's `^[a-z0-9][a-z0-9-]{0,63}$` makes impossible, since an underscore never arrives. What the guard did instead was rename the variable, so the lookup was `undefined` for every hyphenated id, which is every id with more than one word. The failure is silent in the way that matters: **every grader step goes green**, the reporter throws on `Buffer.from(undefined, …)`, and its catch block blames the `runners` input - *"must be a comma-separated list of strings"*, which it was. No annotations are emitted, so the score read falls through to the conclusion and records a **0 nobody measured**. `tests/provision-autograding.test.mjs` derives the key from the `runners` list rather than spelling it, because the two hand-written copies of the folded name are what let it stand.

**A test definition means the same thing on both paths.** The `if:` gate is the template's decision, not the platform's - a cloud exam grades against the student's own sandbox account, which is gone once their lab session ends, so the measurement has to be taken at hand-in rather than afterwards from the archive. `submission_marker: { type: commit_message, value: … }` on the assignment records the literal that workflow compares against. It changes exactly one thing: which commit's check run is read. Preservation, deadline classification and what counts as a submission are unaffected and stay with `lib/effective-deadline.mjs` and the collector. `lib/submission-marker.mjs` is the one judge - `readSubmissionMarker` (absent, blank or unknown-typed is *no marker*; an absent `multiple` is **true**), `messageMatchesMarker` (exact and case-sensitive on the whole trimmed message, because the workflow's `==` is) and `findMarkedCommit`.
...
**A hand-in cap** (`submission_marker.max_hand_ins`, read only beside `multiple: true`, absent = unlimited, no schema default) is enforced when scores are read and nowhere else: pushes and workflows are never blocked. `lib/grade-cohort.mjs` `resolveHandIn` is the one decision for the Admin Panel, the nightly and the CLI. Under a cap it counts every hand-in rather than finding the newest: `listHandIns` walks the whole branch (to 1,000 commits) **and** the repository's push run history (`GET /repos/{o}/{r}/actions/runs?event=push&branch=…`, Actions: read), and takes the union - because the lock ruleset that forbids a force-push is `disabled` until the deadline, so a student can erase hand-ins from the branch, and a run is not rewritten by a force-push. Neither source is tamper-proof (anyone with write access can delete a run); the union means both have to be falsified. Order is when GitHub first saw each one pushed (a run's `created_at`, server time), with branch commits never placed before their ancestors; lateness is still the commit's own timestamp against the student's effective deadline, as everywhere else. `selectHandIn` (pure) takes the on-time hand-ins, keeps the first `limit`, grades the last of those (the first under `multiple: false`), and returns every other one in `ignored` with `over-limit` or `late` - a late one uses no place. A walk that hit its cap, or a run history that could not be read, is a failed read and grades nobody, never a count from the branch alone. The count travels in `grading/<id>/summary.json` as `hand_ins` on graded and failed rows alike.

**The limit per student** is the cap plus a `hand_in_allowance` override (`overrides/<id>/<login>.json`, `value` = EXTRA hand-ins so a later change to the cap still composes; `0` revokes; append-only, last valid entry in force; `lib/hand-in-allowance.mjs`). A team shares one repository and so one count, and its most generous member's allowance applies (the rule `effectiveDeadlineFor` applies to extensions). Every grader refuses to grade under a cap over overrides it could not read whole - the SPA (`overridesProblem`), the nightly (`readOverrides`) and the CLI (`listOverrides` throws) - because a grant that was not read grades as if it had never been made. The nightly's score read has its own token (`finalize.gradetoken`, Actions: read) minted with `continue-on-error`, falling back to the finalize token: an installation that never approved Actions skips only the capped count, never lockdown and preservation.
...
The cases this exists for are common: a lecturer fixes a mistake in the template after students have accepted, or - on a semester-long assignment - publishes each new lab by pushing it to the template. A sync brings **each student from where they are up to a template commit** (the newest by default): what they are sent is everything that differs between the template at their own starting point and the template at that commit, not what one commit changed. A sync that sent one commit could never send it again once a later one was pushed.
...
1. **The plan is decided by blob SHAs.** `lib/starter-sync.mjs` compares three git trees - the template at the commit being synced (`head`), the template at **the student's starting point** (`base`), and the student's default branch - read with `GET /git/trees/{ref}?recursive=1`, one request each and cached per commit. Git blob SHAs are content addresses, identical in every repository for identical bytes, so this compares content exactly without fetching a single file. The paths considered are every path that differs between `base` and `head` (`diffTreePaths`, no 300-file cap). Per path: matching `head` is already done, matching `base` means the student never touched it, a file `head` adds that the student already has is **kept** (theirs), anything else is theirs and goes to a pull request.
   **The starting point** (`startingPointFor`, the reads in `lib/starter-sync-cohort.mjs`), first that answers: the newest sync record with `per_student_range` and `all_files` that reached the student with a known start (`from_source` `synced`, `generated`, or `first-commit` / `other-template` with no `files_kept`) - its `template_sha`; else the template commit whose tree the student repository's **first commit** carries, because `generate` copies the tree (all 111 repositories of `PXL-2TIN-NetAdv-26-27` matched, 2026-09-25; two requests per student, via the `last` link); else, when no commit of this template matches - the assignment's template was **changed after they accepted** - the first commit's **own tree** is the base (`from_source: first-commit`, `from_sha` null) and what differs between it and the target is compared, with no `kept`: a copy byte-identical to any version that path ever had in the template's listed history (`templateVersions`, read once per sync) is untouched starter code and is replaced, and anything else is a pull request - because relative to their first commit every new-template file is "added", and the kept rule would file a file an earlier sync delivered as theirs for ever while the record said they held the commit. A file whose first-commit copy already is the target is in range only when they hold it at an older template version; their own edit to it is theirs. **The first commit is trusted as starter code only when GitHub generated it** (`rootCommit`: no parent, the App's bot as author, `web-flow` as committer, a verified signature - measured on a provisioned repository). Then an untouched file of the old starter is replaced or removed on `main`. Otherwise - a force-pushed history, whose first commit is the student's own work - only a missing file is added and a copy the template had is replaced on `main`; everything else is a pull request, a deletion no template version proves included (it was in their first commit, so it may be an old template's file or their own, and only they can say). **A second switch** (A to B to C) starts from the student's last sync instead, which came from B (`template_repo` on the record): B at that commit is exactly what they hold and is a template tree, so it is a trusted base (`from_source: other-template`, `from_sha` B's commit) - B's own files are removed if untouched, an untouched copy C changed is updated, an edited one is a pull request. From their first commit (A) B's files were in neither tree, so nothing removed them. **A different name is not a different template**: a renamed, transferred or forked template carries the same commits, so a record whose commit is in this template's listed history takes the ordinary path, never-backwards and `kept` included - keyed on the name alone, a rename made every student "switched" and a sync to an older commit rewound `main`. An earlier template that is gone or private (404/403) drops to the first-commit path; any other failed read fails that student and is retried. **And only on a complete listing** (a listing of exactly the page cap reads one page further to know) of the template's history: "no commit matches their first commit" means a switch only when the list is whole, and a failed or capped listing made every student of an unchanged template look switched. Else, only when that tree cannot be read, the head's parent, which is what every sync did before and is recorded as `from_source: unknown`. Records written before per-student ranges are not evidence of where a student is, and nothing is guessed from timestamps: a commit made at 11:30 and pushed at 11:36 would date a repository generated at 11:33 one lab ahead. A wrong start can never overwrite work - only a file byte-identical to a template version is written - but one that is too late leaves a change unsent. **A sync never moves a student backwards:** a start LATER than the target commit (a `template_commit` older than where the student already is) is sent nothing, because that range runs the wrong way and every file the later commit added would be a clean delete. Order comes from the template's listed history; a start or target not in it is not ordered, and that student gets the old behaviour (`unknown`).
   **The selection** is `["*"]` or `["*", "!path", ...]`: everything the student is behind on, minus what the lecturer unticked. An inclusion list could not carry a student's catch-up once their range reaches past the files the dialog showed. Only an unnarrowed selection makes a record evidence (`all_files`).
...
4. **One planner, three surfaces**, and **one plan per repository**: a team is several records naming one repository, and the CLI plans it once (`oneRecordPerRepo`, `lib/sync-issue.mjs`) - planned per record in parallel, each member opened its own pull request - through the member whose start is best known (a reached sync record), since a starting point is per login. `StarterSyncModal.vue`'s pre-flight, `scripts/sync-starter.mjs` and `pxl-classroom sync-starter` all plan through `planStudent` (`lib/starter-sync-cohort.mjs`), so the modal's preview and the workflow's behaviour cannot drift, and none calls `planStarterSync` directly with one commit's parent as the base (`tests/sync-starter.test.mjs`). The modal reads each student's tree and starting point once, lists the **earlier changes some students are still missing** beside the newest commit's files, and re-decides locally as files are ticked, instead of re-scanning the cohort on every checkbox.
5. **Student Tracking Issues:** an informational Issue in each student repository, naming the pull request where there is one.
6. **Execution & Records:** triggered via the Web UI modal (also opened by `?sync=1` on the assignment page, which is consumed; the Admin Panel links there when a save changes the template of an assignment that has repository records - `frontend/src/lib/template-change.js` decides, a rename is not a change, and the records are read only when the template changed), CLI `pxl-classroom sync-starter`, or `.github/workflows/sync-starter-code.yml` (one run per assignment at a time: `concurrency` without cancelling, and GitHub keeps one waiting run per group). `scripts/sync-starter.mjs` writes `syncs/<assignment-id>/<sync-id>.json` itself, through the Git Data API, validated before every write: at the **start** (`status: running`, `run_id`, `total_students` - and if that write fails, no student is touched), **every 20 students or 2 minutes**, and at the **end** (`completed`, or `stopped` with `remaining` when the sync reaches its own time budget of 38 minutes, before the job's 45-minute timeout can kill it without a word). Each student's row carries the `from_sha` and `from_source` their range started at; a partial record is evidence for exactly the students in it. **The assignment page reads the newest record** (`StarterSyncStatus.vue`, wording from `lib/sync-status.mjs`) when it opens and on *Check again*, never on a timer: a `running` record is checked against its run, so a run that died with its record still `running` reads as stopped, with how many it reached; a completed one is checked against the template's newest commit. **The dialog follows the run it started:** it dispatches with `return_run_details` (so GitHub names the run, rather than the dialog guessing one from a list), then reads that run and the record carrying its `run_id` every 10 seconds **while it is open** - waiting, progress, outcome (`describeFollow`, `lib/sync-status.mjs`) - and stops when the outcome is known or the dialog closes. A run that ends before recording its start sent nothing, and says so. The status line's **Follow** reopens it on a running sync without reading the template or any student repository. On opening, the dialog lists the hub's runs once and names a sync of this assignment that is already queued or running, found by the `run-name` the workflow gives it (`syncRunTitle`). The workflow's `template_commit` input syncs up to a named commit instead of the newest - still from each student's own starting point. **The tracking issue is assigned** to every login whose repository record names that repository (`lib/sync-issue.mjs`, at most 10), in a second call after it is created, so an account GitHub will not assign never costs the issue; the row's `issue_assignees` is what GitHub answered. Assigned is a participating notification and is emailed whether or not the student watches the repository. The CLI writes no record, so a CLI sync is never a starting point; a student it reached starts earlier next time, which sends nothing twice (up to date) and loses nothing.
7. **The cost is per student, not per file.** Text files go into a student's commit through ONE tree request (`lib/gittree.mjs`: a scratch tree written with `content`, read back, and each blob sha checked against git's own hash of the original bytes); only binary files, and any text file GitHub wrote differently, take a blob request each. A 93-file update is ~7 requests per student. The job has 45 minutes, under the App token's one-hour life, and a re-run resumes because a student who already has every selected change is skipped. `tests/live/inline-commit.mjs` checks the shortcut against the real API.
...
`claimed_email`, `claim_verified` and `claim_domain_allowed` are written onto the acceptance record **and** reach `reports/<id>.json` and both CSV exports. Under `claim` the Roster tab answers "who is bound"; under `open` there is often no roster at all, so the report is the only surface those fields have - which is why the Assignment detail view's export dropping all three went unnoticed: the sentence was true of the nightly file nobody opens and false of the button lecturers press. `lib/report-csv.mjs` owns the column list for both now.

**The confirm-email link: the same secret asking a smaller question.** A claim was only ever collected *at acceptance* - `ClaimAddressCard` renders inside the SPA's "Accept assignment" branch - so a student who had already accepted could never be asked again, and an assignment not in `claim` mode never asked at all. That is precisely the population the roster cannot identify: a row promoted from an acceptance holds a login and nothing else. `/:org/c/<secret>` (`ConfirmEmailView.vue`) runs the same card against the same hub key and posts a `pxl-confirm:` title to the **same broker**, which the hub answers by writing `students/claims/<github_id>.json` and stopping - outcome `confirmed` or `already-confirmed`, neither of which the provisioning step fires on. It consults **no window, no cap and no `roster_mode`**: a confirmation hands out nothing, so a deadline has nothing to protect, and the weeks after one are exactly when a lecturer reads their roster. `state: published` still gates it. `acceptance/accept.mjs` reaches it through `recordClaim` with `required` forced on - the *same* function `open` uses, because a confirmation asks the question `open` asks as an aside; what differs is that with no repository behind it, every branch that shrugs there must refuse here.

Reusing the assignment's own keypair, nonce and broker is the whole design and not a convenience: it means a confirmation inherits the assignment's kill switch (`INVITE_ENABLED`, flipped by `close-acceptance.mjs` at finalize) and its revocation (rotate the nonce and every link dies at once), rather than acquiring a lifetime nobody would remember to end. The alternative - a standing per-org receiver - is the broker App's private key on a public repository with no expiry, which is the incident `close-acceptance.mjs` exists to have ended. The cost is stated rather than hidden: **there must be a live assignment to carry a link**, and RUNBOOK says so.

**The purpose is inside the signature, not the prefix.** `signAcceptanceTitle` covers `<kid>.<payload>` and the prefix sits outside it, so a title relabelled from `pxl-confirm:` to `pxl-accept:` would otherwise verify perfectly and provision a repository to a student sent a link that never offered one. `lib/acceptance-signature.mjs` folds the purpose into the **signed subject** (`confirm:<id>` rather than `<id>`), so a swapped prefix makes the verifier expect a digest the payload does not carry and the title is refused as `wrong-assignment` - in both directions. That costs no wire-format change, which matters more here than anywhere: every broker ever published checks the hub out at `main` and runs that file, so a payload byte cannot be added without breaking every live acceptance at once. `verifyAcceptanceTitle` returns the purpose it verified, the broker forwards *that* as `client_payload[kind]` rather than re-reading the title in bash, and an absent kind is `accept` - which every dispatch predating this feature is. `tests/confirm-purpose.test.mjs`, `tests/confirm-accept.test.mjs`, `tests/e2e/64-confirm-email.spec.mjs`.
...
**The address also has a FORM, and a binding is final only while it meets the rules.** `claim_address_format` in deployment.yml (the part before the @ contains a dot, `firstname.lastname` at PXL) is applied by the same judge as the domains, in the same three places: the student's card offers only verified addresses of that form and refuses a typed one; under `claim` the hub refuses one with `rejected:claim-format` (its own reason - "wrong domain" sends a student looking for another mailbox); under `open` it is recorded, and the report computes `claim_format_allowed` against **today's** rule from the student's **current** binding. "Already bound" used to short-circuit every later acceptance and confirm-email link, so a `12345678@` binding could never be corrected: now a binding that fails the current domains or form (`bindingMeetsRules`) is asked again, the new one records what it `replaces`, and a roster row whose address came from the old binding (`email_source: claim`) follows it (`readdressed` in `planClaimPromotion`); one a person set is held. A stale binding with nothing new sent is kept, unless the assignment requires an address. Measured before shipping: 16 of 115 bindings across 28 organizations were the number form, all in one course; 4 roster addresses, none.

**A reused binding passes the gates a new claim does.** The binding is org-wide and three paths write it - the claim gate, open enrolment and the confirm link - and only the claim gate checks the roster and the cohort. So under `claim` an existing binding is reused only when its address is on the roster and inside this assignment's domains, the roster row is in the cohort (`rejected:not-in-cohort` otherwise), and this account is the **first holder** of that address among every binding of it (`rejected:claim-taken` otherwise) - the earliest time each account held it (`heldSince`: `claimed_at`, and the binding's `history` of earlier addresses, so correcting away and back keeps an account's place), a missing time last, the lower `github_id` on a tie; a binding whose address is not on the roster is asked again. **The attempt counter comes before all of that, and those refusals count**: the confirm link writes a binding with no roster check and no cost, so "confirm X, then accept" is a guess at X, and uncounted it was a free roster oracle and a way past a spent counter. A student who ever claimed successfully has no counter (a success deletes it). **Under `claim` the roster outranks the form**: an address the roster registers is admitted whatever it looks like, because the roster already says who it is and the name form is not on it - so the student's card does not filter by form there (`claim_address_format: false` is published for a claim-mode assignment). An unregistered address that fails the form is refused (`rejected:claim-format`) and **counted**: whether an address reaches that refusal depends on the roster, and uncounted, sequential student numbers could be guessed for free. **A confirmation refuses** an address outside the domains or the form (`rejected:claim-domain` / `rejected:claim-format`) unless the roster registers it, rather than recording it as confirmed; a confirmation of a **different** valid address replaces the binding - the student correcting themselves - and the same address again is `already-confirmed`, unless it now arrives GitHub-verified where the record says it was typed, which rewrites it and keeps the original `claimed_at`. The report judges `claim_domain_allowed` against this assignment's domains today and `claim_format_allowed` only under `open`, where the address is the student's own word; under `enforced` neither is flagged. A roster row whose address came from a confirmation follows the **account** to its current binding, so two re-confirmations between nightly runs still reach it - and another account's `replaces` naming the row's old address does not stop it - and is held when the new address is outside the domains.

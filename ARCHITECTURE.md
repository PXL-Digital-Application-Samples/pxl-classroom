...
`scripts/migrate-org-lock.mjs` moves an existing cohort: create the organization ruleset, **verify it reads back active**, and only then disable the per-repository ones - never the reverse, because deleting first leaves a window with no lock. It runs from the hub with the App token, so whoever dispatches it need not own the organization being migrated, and its dry run is the default.

...
**An empty result and an unanswered request are never rendered the same way.** Two places in the Admin Panel turn a count into a claim a lecturer acts on, and both distinguish "none" from "we could not find out": `listOrgTemplates` passes `{ failFast: true }` to its `listOrgRepos` fallback (that leg runs only because the `template:true` search already failed, so an empty list there is no evidence rather than evidence of none), and `RosterTab` exposes `studentCount: null` when the roster read failed. A confident zero in either place sends the lecturer to create something they already have. `RosterTab` also exposes `linkedCount`, because `github_login` is optional in the roster CSV and is the only field `accept.mjs` matches on - a roster with no usernames is a roster nobody can accept against.

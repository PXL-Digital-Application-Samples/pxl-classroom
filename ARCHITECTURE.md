...
**Organization scope closes both, and it is `org_scoped_lock: true` on the assignment.** One ruleset named `pxl-classroom-deadline-<assignment-id>` covers the whole cohort, each student's repository lists it as `source_type: "Organization"` - visible to them, manageable only by an org owner - and one `PUT` flips all of them regardless of cohort size. It is **opt-in and defaults to absent**, unlike `lock_down_enabled`: absent has to mean the repository-scoped behaviour every existing assignment already has.

...
6. **Execution & Records:** triggered via the Web UI modal, CLI `pxl-classroom sync-starter`, or `.github/workflows/sync-starter-code.yml`. Results are committed to `syncs/<assignment-id>/<sync-id>.json` (validated by `schemas/sync-record.schema.json`), each student's row carrying the `from_sha` and `from_source` their range started at. The workflow's `template_commit` input syncs up to a named commit instead of the newest - still from each student's own starting point. The CLI writes no record, so a CLI sync is never a starting point; a student it reached starts earlier next time, which sends nothing twice (up to date) and loses nothing.

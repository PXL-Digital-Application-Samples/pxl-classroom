...
**A reused binding passes the gates a new claim does.** The binding is org-wide and three paths write it - the claim gate, open enrolment and the confirm link - and only the claim gate checks the roster and the cohort. So under `claim` an existing binding is reused only when its address is on the roster and inside this assignment's domains, the roster row is in the cohort (`rejected:not-in-cohort` otherwise, uncounted), and no earlier binding holds the same address (`rejected:claim-taken`); a binding whose address is not on the roster is asked again. **Under `claim` the roster outranks the form**: an address the roster registers is admitted whatever it looks like, because the roster already says who it is and the name form is not on it - so the student's card does not filter by form there (`claim_address_format: false` is published for a claim-mode assignment), and a form refusal is not counted, the form being public. **A confirmation refuses** an address outside the domains or the form (`rejected:claim-domain` / `rejected:claim-format`, uncounted) rather than recording it as confirmed, and a confirmation of a **different** valid address replaces the binding - the student correcting themselves - where it used to answer "already confirmed" to every correction; the same address again is `already-confirmed`. The report judges `claim_domain_allowed` against this assignment's domains today and `claim_format_allowed` only under `open`, where the address is the student's own word. A roster row whose address came from a confirmation follows the **account** to its current binding, so two re-confirmations between nightly runs still reach it, and is held when the new address is outside the domains.

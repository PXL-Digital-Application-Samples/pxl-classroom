...
//
// Isomorphic: the SPA reads it through `frontend/src/lib/rejection-notice.js`.
// The one import below is another isomorphic module with no imports of its own,
// and it is here rather than two string literals because an outcome spelled one
// way in the producer and another way in this table is the same silent nothing
// as a drifted dedup key - the producer would emit `rejected:repo-frozen`, this
// would not recognise it, and `rejectionReason` falls back to the raw slug
// rather than failing.
import { REJECT_REPO_EXISTS, REJECT_REPO_FROZEN, REJECT_REPO_UNREADABLE } from "./existing-repo.mjs";
...
const REASONS = Object.freeze({
  ...
  "rejected:claim-format": "address does not have the required form (it has to name the student)",
  ...
  // What is already at the name. Both name a repository the lecturer can go and
  // look at, so both say what it is rather than what we decided - the action
  // (reopen it, rename it, or let them have it) is theirs.
  [REJECT_REPO_FROZEN]: "their repository already exists and is frozen by an earlier deadline",
  [REJECT_REPO_EXISTS]: "they already own a repository with this name",
  // NOT one of the two above, and it borrowed one for a day. A read that failed
  // establishes nothing about what is at the name, so saying "they already own
  // one" sends a lecturer looking for a repository that may not exist - and the
  // way to reach this is a configuration fault (an absent token, an App that
  // lost `administration`), which refuses EVERY student at once. It says what
  // actually happened, and points at the thing that can be checked.
  // "what happened and who can fix it" (DESIGN.md §1.6): the App's installation
  // is an administrator's to check, not a lecturer's, so the sentence says so
  // rather than handing them an instruction they cannot act on. If it is
  // transient the student retrying is the whole remedy, and that half is first
  // because it is the likelier one.
  [REJECT_REPO_UNREADABLE]: "we could not read whether a repository is already at their name - they can try again, and if it keeps happening an administrator should check the App's access to this organization",

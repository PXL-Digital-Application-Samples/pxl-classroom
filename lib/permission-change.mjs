...
import { sentinelStoppedAt } from "./sentinel-window.mjs";
 ...
 * @param {Array<{outcome?: string, due?: boolean, deadline_at?: string}>} [args.sentinelTimelines] lockdowns/<id>/sentinel-*.json
 * @param {Date} [args.now]
 ...
 */
export function planPermissionApply({ records, assignment, overrides = null, reopened = [], lockRecord = null, sentinelTimelines = [], now = new Date() }) {
  // THE SENTINEL'S STOP is a lock too. It stops writes at the instant and
  // writes no lock record - that comes with the nightly's finalize - so in the
  // minutes between, a deadline extended after the stop read "future deadline,
  // no lock" and Apply unlocked the cohort (review 2026-09-26). A stop on
  // record holds every repository nobody reopened, whatever the deadline says.
  const sentinelStopped = sentinelStoppedAt(sentinelTimelines) !== null;
  ...
  for (const rec of records || []) {
    ...
    if ((lockedRepos.has(repoKey(repo)) || sentinelStopped) && !reopenedHere) {

 ...
 *
 * THE FORM IS THE DOCUMENT, EXCEPT WHERE IT IS NOT, and the type says only the
 * difference. Everything the editor holds under the same name as the schema is
 * taken FROM the schema (`import("./types.mjs").Assignment`), so a field
 * renamed there stops type-checking here; what is listed below is the editor's
 * own vocabulary - the local datetime pair the browser edits, the flattened
 * autograde controls, the `owner/repo` string behind the template picker, and
 * the two `_original` values that let a save tell an edited timestamp from an
 * untouched one.
 *
 * @typedef {Omit<import("./types.mjs").Assignment, "template"|"opens_at"|"deadline_at"|"autograde"|"submission_marker">
 *   & {
 *     template?: string,
 *     opens_at_local?: string,
 *     deadline_at_local?: string,
 *     _opens_at_original?: string,
 *     _deadline_at_original?: string,
 *     autograde_enabled?: boolean,
 *     autograde_execution_environment?: string,
 *     autograde_tests?: unknown[],
 *     submission_marker_value?: string,
 *     submission_marker_multiple?: boolean,
 *     submission_marker_max_hand_ins?: string|number|null,
 *   }} AssignmentForm
 */

/**
 * @param {AssignmentForm} form the editor's form state (AdminView's `form.value`)
 * @param {object} [opts]
 * @param {string|null} [opts.state] override the state being saved
 * @param {number|null} [opts.templateRepositoryId] the pin, passed in because this
 *   rebuilds `template` whole - leaving it out is how the next unrelated edit
 *   deletes it
 */
export function buildAssignmentDoc(form, { state = null, templateRepositoryId = null } = {}) {
  ...
  return {
    ...
    // `repository_id` is the template's immutable id, so a repository deleted
    // and recreated under the same name is caught instead of adopted
    // (lib/template-source.mjs). It is REBUILT here like everything else, so
    // it has to be passed in - a `template` block reassembled from the form
    // alone would silently delete the pin on the next edit of any field, which
    // is the same defect the top-level sweep in tests/admin-lifecycle-ui
    // exists for and cannot see one level down.
    //
    // Omitted when absent rather than written as null: an assignment that
    // predates pinning has no pin, and a field saying `null` is not the same
    // answer as no field. The schema types it as an integer, so null would
    // also fail validation.
    template: {
      owner: tplOwner || '',
      repository: tplRepo || '',
      ...(Number.isInteger(templateRepositoryId) ? { repository_id: templateRepositoryId } : {}),
    },
    ...
    ...(String(form.submission_marker_value ?? '').trim()
      ? {
          submission_marker: {
            type: 'commit_message',
            value: String(form.submission_marker_value).trim(),
            // Written EXPLICITLY, both ways. The reader defaults an absent
            // field to `true`, so `false` has to be on the document or the
            // next read quietly re-allows re-handing-in.
            multiple: form.submission_marker_multiple !== false,
            // Only beside `multiple: true`: with one hand-in counting there is
            // nothing to cap, and a cap left behind there would come back to
            // life the day somebody ticks the box again. Omitted when blank -
            // absent is "unlimited", and so is every assignment before this.
            ...(form.submission_marker_multiple !== false &&
            readMaxHandIns(Number(form.submission_marker_max_hand_ins)) != null &&
            String(form.submission_marker_max_hand_ins ?? '').trim() !== ''
              ? { max_hand_ins: Number(form.submission_marker_max_hand_ins) }
              : {}),
          },
        }
      : {}),
    ...
    // WHO THIS ASSIGNMENT IS FOR. OMITTED when empty rather than written as
    // `[]`, because absent and empty mean the same thing here - every student on
    // the roster - and an assignment that never narrowed anything should not
    // gain a field saying so. Written only when the roster is actually the gate:
    // under `open` the roster does not decide who accepts, so a cohort there
    // would be a stored value nothing reads.
    ...(Array.isArray(form.cohort) && form.cohort.length && form.roster_mode !== 'open'
      ? { cohort: [...form.cohort] }
      : {}),
    // NO `cohort_groups`. It recorded the class groups REPRESENTED in the
    // selection, which is not a question anyone asks: three students who happen
    // to be in 3A made a card read "3A" for an assignment that is for three
    // people out of twenty. The honest version - only when the whole class is
    // taken - goes stale the moment somebody joins that class. The count is the
    // truth, and DESIGN.md §1.5 would rather have no label than one that
    // over-claims.
    lock_down_enabled: !!form.lock_down_enabled,
    // Carried, not defaulted, and BOTH booleans are carried now.
    //
    // There is no control for this in the form - organization scope is the
    // default under `late_policy: block` (lib/lock-scope.mjs) and `false` is the
    // explicit opt-out, set by hand. This function rebuilds the whole document,
    // so an opt-out that was not carried would be deleted by the next unrelated
    // edit and the cohort would move to organization scope with nobody asking:
    // the same shape as the invitation token this file used to drop, and worse,
    // because the only way back is a field the form does not offer.
    //
    // `typeof === "boolean"` rather than truthiness: absent must stay absent, or
    // an unset form field would be written as an answer.
    ...(typeof form.org_scoped_lock === "boolean"
      ? { org_scoped_lock: form.org_scoped_lock }
      : {}),
    // What acceptance does when a repository already stands at this name.
    // Written only when it was ANSWERED: absent is not "reuse", it is "nobody
    // said", and lib/existing-repo.mjs resolves it by assignment type -
    // individual reuses, team refuses. Writing a value into a document nobody
    // was asked about would turn "never came up" into a decision AND freeze the
    // wrong one onto a team assignment. Carried the same way `org_scoped_lock`
    // is, and for the same reason - this rebuilds the whole document, so an
    // answer that was not carried would be deleted by the next unrelated edit.
    ...(form.existing_repo_policy === "refuse" || form.existing_repo_policy === "reuse"
      ? { existing_repo_policy: form.existing_repo_policy }
      : {}),
    ...
    // Only the opt-OUT is written. Absent is the deployment's address form, and
    // writing `true` would pin it against a later change to deployment.yml -
    // the same reason claim_domains leaves absence alone.
    ...(form.claim_address_format === false ? { claim_address_format: false } : {}),
    ...
    ...(form.autograde_enabled
      ? {
          autograde: {
            ...
            tests: cleanChecks(form.autograde_tests),
          },
        }
      : {}),
    // The lecturer said the TEMPLATE grades it. That answer used to be written
    // nowhere: no autograde block was the document for both "the checks come
    // with my template" and "remove all", so the two were indistinguishable
    // afterwards and the CI grading controls showed up on assignments that
    // grade nothing. Top level rather than inside `autograde`, because that
    // block means "checks defined HERE" - its `tests` has minItems 1, and a
    // template-graded assignment has no checks here to put in it.
    //
    // THREE STATES, and the field carries two of them. `true` and `false` are
    // both answers the lecturer gave and both get written; ABSENT is "nobody
    // has been asked", which is every assignment saved before this existed.
    // Writing `false` for those would claim an answer nobody gave, and it is
    // the value that stops the dialog from re-suggesting the template branch -
    // so a made-up one would silently change what the next Save does.
    //
    // Not written at all when the checks are defined here: `autograde.enabled`
    // already says where the grading comes from, and a second field saying it
    // again is a fact in two places that can disagree.
    ...(typeof form.template_grades === "boolean" && !form.autograde_enabled
      ? { template_grades: form.template_grades }
      : {}),
  }
}

...
export function buildAssignmentDoc(form, { state = null } = {}) {
  ...
  return {
    ...
    ...(Array.isArray(form.cohort) && form.cohort.length && form.roster_mode !== 'open'
      ? { cohort: [...form.cohort] }
      : {}),
    // The groups the selection was made from. A LABEL - lib/cohort.mjs never
    // reads it - so it is carried only alongside a cohort that exists. Storing
    // it without one would leave a card claiming "3A" over an assignment open to
    // the whole roster.
    ...(Array.isArray(form.cohort) && form.cohort.length &&
      Array.isArray(form.cohort_groups) && form.cohort_groups.length && form.roster_mode !== 'open'
      ? { cohort_groups: [...form.cohort_groups] }
      : {}),

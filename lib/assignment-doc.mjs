...
export function buildAssignmentDoc(form, { state = null, templateRepositoryId = null } = {}) {
  ...
  return {
    ...
    ...(typeof form.org_scoped_lock === "boolean"
      ...
      : {}),
    // What acceptance does when the student already owns a repository at this
    // name. Written only when it was ANSWERED: absent means `reuse`
    // (lib/existing-repo.mjs), which is what every assignment predating the
    // field has been doing, and writing `reuse` into a document nobody was
    // asked about would turn "never came up" into a decision. Carried the same
    // way `org_scoped_lock` is, and for the same reason - this rebuilds the
    // whole document, so a `refuse` that was not carried would be deleted by
    // the next unrelated edit and the cohort would go back to reusing.

...
export function buildAssignmentDoc(form, { state = null, templateRepositoryId = null } = {}) {
  ...
  return {
    ...
    // Carried, not defaulted. There is no control for this in the form - it is
    // set by hand or by the migration - and this function rebuilds the whole
    // document, so writing `!!form.org_scoped_lock` would turn an unset form
    // field into `false` and quietly move a cohort back to repository-scoped
    // rulesets on the next unrelated edit. Absent stays absent, which is what
    // every assignment written before it existed means.
    ...(form.org_scoped_lock === true ? { org_scoped_lock: true } : {}),

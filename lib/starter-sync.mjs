    ...
    .filter((r) =>
      (r.results || []).some(
        (x) =>
          ...
          // whole of what they were missing.
          (x?.from_source === "synced" || x?.from_source === "generated" || x?.from_source === "first-commit"),
      ),
    )
...
export function planStarterSync({ headTree, baseTree, studentTree, paths }) {
  ...
  for (const path of paths || []) {
    ...
    } else if (base === undefined && head !== undefined && student !== undefined) {

 ...
 * @returns {{sha: string|null, source: "synced"|"generated"|"unknown", repo?: string|null}}
 */
    ...
    .filter((r) =>
      (r.results || []).some(
        (x) =>
          ...
          // "other-template" (a second switch, planned from the earlier
          // template's commit) on the same terms as first-commit.
          (x?.from_source === "synced" || x?.from_source === "generated" ||
            ((x?.from_source === "first-commit" || x?.from_source === "other-template") && !x?.files_kept)),
      ),
    )
    .sort((a, b) => String(b.synced_at).localeCompare(String(a.synced_at)))[0];
  // `repo`: the template that sync came from - after a SECOND template switch
  // it is not this one, and its commit is still exactly what they hold.
  if (synced) return { sha: synced.template_sha, source: "synced", repo: synced.template_repo ?? null };
...
export function planStarterSync({ headTree, baseTree, studentTree, paths, knownVersions = null, keepAdded = true, trustBase = true }) {
  ...
  for (const path of paths || []) {
    ...
    if (student === head) {
      ...
      // and so is a deletion no template version proves: the file was in
      // their first commit, so it may be the old template's starter code or
      // their own work, and only they can say which. Dropped instead, an old
      // template's files stayed in such a repository for good (review
      // 2026-09-26); proposed on main, a force-pushed student's own files
      // were deleted. A pull request is neither. Never `kept`: kept files mark
      // a record as not having reached the student.
      if (student === undefined || untouched(path, student)) clean.push({ path, action });
      else conflicts.push({ path, action });

...
//      existing.data`) - the previous cohort's work, under the previous
//      deadline's lockdown ruleset, which is `active` from the moment they get
//      it. It is not permanent (`enforcement` is a flag, and the Unlock action
//      flips it), but nothing in provisioning knows to, so the student meets a
//      repository they cannot push to and did not fill. BLOCKS.
//
  ...
  if (repos.length) {
    ...
    findings.push({
      ...
      detail:
        ...
        `a returning student would be handed the old one, holding last year's work and locked by last year's deadline`,
    });
  }
...
export const COLLISION_REMEDY_LEAD = "Three ways forward:";
 ...
 *
 * The recommendation names a TECHNIQUE, not a name. Two reasons, and both are
 * the sort of thing this file exists to get right:
 *
 *   * A computed name is a name nothing has checked. Handing a lecturer
 *     `2627-lab-3` and calling it free would be a claim about a repository
 *     listing this function never asked about - and `2627-lab-3` can be taken
 *     too. The check runs again on what they actually type; that is the answer.
 ...
 *
 * The academic year is offered as the convention that works, at either end:
 * a prefix sorts one year's repositories together in the organization's
 * listing, a suffix keeps the assignment's own name first. Both are fine, and
 * neither is guaranteed.
 *
 * @param {object} args
 * @param {{findings?: Array<{kind: string, blocking: boolean}>}} args.verdict
 * @param {string|null} [args.yearLabel] the academic year, e.g. "2627"
 * @returns {Array<{key: string, label: string, recommended: boolean}>}
 */
export function collisionRemedies({ verdict, yearLabel = null } = {}) {
  ...
  out.push({
    ...
    recommended: true,
    label:
      "Add something to the name that sets this run apart - a prefix, a suffix, or both. " +
      (yearLabel
        ? `The academic year is the usual choice (${yearLabel}), at either end: in front sorts one year's repositories together, behind keeps the assignment's own name first. `
        : "The academic year is the usual choice, at either end: in front sorts one year's repositories together, behind keeps the assignment's own name first. ") +
      "Whatever you pick is checked again when you type it.",
  });

  out.push({
    key: "pattern",
    recommended: false,
    label: kinds.has("pattern-clash")
      ? "Give this assignment a repository name pattern no other assignment uses."
      : "Keep this name and change only the repository name pattern, so the repositories land somewhere new.",
  });

  if (kinds.has("existing-repos") || kinds.has("archive")) {
    out.push({
      key: "delete",
      recommended: false,
      label:
        "Delete what is listed above on GitHub first. That destroys the students' work and the preserved " +
        "submissions - do it only once the grades are out of the system and stored elsewhere.",
    });
  }
 ...
 * @returns {string|null}
 */
export function describeCollisions(verdict, { yearLabel = null } = {}) {
  ...
  const remedies = collisionRemedies({ verdict, yearLabel })
    .map((r) => r.label)
    .join(" ");

...
/**
 * What is wrong with this pattern, or null when nothing is.
 *
 * Two rules beyond "it must contain a placeholder", and both exist because a
 * pattern is turned into a repository NAME by `deriveRepoName`, which does two
 * literal `String.replace` calls and nothing else. Whatever it does not
 * recognise it leaves in place.
 *
 *   1. AN UNKNOWN `{...}` IS COPIED THROUGH VERBATIM. `{slug}-{github_login}`
 *      produces `{slug}-alice`, which is not even a legal GitHub repository
 *      name. This was reachable and it saved: the form seeded exactly that
 *      string in `emptyForm()`, and a lecturer who clicked into the field and
 *      typed a character then deleted it set `manualRepositoryNamePattern`,
 *      after which both auto-writers stood down and the seed survived to the
 *      commit. Measured 2026-09-09. The same hole admits `{login}` - a real
 *      legacy spelling the acceptance tests pin as NOT substituting - and every
 *      typo of the two real names.
 *
 *   2. A PATTERN THAT IS ONLY A PLACEHOLDER names nothing of its own, so every
 *      assignment using it shares one namespace: two assignments with a team
 *      called `team-a` produce the same repository, and the second cohort is
 *      handed the first one's work. Measured across 13 participating
 *      organizations on 2026-09-09, all 20 live assignments carry their id, so
 *      this refuses nothing that exists - it closes the door before somebody
 *      walks through it.
 *
 * @param {string} pattern
 * @param {{assignmentType?: string}} [opts]
 * @returns {string|null} a sentence for the lecturer, or null
 */
export function patternProblem(pattern, { assignmentType } = {}) {
  const text = typeof pattern === "string" ? pattern : "";
  if (!text.trim()) return "Repository name pattern is required.";

  const unknown = (text.match(/\{[^{}]*\}/g) || []).filter((p) => !PLACEHOLDERS.includes(p));
  if (unknown.length) {
    return `${unknown[0]} is not a placeholder, so it would end up in the repository name exactly as written. Use ${PLACEHOLDERS.join(" or ")}.`;
  }

  const wanted = assignmentType === "group" ? PLACEHOLDERS : ["{github_login}"];
  if (!wanted.some((p) => text.includes(p))) {
    return assignmentType === "group"
      ? 'Pattern must contain "{team_slug}" (or "{github_login}").'
      : 'Pattern must contain "{github_login}".';
  }

  if (patternSpecificity(text) === 0) {
    return "Pattern needs something of its own in front of the placeholder, or every assignment using this name produces the same repositories.";
  }
  return null;
}
...
/**
 * Which of these repositories no live assignment accounts for.
 *
 * THE RESIDUAL OF QUESTION (2) ABOVE. A pattern shared with a LIVE assignment
 * blocks, because both are on screen. A pattern landing on the repositories of
 * an assignment that has since been DELETED or ARCHIVED cannot block - the
 * deletion record is written by every delete, so refusing on it would refuse
 * "I opened it, nobody joined, starting over", and a lecturer who cleans up
 * must be believed. So that case was left to acceptance, where `existing-repo`
 * turns a team away by name, which works and tells the lecturer at the first
 * student rather than at the click.
 *
 * This is the click half, and it is deliberately NOT attribution. Which retired
 * assignment those repositories came from is not knowable here: the manifest
 * records the id, the title and the date, and never the pattern, so naming one
 * would be a guess dressed as a finding. What IS computable, exactly, from what
 * the form already holds is the negative: these names match, and no assignment
 * that still exists would produce them. That is the sentence a lecturer can act
 * on, and it costs no request - the caller has both lists in hand already.
 *
 * NO `selfId`, DELIBERATELY, and the first version had one. Excluding the
 * assignment being edited from the live set makes its OWN repositories look
 * like leftovers - the exact opposite of what the exclusion was for. The
 * caller handles self the way `collidingRepoNames`' caller does: the editing
 * path subtracts what the stored pattern already owns before asking, and a new
 * assignment owns nothing. `live` here means every assignment that exists,
 * including this one.
 *
 * @param {string} pattern
 * @param {string[]} repoNames the matching repository names
 * @param {Array<{id?: string, repository_name_pattern?: string}>} [live] assignments that exist
 * @returns {string[]} names no live assignment explains, in the order given
 */
export function retiredPatternClash(pattern, repoNames, live = []) {
  const mine = repoNameMatcher(pattern);
  if (!mine) return [];

  const explained = (Array.isArray(live) ? live : [])
    .map((a) => repoNameMatcher(a?.repository_name_pattern))
    .filter(Boolean);

  return (Array.isArray(repoNames) ? repoNames : [])
    .filter((n) => typeof n === "string" && n.trim())
    .filter((n) => mine.test(n))
    .filter((n) => !explained.some((re) => re.test(n)));
}
 ...
 * @param {string[]} [args.orphanRepos]     of those, the ones no live assignment explains
 * @param {Array<{id: string, pattern: string}>} [args.clashes]
 * @param {boolean} [args.archiveExists]
 * @param {import("./types.mjs").RetiredManifest|null} [args.manifest]     `retired/<id>/manifest.json`, or null
...
export function assignmentCollisions({
  ...
  if (repos.length) {
    ...
    // THE NARROWER FACT, and only when it is narrower. Of those repositories,
    // the ones no assignment that still exists would produce - a deleted or
    // archived cohort's. Said as a count on its own line rather than folded
    // into the sentence above, because it is a different claim: that one is
    // about the organization, this one is about what is left behind.
    //
    // No consequence clause, for the same reason the finding above has none:
    // the control directly beneath asks what should happen to a student who
    // owns one, and answering it here states the premise of the next sentence
    // the lecturer reads. And never blocking - see the head of this file, and
    // the 300 portfolios.
    //
    // Nothing is named. Which retired assignment these came from is not
    // knowable at this point (the manifest records no pattern), and a guess
    // with a name on it reads as evidence.
    const orphans = (Array.isArray(orphanRepos) ? orphanRepos : []).filter(
      (r) => typeof r === "string" && r.trim(),
    );
    if (orphans.length && orphans.length <= repos.length) {
      findings.push({
        kind: "retired-repos",
        count: orphans.length,
        blocking: false,
        detail:
          `${orphans.length} of them belong${orphans.length === 1 ? "s" : ""} to no assignment ` +
          `that still exists`,
      });
    }
  }
 ...
 *
 * @param {object} [args]
 * @param {{clear?: boolean, findings?: Array<{kind?: string, blocking?: boolean, detail?: string}>}} [args.verdict]
 ...
 *
 * @param {import("./types.mjs").Report|null} retiredReport
 * @returns {string[]}
 */

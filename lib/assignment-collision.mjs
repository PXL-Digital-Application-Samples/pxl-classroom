...
export function assignmentCollisions({
  ...
  if (repos.length) {
    ...
    findings.push({
      ...
      blocking: true,
      detail:
        `${repos.length} repositor${repos.length === 1 ? "y" : "ies"} already exist${repos.length === 1 ? "s" : ""}: ` +
        `${shown} - students would get ${repos.length === 1 ? "it" : "those"} back`,
    });
  }
...
// Not "Three ways forward": there are two when nothing exists to delete, and a
// heading that counts wrong is the same defect as a colspan written by hand.
export const COLLISION_REMEDY_LEAD = "Ways forward:";
...
export function collisionRemedies({ verdict, yearLabel = null } = {}) {
  ...
  out.push({
    ...
    recommended: true,
    label: yearLabel
      ? `Add the academic year - ${yearLabel}, in front or behind.`
      : "Add something to the name - the academic year, in front or behind.",
  });
  ...
  out.push({
    ...
    label: kinds.has("pattern-clash")
      ? "Give it a repository name pattern no other assignment uses."
      : "Keep the name and change only the repository name pattern.",
  });
  ...
  if (kinds.has("existing-repos") || kinds.has("archive")) {
    out.push({
      ...
      // cannot be undone and the other two cost nothing.
      label: "Delete what is listed above on GitHub - that destroys the students' work.",
    });
  }

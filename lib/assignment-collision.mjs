 ...
 *
 * The recommendation is a YEAR PREFIX and not a suffix. An academic year spans
 * two calendar years and is written 2627, so `2627-lab-3` beside `2526-lab-3`
 * sorts the whole organization into contiguous years - which is what a
 * repository listing is read for at the end of one.
 *
 * @param {object} args
 * @param {{findings?: Array<{kind: string, blocking: boolean}>}} args.verdict
 * @param {string} [args.suggestedId]  `<year>-<id>`, when the opening date gave one
 * @returns {Array<{key: string, label: string, recommended: boolean}>}
 */
export function collisionRemedies({ verdict, suggestedId = null } = {}) {
  ...
  out.push({
    ...
    recommended: true,
    label: suggestedId
      ? `Prefix the academic year - "${suggestedId}". It never collides, it sorts a year's repositories together, and last year's stays readable.`
      : "Prefix the academic year, the way 2627 means September 2026 to August 2027. It never collides, it sorts a year's repositories together, and last year's stays readable.",
  });
 ...
 * @returns {string|null}
 */
export function describeCollisions(verdict, { suggestedId = null } = {}) {
  ...
  const remedies = collisionRemedies({ verdict, suggestedId })
    .map((r) => r.label)
    .join(" ");

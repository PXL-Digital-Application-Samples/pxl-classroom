 ...
 * @returns {"enforced"|"claim"|"open"} one of ROSTER_MODES
 */
export function normalizeRosterMode(value) {
  // `Array.prototype.includes` is not a type guard, so the membership test that
  // makes this safe is invisible to a reader that only has the types. The cast
  // says what the line above established and nothing more.
  return ROSTER_MODES.includes(/** @type {string} */ (value))
    ? /** @type {"enforced"|"claim"|"open"} */ (value)
    : DEFAULT_ROSTER_MODE;
}

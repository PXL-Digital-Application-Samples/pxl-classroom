...
// The BARE message, and only that: GitHub answers it when another write moved
// the ref at the same moment (measured 2026-09-26 - a live drill committing to
// the same control repository; the next attempt, from a fresh read, went
// through). "Reference cannot be updated: <reason>" names a real refusal (a
// malformed ref) and is still thrown at once.
const CONCURRENT_REF_RE = /^Reference cannot be updated\.?$/i;
...
// Returns true if a 422 error means the ref moved under us: a non-fast-forward,
// or the bare concurrent-update refusal. Both are answered by re-reading.
function isNonFastForward(err) {
  if (err?.status !== 422) return false;
  const msg = String(err?.response?.data?.message || err?.message || "").trim();
  return NON_FF_RE.test(msg) || CONCURRENT_REF_RE.test(msg);
}
...
export function commitFailureMessage(e) {
  ...
  if (err?.code === EXHAUSTED) {
    // Not "the course data": this writer also commits to student repositories
    // (a starter sync), and the message is shown for both.
    return "the repository was being changed by something else at the same moment, so nothing was saved. Wait a few seconds and try again";
  }
  if (err?.status) {

 ...
 * archived.
 *
 * UNREADABLE IS NOT EVIDENCE, and neither is empty. Only a directory that was
 * actually listed, and actually had something in it, is grounds for deciding an
 * assignment is gone: a failed read or a checkout that produced nothing would
 * otherwise delete every card in the organization. That is far worse than the
 * stale card this fixes, so both cases prune nothing.
 *
 * @param {object} dashboard the parsed dashboard document
 * @param {Iterable<string>|null} onDisk assignment ids that exist, or null if unread
 ...
 */
export function pruneMissingAssignments(dashboard, onDisk, { keep = null } = {}) {
  const entries = dashboard?.assignments;
  if (!entries || typeof entries !== "object") return { dashboard, pruned: [] };

  const present = onDisk instanceof Set ? onDisk : onDisk ? new Set(onDisk) : null;
  if (!present || present.size === 0) return { dashboard, pruned: [] };

...
/**
 * Every field is bracketed because the parameter defaults to `{}`; `org` and
 * `assignmentId` are still required of a caller, and the schema this returns is
 * what refuses a manifest without them.
 *
 * @param {object} [args]
 ...
 * @param {import("./types.mjs").ReportStudent[]} [args.students] The report's rows, for where the submissions
 *                                             actually went and how many there were.
 * @param {string} [args.deletedAt]            Override for the timestamp; tests pin it.
 * @param {boolean|null} [args.orgRulesetRemoved] Whether the org-level lock ruleset went with it.
...
export function buildRetiredManifest({
  ...
  students = [],
  deletedAt = null,
  // null/undefined = this cohort was never held by an organization ruleset, so
  // the field is omitted rather than written as a `false` that would read as a
  // failed removal.
  orgRulesetRemoved = null,
} = {}) {
  ...
  return {
    ...
    // The organization ruleset that held this cohort, when there was one, and
    // whether the delete managed to remove it.
    //
    // Repository rulesets need no entry here: they live inside student
    // repositories, which this delete never touches, so they go when those go.
    // An organization one lives in the ORGANIZATION and outlives the assignment
    // if a delete could not remove it - and then it is a rule named after
    // something that no longer exists, still blocking pushes. `false` is the
    // value a reader acts on; absent means the cohort was never locked that way.
    ...(orgRulesetRemoved === null || orgRulesetRemoved === undefined
      ? {}
      : { org_ruleset_removed: orgRulesetRemoved === true }),

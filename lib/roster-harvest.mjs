...
/**
 * Is this actually an address, and one worth putting in an Email column?
 *
 * `author_email` is a free-text git config field: it holds `Tom Cool`, an empty
 * string, or a hostname as readily as an address. And GitHub's own domain is
 * never a person's mailbox - `collect.mjs` already discards
 * `noreply.github.com`, and `IlkayDuranPXL@github.com` is the shape that gets
 * past it: a real-looking address at a domain that does not take mail.
 *
 * Deliberately no stricter than that. Validating an address properly is a
 * famous waste of time, and this is a HINT: `rayane.waddah@student.pxl` has a
 * domain that does not resolve and is still the string that names the student.
 * What it must not do is put a non-address in a column of addresses.
 */
 ...
 *
 * @param {Array<import("./types.mjs").Report>} reports
 ...
 *
 * @param {object} [args]
 * @param {import("./types.mjs").Roster|null} [args.roster] parsed students/roster.yml
 * @param {Array<import("./types.mjs").Report>} [args.reports]
...
export function harvestPlan({ roster = null, reports = [], emailAllowed = () => false } = {}) {
  ...
  for (const s of Array.isArray(roster?.students) ? roster.students : []) {
    ...
    // Shown in the EMAIL column, so it has to be an address. The domain check
    // below decides whether it may be WRITTEN; this decides whether it is an
    // address at all - two different questions, and this is the first one.
    const email = wantsEmail && looksLikeEmail(hit.email) && addsInformation(emailLocalPart(hit.email), login)
      ? hit.email
      : null;
    if (!name && !email) continue;
 ...
 *
 * @param {import("./types.mjs").Roster} roster
 * @param {Array<{login: string, email: string}>} fillable
 * @returns {import("./types.mjs").Roster}
 */
export function applyHarvest(roster, fillable) {
  ...
  return {
    ...
    students: (Array.isArray(roster?.students) ? roster.students : []).map((s) => {
      ...
      // WHERE IT CAME FROM, beside the value. Two writers of very different
      // trust fill this column - a claim carries an address GitHub verified on
      // the student's own account, this carries whatever they typed into `git
      // config` - and without the marker a reader months later cannot tell
      // them apart. Absent means a person put it there.
      return { ...s, email, email_source: "commit" };
    }),
  };
}

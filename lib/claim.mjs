...
// Re-exported below, not re-implemented. It moved to its own module because
// lib/cohort.mjs needs it and sits on the graph of a script that runs with no
// `npm ci` - see the note in lib/normalize-email.mjs.
...
export const CLAIM_REJECTIONS = Object.freeze({
  ...
  // Right domain, wrong form - `12345678@student.pxl.be` where the deployment
  // asks for `firstname.lastname@`. Its own reason, because telling the
  // student their DOMAIN is wrong sends them looking for another address.
  FORMAT: "rejected:claim-format",
  NO_MATCH: "rejected:no-claim-match",
  // On the roster, but not in this assignment's cohort. Distinct from NO_MATCH
  // on purpose: "your address is not registered" and "your address is
  // registered, but this assignment is not for you" send a student to do
  // different things, and telling them the first when the second is true wastes
  // an email to their lecturer.
  NOT_IN_COHORT: "rejected:not-in-cohort",
...
/**
 * The address FORM this assignment requires, or null for any form.
 *
 * `deploymentFormat` is `{ pattern, example }` from deployment.yml, passed in
 * (this module is isomorphic and never imports configuration). An assignment
 * switches it off with `claim_address_format: false`; ABSENT means the
 * deployment's rule, like `claim_domains` - and there is no schema default for
 * the field, because validation would write one (tests/schema-defaults-are-writers).
 *
 * @param {{claim_address_format?: boolean}|null|undefined} assignment
 * @param {{pattern: string, example: string}|null|undefined} deploymentFormat
 * @returns {{regex: RegExp, example: string}|null}
 */
export function resolveAddressFormat(assignment, deploymentFormat) {
  if (assignment?.claim_address_format === false) return null;
  if (!deploymentFormat?.pattern) return null;
  let regex;
  try {
    // `i`: the address is lowercased before it is tested, so a pattern written
    // with capitals (`^[A-Z]`) would otherwise refuse every student.
    regex = new RegExp(deploymentFormat.pattern, "i");
  } catch {
    // lib/deployment.mjs refuses a bad pattern at load, so this is only ever
    // reached by a caller that built its own - and "no rule" is the honest
    // answer to a rule that cannot be read.
    return null;
  }
  return { regex, example: deploymentFormat.example || "" };
}

/**
 * Does the part before the @ have the required form? True when no form is
 * required. The local part only: the domain is `domainAllowed`'s question.
 *
 * `12345678@student.pxl.be` is the case this exists for - an address that
 * cannot tell a lecturer who the student is.
 */
export function addressFormatAllowed(email, format) {
  if (!format) return true;
  const e = String(email ?? "").trim().toLowerCase();
  const at = e.lastIndexOf("@");
  if (at < 1) return false;
  return format.regex.test(e.slice(0, at));
}
 ...
 * acceptance/claim-keys.json. See INSTALL.md §3.2.
 */
 ...
 *
 * @param {object} [issue]
 * @param {unknown} [issue.body] the issue body, untrusted and shape-checked below
 * @returns {{claim_payload: string, claim_verified: boolean}}
 */
...
export function buildClaimRecord({
  ...
  replaces = null,
  // The WHOLE record being replaced, when there is one: its `history` is
  // carried on, with it appended when the address changes.
  previous = null,
}) {
  const earlier = Array.isArray(previous?.history) ? previous.history : [];
  const changed = previous?.email && previous?.claimed_at && normalizeEmail(previous.email) !== normalizeEmail(email);
  const history = (changed ? [...earlier, { email: normalizeEmail(previous.email), claimed_at: previous.claimed_at }] : earlier).slice(-20);
  return {
    schema_version: 1,
    ...
    ...(replaces?.email && replaces?.claimed_at ? { replaces: { email: normalizeEmail(replaces.email), claimed_at: replaces.claimed_at } } : {}),
    ...(history.length ? { history } : {}),
  };
}

/**
 * Since when this binding has held `email`: the earliest time the account
 * held it, its `history` included - so correcting away from an address and
 * back keeps the account's place in "who was first" (review 2026-09-26). An
 * empty string when the record never says.
 *
 * @param {{email?: string, claimed_at?: string, history?: Array<{email: string, claimed_at: string}>}} record
 * @param {string} email
 */
export function heldSince(record, email) {
  const want = normalizeEmail(email);
  const times = [
    ...(normalizeEmail(record?.email) === want && record?.claimed_at ? [record.claimed_at] : []),
    ...(Array.isArray(record?.history) ? record.history : [])
      .filter((h) => normalizeEmail(h?.email) === want && typeof h?.claimed_at === "string")
      .map((h) => h.claimed_at),
  ];
  return times.sort()[0] ?? "";
}
...
/**
 * Does an EXISTING binding still meet the rules this assignment asks for?
 *
 * A binding is org-scoped and was, until 2026-09-26, final: "already bound"
 * short-circuited every later acceptance and every confirm-email link, so a
 * student who had confirmed `12345678@student.pxl.be` could never be asked
 * again once deployment.yml wanted `firstname.lastname@`. One that fails the
 * CURRENT domains or form is not "done": the student is asked again, and the
 * new binding replaces it (`replaces`). One that passes is kept, exactly as
 * before - nobody is re-asked for a rule they already meet.
 *
 * @param {{email?: string}|null|undefined} binding  students/claims/<github_id>.json
 * @param {{domains: string[], format: {regex: RegExp}|null}} rules
 */
export function bindingMeetsRules(binding, { domains, format }) {
  if (!binding?.email) return false;
  return domainAllowed(binding.email, domains) && addressFormatAllowed(binding.email, format);
}

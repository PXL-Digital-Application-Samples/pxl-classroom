...
/**
 * Whether a team manifest's `repo_name` is the repository this acceptance is
 * about to hand out - i.e. this assignment already made it for this team.
 *
 * The manifest stores `owner/name`: its one writer,
 * scripts/write-repository-record.mjs, copies the repository record's
 * `repo_name` onto it. accept.mjs compared that to the bare name, so the answer
 * was never yes and EVERY second member of a team was refused
 * `rejected:repo-exists` over their own team's repository. The schema allows a
 * bare name as well, so both are read; a stated owner must be this org.
 * Repository names and logins are case-insensitive on GitHub.
 *
 * @param {unknown} stored     the team manifest's `repo_name`
 * @param {unknown} org        the organization being provisioned in
 * @param {unknown} targetRepo the bare repository name about to be handed out
 * @returns {boolean}
 */
export function teamManifestNamesRepo(stored, org, targetRepo) {
  if (typeof stored !== "string" || !stored.trim()) return false;
  if (typeof targetRepo !== "string" || !targetRepo) return false;
  const slash = stored.indexOf("/");
  const name = slash === -1 ? stored.trim() : stored.slice(slash + 1).trim();
  if (slash !== -1 && !sameLogin(stored.slice(0, slash).trim(), org)) return false;
  return name.toLowerCase() === targetRepo.toLowerCase();
}
...
/**
 * THE READ FAILED, which is not the same refusal and must not borrow one.
 *
 * This reported `rejected:repo-exists` for a day, so a lecturer whose App had
 * lost `administration` - or whose token was absent - was told "they already own
 * a repository with this name" about every student, and would go looking for
 * repositories that do not exist. A message no branch computed is a guess, and
 * this one was reachable by a configuration fault rather than by anything a
 * student did.
 */
export const REJECT_REPO_UNREADABLE = "rejected:repo-unreadable";

/**
 * All of them, so a reader can iterate rather than name them.
 *
 * `tests/acceptance-outcome-persisted.test.mjs` derives outcomes by scanning
 * accept.mjs for literals, and step 7 rejects through a VARIABLE - so these are
 * invisible to it and have to be handed over. It listed two by name for an
 * hour, and the third was added without joining them, which is the same silent
 * gap one level up. Iterate this; do not re-spell the members.
 */
export const EXISTING_REPO_REJECTIONS = Object.freeze([
  REJECT_REPO_FROZEN,
  REJECT_REPO_EXISTS,
...
/**
 * What a rulesets read means for "is this repository frozen".
 *
 * A 403 IS AN ANSWER, AND THE ANSWER IS NO. Measured 2026-09-09 against two
 * live organizations:
 *
 *   free org, private repo   403 "Upgrade to GitHub Pro or make this
 ...
 *
 * So the 403 is the plan gate, not a permission failure, and reading it as
 * "unreadable" refused every student on a free organization whose repository
 * already existed - which is most of the population this check exists to help.
 *
 * Treating it as NOT FROZEN is right twice over. The feature is unavailable, so
 * no ruleset can be enforcing and the student can push. And the lock would have
 * been written by this same App with this same token: a token that cannot read
 * rulesets here cannot have created one, so there is no lock of ours to find
 * either way. An organization that has downgraded since a lock was applied
 * lands in the same place, correctly - an inert ruleset stops nobody.
 *
 * Anything else that is not a readable array is still `null`, because a 500 or
 * a shape we did not expect establishes nothing.
 *
 * `isOurs` is a parameter rather than an import: the name lives in
 * lib/submission-lock.mjs, which carries lib/audit.mjs and #deployment behind
 * it, and this module is on the SPA's graph.
 *
 * @param {{ok?: boolean, status?: number, data?: unknown}} res
 * @param {(name: unknown) => boolean} isOurs
 * @returns {string|false|null} the lock's name, `false` for none, `null` unreadable
 */
export function frozenFromRulesets(res, isOurs) {
  if (res?.status === 403) return false;
  if (!res?.ok || !Array.isArray(res.data)) return null;
  return res.data.find((r) => isOurs(r?.name))?.name ?? false;
}

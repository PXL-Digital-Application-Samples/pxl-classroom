...
export function hasGradeDispatch(text) {
  const t = String(text ?? "");
  return /\bworkflow_dispatch\b/.test(t)
    && new RegExp(`ref:\\s*\\$\\{\\{\\s*inputs\\.${GRADE_DISPATCH_INPUT}\\b`).test(t)
    && new RegExp(`format\\('Grade \\{0\\} \\(PXL Classroom\\)',\\s*inputs\\.${GRADE_DISPATCH_INPUT}\\)`).test(t);
}

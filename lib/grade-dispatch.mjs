...
export function hasGradeDispatch(text) {
  ...
  const titled = squash(doc?.["run-name"]).includes(`format('Grade {0} (PXL Classroom)', inputs.${GRADE_DISPATCH_INPUT})`);
  const checksOut = Object.values(doc?.jobs || {}).some((job) => (job?.steps || []).some((s) =>
    /actions\/checkout@/.test(String(s?.uses ?? "")) && new RegExp(`inputs\\.${GRADE_DISPATCH_INPUT}\\b`).test(squash(s?.with?.ref))));
  return Boolean(input) && titled && checksOut;
}
...
export async function withoutDispatchedRuns(request, { repoFullName, sha, checkRuns, onlyWhenAmbiguous = false }) {
  ...
  if (res?.status === 403) return { ok: true, checkRuns: runs };

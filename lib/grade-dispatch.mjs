...
/**
 * Does this workflow file carry the WHOLE entry? A text check, the same kind
 * `isGradingWorkflow` is - but of all three parts, because a half-done one
 * burns a run: without the checkout it grades the tip, without the title
 * `readRunScore` refuses the run it produced.
 */
export function hasGradeDispatch(text) {
  // PARSED, not grepped: a serialiser or an editor folds a long `run-name`
  // across lines - valid YAML the text check did not recognise, measured on
  // the live probe (tests/live/grade-dispatch.mjs).
  let doc;
  try {
  ...
  // Declared, whatever its body - `grade_sha:` alone is a declaration.
  const declared = inputs != null && typeof inputs === "object" && Object.hasOwn(inputs, GRADE_DISPATCH_INPUT);
  const titled = new RegExp(`format\\('Grade \\{0\\} \\(PXL Classroom\\)',\\s*inputs\\.${GRADE_DISPATCH_INPUT}\\)`)
    .test(squash(doc?.["run-name"]));
  // ONE JOB that both checks out the chosen commit and lets a dispatch run:
  // a checkout in another job grades the tip in this one, and a hand-in gate
  // without the dispatch clause skips the run it was started for (review
  // 2026-09-26 - a lecturer adding "the three lines" by hand gets both wrong).
  const refUsesInput = new RegExp(`inputs\\.${GRADE_DISPATCH_INPUT}\\b`);
  // The GRADING jobs: those running a GitHub Classroom grader or reporter,
  // or every job when none does (a workflow that grades its own way).
  const jobs = Object.values(doc?.jobs || {});
  const classroom = jobs.filter((job) => (job?.steps || []).some((s) => /classroom-resources\/autograding-/.test(String(s?.uses ?? ""))));
  const grades = (classroom.length ? classroom : jobs).some((job) => {
    const checksOut = (job?.steps || []).some((s) => /actions\/checkout@/.test(String(s?.uses ?? "")) && refUsesInput.test(squash(s?.with?.ref)));
    const gate = job?.if;
    const letsDispatchThrough = gate === undefined || gate === null || gate === true || /workflow_dispatch/.test(String(gate));
    return checksOut && letsDispatchThrough;
  });
  return declared && titled && grades;
}
...
export async function findGradingWorkflow(request, { repo, branch = "main" }) {
  ...
  // One carrying the entry wins; then one with GitHub Classroom's reporter;
  // then anything that merely mentions grading. A lint workflow that says
  // "grading" in a comment must not stand in for the real one and report
  // "sync the updated workflow" when the grading workflow is fine.
  let best = null;
  for (const w of workflows) {
    ...
    const rank = hasGradeDispatch(text) ? 3
      : /classroom-resources\/autograding-grading-reporter/.test(text) ? 2
        : /autograd|grading/i.test(text) ? 1 : 0;
    if (rank > (best?.rank ?? 0)) best = { rank, workflowId: w.id, path: w.path };
    if (rank === 3) break;
  }
  if (best) return { ok: true, workflowId: best.workflowId, path: best.path, available: best.rank === 3 };
 ...
 *
 * Asked whenever there is a check run with a suite - a tip whose ONLY suite is
 * a dispatched run (`[skip ci]`, the push run deleted) would otherwise read the
 * old commit's score. `onlyWhenAmbiguous` (the page's cheap Refresh column,
 * display only) asks only when the check runs span two or more suites.
 *
 * A 403 keeps every check run. Without Actions (read) the App's token cannot
 * list runs - and a lecturer's page token, capped by the App, cannot have
 * dispatched one either, so what remains is a run the student started
 * themselves, which is no more than editing their own workflow (the trust this
 * reader always had). Failing closed there stopped every assignment in an
 * installation without Actions from grading. Any other failure is a failed
 * read, never "no dispatches".
 *
 * @returns {Promise<{ ok: boolean, checkRuns?: object[], status?: number }>}
 */
export async function withoutDispatchedRuns(request, { repoFullName, sha, checkRuns, onlyWhenAmbiguous = false }) {
  const runs = Array.isArray(checkRuns) ? checkRuns : [];
  const suites = new Set(runs.map((c) => c?.check_suite?.id).filter((id) => id != null));
  if (suites.size === 0 || (onlyWhenAmbiguous && suites.size < 2)) return { ok: true, checkRuns: runs };
  const res = await request("GET", `/repos/${repoFullName}/actions/runs?head_sha=${sha}&event=workflow_dispatch&per_page=100`);
  // A PERMISSION 403 only. GitHub answers a rate limit (primary or secondary)
  // with 403 too, and reading that as "no Actions permission" kept the
  // dispatched run - whose check suite is newest - so the old commit's score
  // was written for the tip (review 2026-09-26). A rate limit is a failed read.
  if (res?.status === 403 && !isRateLimited(res)) return { ok: true, checkRuns: runs };
...
/** Is this 403 a rate limit rather than a missing permission? */
function isRateLimited(res) {
  const h = res?.headers;
  const header = (name) => (typeof h?.get === "function" ? h.get(name) : h?.[name]);
  return /rate limit/i.test(String(res?.data?.message ?? "")) || String(header("x-ratelimit-remaining") ?? "") === "0" || header("retry-after") != null;
}
...
export async function readRunScore(request, { repoFullName, runId, sha, fallbackTotal = 0 }) {
  ...
  // Gone - deleted by someone with write access, or aged out. A named refusal
  // for this one student ("no-run"), never an API failure: that would block
  // the whole cohort, for ever, without saying who.
  if (runRes?.status === 404 || runRes?.status === 410) {
    // ...but only if the REPOSITORY is still there: a deleted or transferred
    // repository, or one this token's installation does not include, answers
    // 404 for the run too - and that is a failed read, not the student's
    // decided score replaced by "the run no longer exists" (review 2026-09-26).
    const repo = await request("GET", `/repos/${repoFullName}`);
    if (!repo?.ok) return { verdict: "api-failed", permissionDenied: repo?.status === 403, reason: `could not read the repository ${repoFullName} (HTTP ${repo?.status ?? 0})` };
    return { verdict: "no-run", reason: `the grading run you started for commit ${short} (run ${runId}) no longer exists - grade that commit again, or go back to the rules` };
  }

...
export function rosterKey(student) {
  // DERIVED, not re-implemented. This was a hand-written `num:` then `login:`,
  // which is exactly `rosterIdentities(entry)[0]` - one rule with two spellings
  // in two files, and the shape that let `diffRosters` fork in the first place.
  // Adding a third identity form to one of them and not the other would have
  // been the same defect again.
  if (!student || typeof student !== "object") return null;
  return rosterIdentities(student)[0] ?? null;
}

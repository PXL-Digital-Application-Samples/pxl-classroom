...
// (2026-09-17). They were narrowed in three stages the same day, each proven on
// the testbed as it landed (LESSONS.md, "Every App token asks for exactly its
// row"): a missing permission is a 403 at the moment of use, and some of these
// steps run only at a deadline.
//
...
// only running the job does - tests/live/drill.mjs, the moment it lands.
//
// Row fields:
...
// A row with any "all" carries a `reason`, and a fully scoped row does not.
//
...
export const APP_TOKEN_SCOPES = Object.freeze({
  ...
  ".github/workflows/acceptance-handler.yml#accept.token": {
    ...
    repositories: "all",
    permissions: { contents: "write", issues: "write", metadata: "read" },
    // And the ORG is never validated against that broker at all, on any path -
    // resolveBrokerIssue confines which issue is read and written, not which
    // organization is acted in. So this token is minted for whichever org the
    // payload names. OPEN-ITEMS §10; accepted, not overlooked.
  ...
  // (Secrets: write). Checks (read) is kept although the score read below now
  // has its own token: removing it is a change to a token that runs every
  // organization's finalize, for no gain. The composite actions mint their
  // own. Step 5 runs with continue-on-error: the drill checks the broker is
  // closed.
  ...
  // grade-at-deadline.mjs: commits (Contents: read), check runs and their
  // annotations (Checks: read), and - under a hand-in cap - the push run
  // history the cap counts from (Actions: read, GET /repos/{o}/{r}/actions/runs).
  // Its OWN token, minted with continue-on-error: Actions is asked for nowhere
  // else in finalize, and an installation that never approved it must fail
  // this mint and skip only the score read - never the finalize token above,
  // whose failure would skip lockdown and preservation.
  ".github/workflows/daily-activity.yml#finalize.gradetoken": {
    app: "provisioning",
    repositories: "all",
    permissions: { actions: "read", checks: "read", contents: "read", metadata: "read" },
    reason: "It reads the student repositories named in the report; no list in the YAML can know their names.",
  },
  // Checkout, pull and the timeline push (Contents: write). deadline-sentinel.mjs
  // lists the org's repositories for pushed_at (Metadata) and re-reads the
  // control repo while it waits (Contents): the `assignments/` and
  // `repositories/<id>/` listings and the blobs they name, which is how it
  // finds the assignments and the student repositories that reached its instant
  // after it was armed. Stop writes runs lockdown.mjs with STOP_ONLY, which
  ...
  // The same work on a fresh credential. A token lives for an hour and the
  // watch waits for up to 4h45m, so the last stretch - the polling that matters,
  // the stop, the timeline push - runs on this one. Identical scope: a narrower
  // row here would 403 at the deadline and nowhere else.
  ".github/workflows/deadline-sentinel.yml#watch.token_late": {
    app: "provisioning",
    repositories: "all",
    permissions: { administration: "write", contents: "write", metadata: "read", organization_administration: "write" },
    reason: "It watches and locks the student repositories named in the records; no list in the YAML can know their names.",

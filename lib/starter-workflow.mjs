...
import { GRADE_RUN_NAME, gateAllowingDispatch, gradeCheckoutStep, gradeDispatchTrigger } from "./grade-dispatch.mjs";
...
export function buildStarterWorkflow({ handInMessage = null } = {}) {
  ...
  const job = {
    ...
    // A lecturer's "grade this commit now" (lib/grade-dispatch.mjs) has no
    // head_commit, so the hand-in gate must let a dispatch through.
    ...(gate ? { if: gateAllowingDispatch(gateExpression(gate)) } : {}),
    steps: [
      ...
      // GITHUB_TOKEN. Checks out the commit a lecturer named on a dispatch.
      gradeCheckoutStep(),
      {
        name: "example",
        id: "example",
  ...
  const doc = new Document({
    ...
    on: { push: { branches: ["main"] }, ...gradeDispatchTrigger() },

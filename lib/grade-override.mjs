 ...
 *                | { kind: "commit", sha: string, by: string, at: string, reason: string, runId: number|null }}
 */
export function gradeDecisionFor(login, { overrides = null, team = null } = {}) {
  ...
  if (commit) {
    return {
      kind: "commit", sha: commit.value, by: commit.overridden_by, at: commit.overridden_at, reason: commit.reason,
      // The run a lecturer started for it, when "Grade this commit now" graded
      // it (lib/grade-dispatch.mjs); null reads the commit's own run.
      runId: Number.isInteger(commit.run_id) ? commit.run_id : null,
    };
  }
  return null;
}
...
export function decisionEntry({ type, value, reason, by, at = new Date().toISOString(), runId = null }) {
  return {
    type, value, reason: String(reason ?? "").trim(), overridden_by: by, overridden_at: at,
    ...(type === CHOSEN_COMMIT && value !== null && Number.isInteger(runId) ? { run_id: runId } : {}),
  };
}

 ...
 */
/**
 * The archive's own README, written once when the repository is created.
 *
 * Every preservation is a branch, so the default branch holds this file and
 * nothing else - and the repository root, which is where a link lands and where
 * anyone browsing starts, therefore shows one small file and reads as an EMPTY
 * REPOSITORY. A lecturer opening the archive for a finished exam concluded
 * exactly that (2026-09-02): that the evidence was gone. GitHub's `auto_init`
 * README is just the repository name, which does nothing to correct it.
 *
 * It deliberately does NOT document the branch naming scheme. Nobody types one
 * of these by hand - `archiveBranchUrl` links them - and a spelled-out pattern
 * in prose is a second source of truth that drifts from `archiveBranchName`.
 * What a reader needs is that the work is here and which tab lists it.
 *
 * The archive is private and no student is ever a collaborator on it, so this
 * is staff-facing text.
 *
 * @param {{assignmentId?: string}} args
 * @returns {string|null} markdown, or null when there is no assignment to name
 */
export function archiveReadme({ assignmentId } = {}) {
  const id = typeof assignmentId === "string" ? assignmentId.trim() : "";
  if (!id) return null;
  return `# Archived submissions - ${id}

This repository holds the preserved student submissions for **${id}**, copied
here as they stood at the deadline. It is private, and the students whose work
is in it have no access to it.

## The submissions are branches, not files here

You are looking at the default branch, which contains only this file. That is
why the repository can look empty at first glance - it is not. Every preserved
submission is stored as its own branch.

**Open the Branches tab** to list them, then pick one to browse that submission
exactly as it was preserved. With a branch selected, the Code tab's Download ZIP
gives you that submission as a file.

## Deleting it

This archive belongs to the assignment named above. When you retire the course
year, delete it together with that assignment's student repositories.
`;
}
 ...
 * @param {Array<{preservation_status?: string, archive_repo?: string}>} [input.students]
 * @returns {string}
 */

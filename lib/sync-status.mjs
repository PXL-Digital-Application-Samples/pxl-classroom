...
export function describeFollow({ run, record } = {}) {
  ...
  if (record) {
    ...
    // "Last sync" is the status line's word, for a sync that happened before
    // the page was opened. In the dialog it is the sync you just watched.
    if (view.state === "completed" || view.state === "template-changed") {
      const total = Number.isInteger(record.total_students) ? record.total_students : results.length;
      view.title = `The sync of ${view.commit} finished: all ${total} student${total === 1 ? "" : "s"} handled`;
    }
    return {
      ...view,

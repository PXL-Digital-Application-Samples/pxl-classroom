 ...
 * them and tick them.
 *
 * @param {import("./types.mjs").RosterStudent|null|undefined} student
...
/**
 * The group chips a filter row should actually render.
 *
 * `classGroupCounts` answers "how many in each", including a zero for the
 * ungrouped, and deliberately leaves the display decision to the caller. This
 * is that decision, in one place because two surfaces make it: the cohort
 * picker on the assignment form and the Roster tab's filter row. It was written
 * out in the picker and would have been copied into the second one.
 *
 * Two rules, and both are about not offering a control that decides nothing:
 *
 *  - NO GROUPS AT ALL MEANS NO CHIPS. An organization that has never used them
 *    would otherwise get a lone "No group · 40" beside "All 40" - two controls
 *    filtering to the same set, which is worse than neither. It also means a
 *    roster with no groups yet is filtered by "All", which is the correct way
 *    to select everybody for the FIRST group.
 *  - THE UNGROUPED CHIP ONLY WHEN SOMEBODY IS IN IT, for the same reason: a
 *    "No group · 0" is a filter onto an empty table.
 *
 * @param {{students?: unknown}|Array|null|undefined} roster
 * @returns {Array<{group: string, count: number}>} named groups, ungrouped last
 */
export function classGroupChips(roster) {
  const counts = classGroupCounts(roster);

...
async function checkForExposedInvitations({ req, addCheck, check, org, brokerName, TIER, migrated = false }) {
  ...
  // §4.3.2, INSTALL.md §3.1 and the acceptance-handler warning; it survived
  // here.
  //
  ...
  addCheck(
    ...
    migrated
      ...
      : check(
          ...
          "Invitation Exposure",
          "fail",
          `${exposed.length} issue(s) on the PUBLIC repository ${org}/${brokerName} still carry this assignment's invitation token in their title (${numbers}${more}). Anyone can read it there, so the link is effectively public. Delete those issues, then regenerate the invitation so the exposed one stops working. See INSTALL.md §3.1.`
        )
  );
}
    ...
    if (proxy && proxy.configured === false) {
      addCheck(
        ...
        check(
          ...
          "Sign-in Proxy",
          "fail",
          `No usable device-flow proxy is configured, so nobody can sign in. Set device_flow_proxy in deployment.yml (see INSTALL.md §9), then redeploy - the value is baked in at build time.`
        )
      );
      ...
      addCheck(
        ...
        check(
          ...
          "Sign-in Proxy",
          "warn",
          `The primary sign-in proxy did not answer${proxy.detail ? ` (${proxy.detail})` : ""}. Sign-in falls back to the secondary, so it probably still works - but the fallback is a third party that sees the token in transit, so this is worth fixing rather than living with. See INSTALL.md §9.`
        )
      );
    } else if (proxy) {
      addCheck(

...
import { parseToken, invitePath, linkSecretFrom } from "./invite-token-format.mjs";
...
import { isAcceptanceIssueTitle } from "./broker-issue-titles.mjs";
...
async function checkForExposedInvitations({ req, addCheck, check, org, brokerName, TIER, migrated = false }) {
  ...
  // "NOT ONE OF OURS", not "not currently titled like an acceptance". This read
  // `!i.title.startsWith("pxl-accept:")`, and the broker redacts every handled
  // acceptance to "Acceptance (processed)" and deliberately leaves it OPEN -
  // closing it would email the student who authored it. So every successful
  // acceptance matched: measured 2026-09-22 on PXL-2TIW-DevOps-2627/
  // broker-groepsindeling, 11 of 11 issues were reported as stray on a broker
  // with nothing wrong with it. A warning that is always wrong is how a health
  // panel stops being read, which is the thing the comment above worries about.
  const stray = issues.filter((i) => i?.state === "open" && !isAcceptanceIssueTitle(i?.title));
  addCheck(
    4,
    stray.length === 0
      ? check(
...
async function checkOrgAppTier({ req, addCheck, org, assignmentId, formDoc, hubOwner, hubRepo, fetchPages, probeProxy }) {
  ...
  // Check Hub Registry
  if (hubOwner && hubRepo) {
    const orgsRes = await req("GET", registryContentsPath(hubOwner, hubRepo));
    const registration = lookupRegisteredOrg(orgsRes, org);
    const label = "Course Organization Enrollment (participating-orgs.yml)";
    // Only a 404 is "not found". Every failed read used to say so, which told a
    // lecturer the hub had no registry when it had merely answered 500.
    if (registration.reason === "no-registry") {
      addCheck(1, check("hub-registry", "tier-1-org", label, "warn",
        "participating-orgs.yml not found on the central hub repository."));
    } else if (registration.reason === "http") {
      addCheck(1, check("hub-registry", "tier-1-org", label, "warn",
        `Could not read participating-orgs.yml from the central hub repository (HTTP ${registration.status ?? "unknown"}).`));
    } else if (registration.reason === "parse") {
      addCheck(1, check("hub-registry", "tier-1-org", label, "warn",
        `Could not parse participating-orgs.yml: ${registration.error}`));
    } else if (registration.state === "listed") {
      addCheck(1, check("hub-registry", "tier-1-org", label, "ok",
        `Registered in hub (Budget owner: @${registration.entry.budget_owner_login || "unknown"}).`));
    } else {
      addCheck(1, check("hub-registry", "tier-1-org", label, "fail",
  ...
  // all-clear. Same rule the App-declaration checks above follow.
  {
    ...
    // THE SAME RESPONSE, one more field. `plan.name` sits beside
    // `default_repository_permission` and needs the same access to read, so a
    // second call would buy nothing - and on an organization that never
    // upgraded, this is the check that says so BEFORE a deadline turns the
    // freeze into a blunt instrument. Confirmed live 2026-09-07: four of the
    // twelve readable participating organizations answer "free".
    const planFinding = orgRes.ok ? organizationPlanFinding(orgRes.data?.plan?.name, { org }) : null;
    if (planFinding) {
      addCheck(
        1,
        check(
          "org-plan",
          "tier-1-org",
          "Organization Plan (rulesets and protected branches)",
          planFinding.severity,
          planFinding.message,
          planFinding.severity === "ok"
            ? null
            : { plan: planFinding.plan, upgrade_url: "https://github.com/settings/education/benefits" }
        )
      );
    }
  }
...
async function checkPagesTier({ req, addCheck, org, assignmentId, formDoc, hubOwner, hubRepo, fetchPages, probeProxy }, { doc, isPublished, brokerName }) {
  ...
  if (assignmentId && isPublished) {
    ...
    if (linkSecret) {
      ...
      const cardPath = invitePath(digestBytes);

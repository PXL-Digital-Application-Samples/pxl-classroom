...
export async function runDiagnostics({
  ...
  if (appRes.ok) {
    ...
    const undeclared = Object.entries(MANIFEST_APP_PERMISSIONS).filter(
      ([perm, expected]) => !permissionMeetsRequirement(declaredPerms[perm], expected)
    );
    if (undeclared.length === 0) {
      ...
      const labels = undeclared
        .map(([perm, expected]) => `${perm}=${declaredPerms[perm] ?? "missing"} (want ${expected})`)
        .join(", ");
      addCheck(
        1,
        check(
          ...
          { undeclared: undeclared.map(([permission, expected]) => ({ permission, expected, actual: declaredPerms[permission] ?? null })) },
          {
            type: "link",

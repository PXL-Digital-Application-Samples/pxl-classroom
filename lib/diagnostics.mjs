...
import { parseToken, inviteFileName } from "./invite-token-format.mjs";
import {
  APP_INSTALL_URL,
  APP_SLUG,
  CONTROL_REPO,
...
export async function runInvitationChecks({ req, addCheck, check, doc, org, brokerName, assignmentId, fromEditor = false }) {
  ...
  const token = typeof doc?.invite_token === "string" ? doc.invite_token : "";

  // Before anything else, and independent of what the assignment currently
  // holds: a leftover acceptance issue is a published credential whether the
  // token in the YAML is present, absent or malformed. The one case that stays
  // silent is an unsaved editor form with no invitation, which is not yet a
  // fact about the world.
  if (token || !fromEditor) {
    await checkForExposedInvitations({ req, addCheck, check, org, brokerName, TIER });
  }
  ...
  // that is not published there is rejected as unknown-key, every time.
...
async function checkForExposedInvitations({ req, addCheck, check, org, brokerName, TIER }) {
  const issues = [];
  let truncated = false;
  for (let page = 1; ; page++) {
    const res = await req(
  ...
  addCheck(
    ...
    check(
      ...
      "Invitation Exposure",
      "fail",
      `${exposed.length} issue(s) on the PUBLIC repository ${org}/${brokerName} still carry this assignment's invitation token in their title (${numbers}${exposed.length > 5 ? ", …" : ""}). Anyone can read it there, so the link is effectively public. Delete those issues, then regenerate the invitation so the exposed one stops working. See RUNBOOK §1.3.2.`
    )
  );
}
...
export async function runDiagnostics({
  ...
  if (appRes.ok) {
    ...
    if (undeclared.length === 0) {
      ...
      addCheck(
        ...
        check(
          ...
          "GitHub App Declaration",
          "fail",
          `The App itself does not declare: ${labels}. No organization can approve a permission the App does not declare - the App owner adds it under Permissions & events, then every org owner approves the update (RUNBOOK.md section 10.6).`,
          { undeclared },
          {
            type: "link",
    ...
    // had no inbound link from anywhere in the app (UX_PLAN §8 / UX18).
    //
    ...
    addCheck(
      ...
      check(
        ...
        "GitHub App Declaration",
        "fail",
        `No GitHub App named \`${APP_SLUG}\` exists. Nothing can be provisioned until one is created from the App Manifest form and its credentials are stored as hub secrets (RUNBOOK.md section 1).`,
  ...
  if (doc) {
    ...
    // only fired on an explicit "enforced"; absent means enforced too (§15).
    if (doc.roster_mode !== "open" && assignmentId) {
      const rosterRes = await req("GET", `/repos/${org}/${CONTROL_REPO}/contents/students/roster.yml`);
      if (rosterRes.status === 404) {
        addCheck(
          ...
          check(
            "roster-check",
            "tier-3-assignment",
            "Enforced Roster File (students/roster.yml)",
            "warn",
            `roster_mode is enforced, but students/roster.yml was not found. Every acceptance will be rejected as rejected:no-roster until a roster is imported.`,
            null,
        ...
        const label = "Enforced Roster (students/roster.yml)";
        ...
        if (parsed === undefined) {
          addCheck(3, check("roster-check", "tier-3-assignment", label, "warn",
            "students/roster.yml exists but could not be parsed as YAML, so it is not possible to say who may accept. Re-import the roster."));
        } else if (!Array.isArray(parsed?.students)) {
          addCheck(3, check("roster-check", "tier-3-assignment", label, sev,
            "students/roster.yml has no `students:` list. Acceptance reads that key and nothing else, so every acceptance is rejected as not-on-roster. Re-import the roster from the Roster tab.",
            null, fix));
        } else {
          const students = parsed.students;
          const linked = students.filter((s) => typeof s?.github_login === "string" && s.github_login.trim()).length;
          if (students.length === 0) {
            addCheck(3, check("roster-check", "tier-3-assignment", label, sev,
              "students/roster.yml is empty, so nobody can accept this assignment - every attempt is rejected as not-on-roster. Import the roster, or set this assignment to open enrolment.",
              { students: 0, linked: 0 }, fix));
          } else if (linked === 0) {
            addCheck(3, check("roster-check", "tier-3-assignment", label, sev,
              `${students.length} student(s) are on the roster but none has a github_login, which is the only field acceptance matches on - so nobody can accept. Add the GitHub usernames column and re-import.`,
              { students: students.length, linked: 0 }, fix));
          } else if (linked < students.length) {
            addCheck(3, check("roster-check", "tier-3-assignment", label, "warn",
              `${linked} of ${students.length} students on the roster have a github_login. The other ${students.length - linked} cannot accept until theirs is added.`,
              { students: students.length, linked }, fix));
          } else {
            addCheck(3, check("roster-check", "tier-3-assignment", label, "ok",
              `${students.length} student(s) on the roster, all with a GitHub username.`,
  ...
  if (assignmentId && isPublished) {
    ...
    if (typeof doc?.invite_token === "string" && doc.invite_token) {
      const digestBytes = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(doc.invite_token));

...
async function checkForExposedInvitations({ req, addCheck, check, org, brokerName, TIER, migrated = false }) {
  ...
  // §4.3.2, RUNBOOK §1.3.1 and the acceptance-handler warning; it survived
  // here.
  //
    ...
    if (proxy && proxy.configured === false) {
      addCheck(
        ...
        check(
          ...
          "Sign-in Proxy",
          "fail",
          `No usable device-flow proxy is configured, so nobody can sign in. Set device_flow_proxy in deployment.yml (see RUNBOOK §1.9), then redeploy - the value is baked in at build time.`
        )
      );
      ...
      addCheck(
        ...
        check(
          ...
          "Sign-in Proxy",
          "warn",
          `The primary sign-in proxy did not answer${proxy.detail ? ` (${proxy.detail})` : ""}. Sign-in falls back to the secondary, so it probably still works - but the fallback is a third party that sees the token in transit, so this is worth fixing rather than living with. See RUNBOOK §1.9.`
        )
      );
    } else if (proxy) {
      addCheck(

  ...
  addCheck(
    ...
    migrated
      ...
      : check(
          ...
          "Invitation Exposure",
          "fail",
          `${exposed.length} issue(s) on the PUBLIC repository ${org}/${brokerName} still carry this assignment's invitation token in their title (${numbers}${more}). Anyone can read it there, so the link is effectively public. Delete those issues, then regenerate the invitation so the exposed one stops working. See RUNBOOK §3.12.`
        )
  );
}

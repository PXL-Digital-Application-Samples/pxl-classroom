...
async function checkForExposedInvitations({ req, addCheck, check, org, brokerName, TIER, migrated = false }) {
  ...
  // that is worth saying, because a broker that cannot delete its issues is
  // usually an App without `administration: write`.
  addCheck(
    4,
    migrated
      ? check(
          ...
          "Invitation Exposure",
          "warn",
          `${exposed.length} acceptance issue(s) on ${org}/${brokerName} were never cleaned up (${numbers}${more}). Since this assignment uses signed acceptance the titles are signatures, not links, so nothing is exposed - but the cleanup failing usually means the App lacks Administration: write on this organization. Do NOT regenerate the invitation: it would retire every student's link and fix nothing. See RUNBOOK §1.3.1.`
        )
      : check(
          "invite-exposure",
          TIER,
          "Invitation Exposure",
          "fail",
          `${exposed.length} issue(s) on the PUBLIC repository ${org}/${brokerName} still carry this assignment's invitation token in their title (${numbers}${more}). Anyone can read it there, so the link is effectively public. Delete those issues, then regenerate the invitation so the exposed one stops working. See RUNBOOK §1.3.1.`
        )
  );
}

 ...
 * `{ pattern, example }` as written; lib/claim.mjs compiles and applies it,
 * and an assignment may switch it off with `claim_address_format: false`.
 */
...
// Checked here, where a mistake fails the hub, the tests and CI at once. The
// SPA's reader cannot throw at module scope (a blank page), so a bad pattern
// must never get past this one.
if (CLAIM_ADDRESS_FORMAT) {
  if (!CLAIM_ADDRESS_FORMAT.pattern || !CLAIM_ADDRESS_FORMAT.example) {
    throw new Error('deployment.yml "claim_address_format" needs both "pattern" and "example"');
  }
  try {
    new RegExp(CLAIM_ADDRESS_FORMAT.pattern);
  } catch (e) {
    throw new Error(`deployment.yml "claim_address_format.pattern" is not a valid regular expression: ${e.message}`, { cause: e });
  }
}

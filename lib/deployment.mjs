 ...
 *
 * Passed through as written rather than parsed here: `lib/academic-year.mjs`
 * owns what a start date means and answers `null` for one it cannot use, so a
 * typo produces no year in the collision refusal rather than a confident wrong
 * one. Not defaulted for the same reason - a silent fallback to 1 September
 * would be a boundary nobody configured, and the refusal reads perfectly well
 * with no year at all.
 */

...
// ONE email normaliser, in its own module. Importing it from claim.mjs would
// put 470 lines of ECDH and AES-GCM on the graph of
// scripts/scaffold-control-repo.mjs, which setup-org.yml runs with no `npm ci`.
 ...
 *
 * @param {{student_number?: unknown, github_login?: unknown, email?: unknown}|null|undefined} entry
...
export function rosterIdentities(entry) {
  ...
  // THIRD, AND LAST, so nothing that already had an identity changes the one it
  // records. It exists because `student_number` was REQUIRED of every imported
  // row purely so the row would have a key - and most institutions hand a
  // lecturer addresses, not SIS numbers. Without it a student added by name and
  // address had no identity at all, so the import diff, the cell editor, Edit
  // details and Remove could none of them find the row again.
  //
  // Normalised, because an address typed `A.Nother@Student.PXL.be` and one
  // dispatched lowercase are one person - the same rule the login above obeys.
...
export function normalizeCohortEntry(value) {
  ...
  // `email:` EXISTS HERE BECAUSE `rosterIdentities` CAN MINT IT. It was added
  // there and not here, and the two halves disagreeing is the whole failure:
  // the picker stored `email:...` for a row that had only an address, this
  // dropped it on the way back in, and so the student the lecturer ticked was
  // refused by the gate with nothing on any screen saying so. Worse when every
  // picked student was email-keyed - the set came back EMPTY, and an empty
  // cohort means EVERY STUDENT ON THE ROSTER, so a narrowed assignment opened
  // itself to the whole course. Fail-open, from a spelling this file did not
  // know about.
  //
  // Normalised the same way `rosterIdentities` normalises it, or a cohort
  // written from one spelling would not match the row it was chosen from.
  if (trimmed.startsWith("email:")) {
    const email = normalizeEmail(trimmed.slice("email:".length));
 ...
 * @returns {import("./types.mjs").RosterStudent[]}
 */
 ...
 * @param {unknown} login
 * @param {{students?: unknown}|Array|null|undefined} roster
 * @param {Map<string, import("./types.mjs").RosterStudent>|null} [extraLogins] lowercased login -> roster entry
 * @returns {import("./types.mjs").RosterStudent|null}
 */

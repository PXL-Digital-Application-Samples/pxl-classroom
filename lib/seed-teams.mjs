...
import { assignmentAdmitsStudent, cohortStudents, rosterEntryForLogin } from "./cohort.mjs";
import { teamPath } from "./control-layout.mjs";
import { DEFAULT_MAX_TEAM_SIZE, maxTeamSize as teamMax, minTeamSize as teamMin } from "./group-config.mjs";

// `SEED_SOURCES = ["assignment", "roster"]` was exported here and read nowhere.
// The `source` parameter of planSeed() is the actual contract, and it is
// compared against the literal "roster" inside this file - so the constant
// documented a rule it did not enforce. Removed rather than wired up: a name
// nobody imports cannot be the source of truth for anything.

const SLUG_RE = /^[a-z0-9][a-z0-9-]{0,63}$/;

// Re-exported from lib/group-config.mjs, which the acceptance gate and the
// student page read too - this used to be declared here and read only here.
export { DEFAULT_MAX_TEAM_SIZE };
...
/**
 * Group roster entries into pseudo-teams using the roster's `team_slug` column.
 *
 * IT USED TO PREFER `s.teams?.[assignmentId]`, A FIELD THAT CANNOT EXIST.
 * `students[].teams` was a per-assignment map with a reader and no writer, and
 * `acceptance/accept.mjs` deleted its copy of this read with a paragraph
 * explaining why - the roster schema has never declared it and its rows are
 * `additionalProperties: false`, so no document this system writes or accepts
 * can carry one. This copy survived that sweep, and with it the `assignmentId`
 * argument, whose only use was that lookup. The type checker found it on
 * 2026-09-17; the test that covered it was passing a row the schema forbids.
 *
 * `team_slug` carries no assignment - one slug per student, applied to every
 * group assignment in the organization - which is why it is a SEED a lecturer
 * reviews before anything is written, and never a rule.
 *
 * @param {import("./types.mjs").RosterStudent[]|null} rosterStudents
 */
export function teamsFromRoster(rosterStudents) {
  const bySlug = new Map();
  for (const s of rosterStudents || []) {
    if (!s || s.active === false) continue;
    const login = s.github_login;
    if (!login) continue;
    const slug = s.team_slug;
    if (!slug) continue;
 ...
 *
 * @param {object} [opts]
 * @param {import("./types.mjs").Team[]} [opts.sourceTeams]
 * @param {import("./types.mjs").Team[]} [opts.existingTeams]
 * @param {import("./types.mjs").Assignment|null} [opts.targetAssignment] the assignment
 *   being seeded INTO
 * @param {import("./types.mjs").Assignment|null} [opts.sourceAssignment]
 * @param {import("./types.mjs").Roster|null} [opts.roster]
 * @param {Map<string, import("./types.mjs").RosterStudent>|null} [opts.extraLogins]
...
export function planSeed({
  ...
  roster = null,
  // Lowercased login -> roster entry, for students whose login is not in the
  // roster's own column yet. Under `claim` that is most of them until the
  // nightly folds their claim in; without it, seeding warns that properly
  // enrolled students are about to be rejected. Optional so a caller with no
  // claims to hand still plans, just less precisely.
  extraLogins = null,
  ...
  const maxTeamSize = teamMax(cfg);
  const minTeamSize = teamMin(cfg);
  ...
  // A carried-over student who will be refused, and WHICH refusal it is.
  //
  // Only predicts anything where the ROSTER is the gate; under `open` it
  // predicts nothing, and telling a lecturer their students will be rejected
  // when they will not is worse than silence.
  //
  // TWO DIFFERENT ANSWERS, because they send a lecturer to do different things:
  // not on the roster at all means import them, and on the roster but outside
  // this assignment's cohort means add them to it. The second case did not
  // exist while a cohort was a rule over class groups - it does now, and the
  // roster-only check was blind to exactly it.
  //
  // Resolution, not string comparison: under `claim` a student's login lives in
  // their claim record until the nightly folds it into the roster, so a check
  // reading only the roster column reported a properly enrolled student as
  // about to be rejected. `extraLogins` carries the bindings in.
  if (roster && rosterGatesAcceptance(targetAssignment.roster_mode)) {
    const offRoster = [];
    const offCohort = [];
    for (const l of seededLogins) {
      const entry = rosterEntryForLogin(l, roster, extraLogins);
      if (!entry) offRoster.push(l);
      else if (!assignmentAdmitsStudent(targetAssignment, entry)) offCohort.push(l);
    }
    if (offRoster.length > 0) {
      warnings.push({
        code: "not-on-roster",
        message:
          `${offRoster.length} carried-over student(s) are not on the roster and will be rejected when they accept: ` +
          offRoster.map((l) => `@${l}`).join(", ") + ".",
        logins: offRoster,
      });
    }
    if (offCohort.length > 0) {
      warnings.push({
        code: "not-in-cohort",
        message:
          `${offCohort.length} carried-over student(s) are on the roster but not in this assignment and ` +
          `will be rejected when they accept: ` + offCohort.map((l) => `@${l}`).join(", ") +
          ". Add them under “Who is this assignment for”.",
        logins: offCohort,
      });
    }
  }
  ...
  // students whose whole group left, the ones nobody paired with.
  //
  // COUNTED AGAINST THE COHORT, not the organization. It used to be roster
  // relative, and this file said so: "it means nothing unless the roster IS the
  // cohort - which is `enforced` only". That was true and it was still wrong -
  // seeding a 22-person cohort in a 200-student org reported 178 unplaced
  // students, none of whom the assignment was for. An explicit cohort makes it
  // correct for the first time, under every gated mode.
  let unplaced = [];
  if (roster && rosterGatesAcceptance(targetAssignment.roster_mode)) {
    unplaced = cohortStudents(targetAssignment, roster)
  ...
  const changes = teams.map((t) => ({
    path: teamPath(targetId, t.team_slug),
 ...
 *
 * @param {object} [opts]
 * @param {import("./types.mjs").Team[]} [opts.teams]
 * @param {Set<string>|string[]} [opts.acceptedLogins]
...
export function planUnseed({ teams = [], acceptedLogins = [], assignmentId } = {}) {
  ...
  return {
    ...
    changes: removable.map((t) => ({
      path: teamPath(assignmentId, t.team_slug),
      content: null,
    })),
  };
}

...
export function planSeed({
  ...
  // "not on the roster" only predicts a rejection where the ROSTER is the gate.
  // Under `open` it predicts nothing, and telling a lecturer
  // their students will be rejected when they will not is worse than silence.
  ...
  // relative, so it means nothing unless the roster IS the cohort - which is
  // `enforced` only. Under `open` the roster may be empty or
  // stale and the list would be fiction.
  let unplaced = [];
  if (roster && rosterGatesAcceptance(targetAssignment.roster_mode)) {

...
export function planSeed({
  ...
  if (!targetAssignment?.id) {
    return {
      ...
      warnings, teams: [], changes: [], skipped, stats: emptyStats(),
    };
  }
  ...
  if (errors.length > 0) {
    return { ok: false, errors, warnings, teams: [], changes: [], skipped, stats: emptyStats() };
  }
  ...
  if (keptExisting.length > 0) {
    warnings.push({
      ...
      message: `${keptExisting.length} team(s) already exist with members and were left untouched: ${keptExisting.join(", ")}.`,
      teams: keptExisting,
    });
  }
...
function emptyStats() {
  return { teams: 0, students: 0, skipped: 0, source_teams: 0 };
}

 ...
 *
 * The parameter is a SHAPE with every field optional, not `unknown`: anything
 * at all may be passed - that is the point, the document is read off disk - and
 * every field is then tested before it is used. `unknown` said the same thing
 * and made reading `doc.state` an error rather than the check it is.
 *
 * @param {{state?: unknown, deadline_at?: unknown}|null|undefined} doc a parsed assignment document
 * @returns {string|null}
 */
export function sentinelInstant(doc) {
  if (doc?.state !== 'published' && doc?.state !== 'closed') return null;
  // Cast, not a `typeof` gate: the schema says this field is a date-time string
  // and `new Date()` below is what decides, so tightening it here would be a
  // behaviour change smuggled in under a type annotation. Whatever arrives that
  // is not a date lands on `Invalid Date` and returns null, as it always has.
  const deadline = /** @type {string} */ (doc?.deadline_at);
...
/**
 * When the deadline sentinel STOPPED WRITES for this assignment, from its
 * timelines (`lockdowns/<id>/sentinel-*.json`) - the earliest that fired, or
 * null. One judge for lockdown.mjs (which credits the stop) and the Admin
 * Panel's permission plan (which must not unlock it): the sentinel stops
 * writes at the instant and writes no lock record, so between it and the
 * nightly's finalize, this is the only record that the cohort is stopped.
 *
 * `fired` is about the sentinel, `due` about THIS assignment: a sentinel fires
 * for its group while a member extended past the instant records `due: false`.
 * Absent `due` is a timeline written before the field, when firing meant stopped.
 *
 * @param {Array<{outcome?: string, due?: boolean, deadline_at?: string}|null>} timelines
 * @returns {string|null}
 */
export function sentinelStoppedAt(timelines) {
  let earliest = null;
  for (const doc of timelines || []) {
    if (doc?.outcome !== "fired" || !doc?.deadline_at || doc?.due === false) continue;

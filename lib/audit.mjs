...
import { CONTROL_REPO, APP_SLUG, HUB_REPO } from "#deployment";
...
export const MANIFEST_APP_PERMISSIONS = Object.freeze({
  ...
  // READ, and it is genuinely required - `unfreezableAcceptorsFinding` lists
  // `GET /orgs/{org}/members?role=admin` to find acceptors who are org OWNERS,
  // whom lockdown cannot demote because GitHub grants owners admin on every
  // repository. That check exists because the case was found sitting in a real
  // exam cohort four days before its deadline.
  //
  // The 2026-08-31 review first called this permission dead, on the strength of
  // `roster_mode: org_member` having been deleted - and was wrong: that removed
  // the ENROLMENT use, not the diagnostic one. Dropping it would have silently
  // turned a Tier 3 check into "no owner list readable", which by its own rule
  // yields no check at all rather than a red one. The live App holds `write`,
  // which is the part that is excess: listing members needs only read.
  members: "read",
});
...
// for starring, which EXPECTED_APP_PERMISSIONS verifies separately during setup
// precisely because an installation cannot carry it.

...
async function checkInstallation(request, org) {
  let res = await request("GET", `/orgs/${org}/installation`);
  if (res.status === 401 || res.status === 403) {
    // If user OAuth token is used, GET /installation returns 401/403.
    // Fall back to listing all installations on the org.
    const listRes = await request("GET", `/orgs/${org}/installations`);
    if (listRes.ok && Array.isArray(listRes.data)) {
      const inst = listRes.data.find(
        (i) => i.app_slug === "pxl-classroom-provisioner" || i.client_id === "Iv23li0H0Je93H2FkMPW"
      );
      if (inst) {
        return {

...
async function checkAppDeclaration(request) {
  ...
  const missing = [];
  for (const [perm, expected] of Object.entries(MANIFEST_APP_PERMISSIONS)) {
    if (!permissionMeetsRequirement(declared[perm], expected)) {
      missing.push({ permission: perm, expected, actual: declared[perm] ?? null });
    }
  }
  if (missing.length === 0) {

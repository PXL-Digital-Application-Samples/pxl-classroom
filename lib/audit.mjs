...
async function checkParticipatingOrgs(request, org, hubOwner, hubRepo) {
  ...
  const res = await request("GET", `/repos/${hubOwner}/${hubRepo}/contents/participating-orgs.json?ref=participating-orgs`);
  if (res.status === 404) {
    return check("participating-orgs", "Org listed on participating-orgs branch", "warn", "participating-orgs.json not found on the hub's participating-orgs branch.");
  }
  if (!res.ok) {
    return check("participating-orgs", "Org listed on participating-orgs branch", "warn", `Could not read participating-orgs.json (HTTP ${res.status}).`);
  }
  let parsed;
  try {
    ...
    return check("participating-orgs", "Org listed on participating-orgs branch", "warn", `participating-orgs.json could not be parsed: ${e.message}.`);
  }
  const entry = (parsed.orgs || []).find((o) => o.login?.toLowerCase() === org.toLowerCase());
  if (!entry) {
    return check("participating-orgs", "Org listed on participating-orgs branch", "fail", `${org} is missing from participating-orgs.json. Weekly usage report will not include this org.`);
  }

 ...
 *
 * The viewer's own account is a WARN, not a FAIL. A lecturer accepting their
 * own assignment to test it is the ordinary case, and a permanent red beside
 * the real findings is how a check stops being read.
 */
export function unfreezableAcceptorsFinding({
  ...
  if (others.length) {
    return {
      ...
      self,
      message:
        `${others.length} accepted ${others.length === 1 ? "student is an owner" : "students are owners"} ` +
        `of ${org} and cannot be frozen at the deadline: ${others.join(", ")}. ${why} ` +
        `Every other student still freezes normally. Fix by removing them from the cohort, or by ` +
        `changing their organization role from Owner to Member under People -> Role before the deadline.`,
    };
  }

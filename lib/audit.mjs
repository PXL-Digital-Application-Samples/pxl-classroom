...
import { lookupRegisteredOrg, registryContentsPath } from "./org-registry.mjs";
...
/** The plan name GitHub returns for an organization that has not been upgraded. */
export const FREE_PLAN = "free";

/**
 * What an un-upgraded organization costs, said before a deadline rather than at one.
 *
 * NOTHING HERE IS BROKEN ON FREE, which is why this is a warning and not a
 * failure. Every endpoint this system calls works: repositories from a
 * template, collaborator grants, invitations, contents, git data, issues,
 * pull requests, check runs. Group assignments do not touch the org Teams
 * feature at all - membership is `teams/<id>/<slug>.json` and repositories are
 * shared by adding collaborators - and acceptance costs no Actions minutes,
 * because brokers are public repositories and public repositories have
 * unlimited minutes on every plan.
 *
 * TWO THINGS DEGRADE, both silently, and both only at the deadline:
 *
 *  1. Rulesets are a Team-and-above feature and are not available on private
 *     repositories on Free. Student repositories are private, so the ruleset
 *     never applies and `lockdown.mjs` falls back to demoting the student to
 *     `pull` - which is correct, and is recorded as `lock_method: "demotion"`
 *     so the unlock applies the matching inverse. But demotion does not just
 *     remove push: it removes Actions, secrets, environments, runners and
 *     settings, which on a course whose SUBJECT is those things confiscates
 *     the subject matter. The ruleset exists precisely to avoid that.
 *  2. Protected branches are Team-and-above on private repositories too, so
 *     the Feedback PR baseline branch is created and left unprotected. A
 *     student is admin on their own repository and can force-push or delete
 *     it, which corrupts the diff the Feedback PR is built from.
 *
 * The lecturer finds out at the deadline, or does not find out at all, because
 * both degrade into a working-but-blunter path. Reading the plan costs nothing:
 * `GET /orgs/{org}` already carries it beside `default_repository_permission`,
 * at the same access level, so the caller reuses that one response.
 *
 * Absent means UNREADABLE, never "it is fine" - same rule as the sibling above.
 *
 * @param {unknown} value `plan.name` from GET /orgs/{org}
 * @returns {{severity: string, plan: string, message: string}|null}
 */
export function organizationPlanFinding(value, { org = "This organization" } = {}) {
  if (value === undefined || value === null || value === "") return null;
  ...
  if (plan.toLowerCase() !== FREE_PLAN) {
    return {
      ...
      plan,
      message:
        `${org} is on GitHub's "${plan}" plan, so repository rulesets and protected ` +
        `branches are available on the private student repositories.`,
    };
  }
  ...
  return {
    ...
    plan,
    message:
      `${org} is on GitHub's "free" plan, where rulesets and protected branches do not ` +
      `apply to private repositories - and student repositories are private. Nothing ` +
      `fails, but two things get blunter at the deadline. The freeze falls back to ` +
      `demoting each student to "pull", which also takes away Actions, secrets and ` +
      `environments until the repository is reopened, so a course about those things ` +
      `loses its subject matter for the duration. And the Feedback PR baseline branch ` +
      `is left unprotected, so a student can force-push or delete it. Actions minutes ` +
      `are 2,000 a month here rather than 3,000. GitHub Team is free for verified ` +
      `educators: apply at https://github.com/education/teachers, then upgrade this ` +
      `organization from the Global Campus dashboard. Nothing in the course needs ` +
      `changing afterwards.`,
  };
}

/**
 * THIS assignment, on THIS plan: will its deadline do what the form promised?
 *
 * `organizationPlanFinding` above says what a free organization costs in
 * general, and says it in System Health - a page a lecturer may never open,
 * about an organization rather than about the thing they are publishing. This
 * one is the combination, asked at publish, where the freeze stops being a
 * setting and becomes a scheduled future event.
 *
 * ABSENT `lock_down_enabled` MEANS TRUE. The schema's own default, and the
 * rule the SPA follows (`a.lock_down_enabled ?? true`). Reading it with `!!`
 * would take every assignment that never mentioned the field - which is most
 * of them - and quietly report that nothing freezes, which is the opposite of
 * what happens. `tests/org-plan.test.mjs` derives the default from the schema
 * rather than restating it here.
 *
 * A WARNING, NEVER A REFUSAL. Nothing is broken on free: the freeze still
 * happens, by demotion, and is recorded as `lock_method: "demotion"` so the
 * unlock applies the matching inverse. Refusing to publish would block a
 * course that works.
 *
 * AND THE ANSWER GOES STALE THE MOMENT IT IS TAKEN. The lecturer can upgrade
 * to GitHub Team - it is free for verified educators - the day after
 * publishing, and then none of this applies. So this is computed from a LIVE
 * read of the plan each time it is asked, and no verdict is ever stored on the
 * assignment: a recorded "this one cannot freeze properly" would outlive the
 * fact by a year and be believed.
 *
 * @param {object} [input]
 * @param {unknown} [input.plan] `plan.name` from GET /orgs/{org}, read just now
 * @param {import("./types.mjs").Assignment} [input.assignment] the assignment document
 * @param {string} [input.org]
 * @returns {{severity: string, plan: string, message: string}|null}
 */
export function assignmentFreezePlanFinding({ plan, assignment, org = "this organization" } = {}) {
  // Unreadable is not evidence: with no plan we do not know, and inventing
  // "fine" here is exactly the silent degradation this exists to end.
  if (plan === undefined || plan === null || plan === "") return null;
  if (String(plan).toLowerCase() !== FREE_PLAN) return null;

  const freezes = assignment?.lock_down_enabled ?? true;
  const feedbackPr = assignment?.feedback_pr === true;
  // THE CASE THIS MISSED, AND IT IS THE WORST ONE. `late_policy: block` is the
  // configuration a lecturer picks precisely to stop late pushes WITHOUT taking
  // Actions, secrets and runners away - the form even unticks the demotion
  // checkbox for them when they choose it. On free that ruleset cannot be
  // created, and `applySubmissionLock` degrades to demotion rather than leaving
  // the cohort unlocked. So the setting chosen to avoid confiscating the
  // toolchain confiscates it, and this finding gated only on
  // `lock_down_enabled`, which is FALSE in exactly that configuration - so it
  // returned null and said nothing at all.
  const blocksLate = assignment?.late_policy === "block";
  if (!freezes && !feedbackPr && !blocksLate) return null;

  // Named separately because they degrade differently and a lecturer may have
  // asked for only one of them.
  const costs = [];
  if (blocksLate && !freezes) {
    costs.push(
      `"late work does not count" cannot be enforced with a ruleset here, so the deadline falls back ` +
        `to demoting every student to "pull" - which takes away the Actions, secrets and environments ` +
        `that choosing a branch lock instead of a demotion was meant to preserve`,
    );
  }
  if (freezes) {
    costs.push(
      `the deadline freeze will demote every student to "pull" instead of applying a ruleset, ` +
        `which also takes away Actions, secrets and environments until you reopen the repository`,
    );
  }
  if (feedbackPr) {
    costs.push(
      `the Feedback PR baseline branch cannot be protected, so a student can force-push or delete it`,
    );
  }
  ...
  return {
    ...
    plan: String(plan),
    message:
      `${org} is on GitHub's "free" plan, where rulesets and protected branches do not apply to ` +
      `private repositories - and student repositories are private. This assignment still works, but ` +
      `${costs.join(", and ")}. GitHub Team is free for verified educators ` +
      `(https://github.com/education/teachers); upgrading changes nothing in the course, and this ` +
      `assignment picks it up automatically.`,
  };
}
 ...
 *
 * @param {object} [input]
 * @param {Array<object>} [input.acceptors] the acceptance records
 * @param {string[]|null} [input.owners] organization owners, as logins
 * @param {boolean} [input.ownersComplete] false when the owner walk was capped
 * @param {string} [input.org]
 * @param {string|null} [input.viewerLogin]
 */
export function unfreezableAcceptorsFinding({
  ...
  // BOTH MECHANISMS, because this fires on "an owner is in the cohort" and does
  // not know which one the assignment uses - and the two fail differently.
  // Measured on a live drill, 2026-09-09: an owner IS blocked by an
  // organization ruleset (409 on push, and the record says verified: true, not
  // false), so the old wording described a mechanism that no longer applies to
  // the default. It also understated the risk, which is the part that matters:
  // deleting an organization ruleset is an owner's power, and doing it releases
  // the WHOLE cohort - the other student in that drill pushed straight
  // afterwards.
  const why =
    `An organization owner holds admin on every repository in ${org}, and administers the ` +
    `organization's rulesets. Under a deadline that takes admin away, GitHub grants the highest ` +
    `applicable permission - the demotion to "pull" is written and verified, reads back "admin", ` +
    `and the freeze does not hold for them (the lockdown record says verified: false, but only ` +
    `after the deadline has passed). Under a deadline that blocks pushes, the block DOES hold for ` +
    `them - and they can delete the organization ruleset that carries it, which releases every ` +
    `student in the cohort at once, not just themselves.`;
 ...
 *
 * @param {Record<string, string>} declared the `permissions` object from GET /apps/{slug}
 */
 ...
 *
 * @param {Record<string, string>} declared `permissions` from GET /apps/{slug} or GET /app.
 * @param {Array<{id?: number, account?: {login?: string}, permissions?: Record<string, string>}>} installations
 *        Items from GET /app/installations.
...
async function checkParticipatingOrgs(request, org, hubOwner, hubRepo) {
  ...
  const res = await request("GET", registryContentsPath(hubOwner, hubRepo));
  const { state, entry, reason, error } = lookupRegisteredOrg(res, org);
  if (reason === "no-registry") {
  ...
  if (reason === "parse") {
    return check("participating-orgs", "Org listed on participating-orgs branch", "warn", `participating-orgs.yml could not be parsed: ${error}.`);
  }
  if (state !== "listed") {
...
async function checkAssignmentLockdown(request, org, assignmentId) {
  ...
  const label = `Lockdown record matches repo permissions (${assignmentId})`;
  const results = Array.isArray(record.results) ? record.results : [];
  // ONLY rows that CLAIM a demotion. This read `r.login`, which a lockdown row
  // does not have (it is `github_login`, additionalProperties: false), so every
  // row was skipped and the check reported "all demoted" having looked at
  // nobody. And not every lock is a demotion: under a ruleset the student keeps
  // write and the ruleset refuses the push, so expecting `read` there would be
  // a false alarm on every Team-plan cohort. A reopened, deferred or
  // unfreezable row claims nothing.
  const claimed = results.filter((r) =>
    typeof r?.github_login === "string" && r.github_login &&
    typeof r?.repo_name === "string" && r.repo_name &&
    (r.lock_method === "demotion" || r.demoted === true) &&
    !r.reopened_at && !r.deferred_until && !r.unfreezable_reason);
  if (claimed.length === 0) {
    return check("assignment-lockdown", label, "info",
      results.length
        ? "No student in this lockdown was demoted (the lock is a ruleset), so there is no permission to compare."
        : "The lockdown record lists no students.");
  }
  // Sample up to 10 students rather than hammering the API on a large class.
  const sample = claimed.slice(0, 10);
  const mismatches = [];
  const unread = [];
  for (const r of sample) {
    const login = r.github_login;
    const repoOnly = r.repo_name.includes("/") ? r.repo_name.split("/")[1] : r.repo_name;
    const permRes = await request("GET", `/repos/${org}/${repoOnly}/collaborators/${login}/permission`);
    // Unreadable is not evidence: it is counted, never passed.
    if (!permRes.ok) { unread.push(login); continue; }
    const got = permRes.data?.permission ?? "unknown";
    if (got === "read" || got === "none") continue;
    // Reopened by a lecturer after this record was written: write access is
    // what they deliberately gave back.
    const reopened = await request("GET", `/repos/${org}/${CONTROL_REPO}/contents/lockdowns/${assignmentId}/unlocked/${login}.json`);
    if (reopened.ok) continue;
    mismatches.push({ login, expected: "read", got });
  }
  if (mismatches.length > 0) {
    return check("assignment-lockdown", label, "fail", `${mismatches.length} student(s) still have write access after lockdown.`, { mismatches });
  }
  if (unread.length > 0) {
    return check("assignment-lockdown", label, "warn",
      `Could not read the permission of ${unread.length} of ${sample.length} sampled student(s): ${unread.join(", ")}.`);
  }
  return check("assignment-lockdown", label, "ok", `Sampled ${sample.length} demoted student(s); all at read or less.`);
}

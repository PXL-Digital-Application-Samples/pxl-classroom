...
export async function listTemplateCommits(get, templateFullName) {
  ...
  // The last page was FULL: whether anything follows is a question, not an
  // answer. A template of exactly the cap read as incomplete - which now turns
  // off the switched-template path for every student (review 2026-09-26).
  const beyond = await get(`/repos/${templateFullName}/commits?per_page=${PER_PAGE}&page=${MAX_PAGES + 1}`);
  const done = okStatus(beyond) && Array.isArray(beyond.data) && beyond.data.length === 0;
  return { ok: true, status: 200, complete: done, commits };
}
...
export async function planStudent({
  ...
  // THE TEMPLATE WAS SWITCHED AGAIN (A -> B -> C). Their last sync that
  // reached them came from B, so B at that commit is exactly what they hold -
  // a TEMPLATE tree, so a trusted base, which their first commit (A) is not a
  // good one for: B's files are in neither A nor C, so they were never
  // removed, and an untouched copy of one B also has was offered as a pull
  // request. From B's commit: an untouched B file is replaced or removed, an
  // edited one is a pull request (review 2026-09-26).
  //
  // A DIFFERENT NAME IS NOT A DIFFERENT TEMPLATE. A renamed template keeps its
  // id and GitHub redirects the old name, so every record written before the
  // rename names "another" template - and taking this branch for them skipped
  // the never-backwards guard (a sync to an older commit rewound main) and the
  // rule that keeps a student's own added file (review 2026-09-26). So the
  // test is the COMMIT: one in this template's listed history is this
  // template, whatever the record calls it (rename, transfer, fork).
  const inThisHistory = (sha) => templateCommits.some((c) => c.sha === sha);
  if (start.source === "synced" && start.repo && String(start.repo).toLowerCase() !== String(templateFullName).toLowerCase()
    && !inThisHistory(start.sha)) {
    try {
      const priorTree = await readTree(start.repo, start.sha);
      const knownVersions = await templateVersions({ readTree, templateFullName, templateCommits, headTree });
      const range = diffTreePaths(priorTree, headTree);
      const paths = resolveSelection(range, selected);
      const plan = planStarterSync({ headTree, baseTree: priorTree, studentTree, paths, knownVersions, keepAdded: false });
      return { from: start.sha, source: "other-template", paths, plan };
    } catch (err) {
      // GONE or not readable (404/403 - the earlier template deleted, or
      // private to this token): what follows treats them as not placed in this
      // template's history. ANYTHING ELSE (a 5xx, a rate limit) is a failed
      // read, thrown so the student is reported failed and retried - silently
      // re-planning from the first commit left the earlier template's files
      // behind for good, the defect this branch exists to fix.

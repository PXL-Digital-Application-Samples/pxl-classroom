...
export async function planStudent({
  ...
  // pull request. `range` is every path either tree has, so a new-template
  // file the student already holds is compared too.
  if (start.source === "unknown" && studentRepo) {
    const own = await rootOnce();
    if (own) {
      try {
        const firstTree = await readTree(studentRepo, own);
        const range = [...new Set([...diffTreePaths(firstTree, headTree), ...headTree.keys()])].sort();
        ...
        const plan = planStarterSync({ headTree, baseTree: firstTree, studentTree, paths, knownVersions, keepAdded: false });

...
import { readRunScore, withoutDispatchedRuns } from "./grade-dispatch.mjs";
...
export async function readScoreAtCommit(request, { repoFullName, sha, marker = null, fallbackTotal = 0 }) {
  ...
  // Never a dispatched run's result: one dispatched for an OLDER commit is
  // listed on this one (lib/grade-dispatch.mjs, measured).
  const own = await withoutDispatchedRuns(request, { repoFullName, sha, checkRuns: checksReq.data?.check_runs });
  if (!own.ok) {
    return {
      verdict: "api-failed",
      permissionDenied: own.status === 403 || own.status === 401,
      reason: `could not read which runs at commit ${short} a lecturer started - HTTP ${own.status}`,
    };
  }
  ...
  if (!parsed.graded) {
    // The hand-in explanation belongs to a SKIPPED run only - that is what a
    // job gated on the message leaves at every other commit. A hand-in whose
    // run was cancelled or timed out IS a hand-in, and telling a lecturer it
    // "only runs on a commit whose message is ..." about it was false.
    return {
      verdict: "not-run",
      reason: marker && parsed.conclusion === "skipped"
        ? `the grading workflow was skipped at commit ${short} - ` +
...
export async function gradeStudent(
  ...
  if (decision?.kind === "commit") {
    ...
    // Graded by a lecturer's "Grade this commit now": that RUN is the result
    // (it belongs to the branch tip, so nothing is attached to the commit).
    const outcome = decision.runId
      ? await readRunScore(request, { repoFullName, runId: decision.runId, sha: decision.sha, fallbackTotal })
      : await readScoreAtCommit(request, { repoFullName, sha: decision.sha, marker, fallbackTotal });
...
/**
 * Who the ARCHIVE-BASED grader (`pxl-classroom grade`) grades: every preserved
 * submission, and every student a lecturer set a score by hand for - who needs
 * no submission at all (an oral exam, no repository). Leaving them out wrote a
 * summary without them, and that summary replaces the whole file, so their
 * score was lost (review 2026-09-26).
 *
 * @param {Array<{github_login?: string, preservation_status?: string|null, preserved_sha?: string|null, team_slug?: string|null}>} students report rows
 * @param {Map|Array|null} overrides
 */
export function archiveGradeQueue(students, overrides) {
  const all = students || [];
  return all.filter((s) => s?.github_login && (
    (s.preservation_status === "preserved" && s.preserved_sha) ||
    gradeDecisionFor(s.github_login, { overrides, team: teamOf(s, all) })?.kind === "score"
  ));
}
...
export async function gradeCohort(
  ...
  // A student with no repository is graded only by a score set by hand - which
  // reads nothing, and was otherwise dropped by the next Re-grade all or the
  // nightly after the lecturer entered it (an oral exam, no repository).
  const queue = (students || []).filter((s) => s?.github_login && (
    s?.repo_name || gradeDecisionFor(s.github_login, { overrides, team: teamOf(s, students) })?.kind === "score"
  ));
  const graded = [];
  const failed = [];
    ...
    while (cursor < queue.length) {
      ...
      try {
        ...
        if (outcome.verdict === "api-failed") {
          apiFailedCount++;
          // Named, so "try again later" says WHO - a read that keeps failing
          // for one student is otherwise a refusal nobody can act on.
          unreadable.push({ login: row.github_login, reason: outcome.reason || "read failed" });
        } else {
        ...
        apiFailedCount++;
        // In `unreadable` too: the page names only those, and a thrown read (a
        // browser fetch timeout) showed "could not be read for 1 student(s): ."
        unreadable.push({ login: row.github_login, reason: `read failed: ${err?.message || String(err)}` });
  ...
  if (permissionDenied) {
    return { ok: false, refusal: "permission", graded, failed, apiFailedCount, unreadable };
  }
  if (apiFailedCount > 0) {
    return { ok: false, refusal: "api-errors", graded, failed, apiFailedCount, unreadable };
  }
  ...
  if (graded.length === 0) {
    return { ok: false, refusal: "nothing-graded", graded, failed, apiFailedCount, unreadable };
  }
  return { ok: true, refusal: null, graded, failed, apiFailedCount, unreadable };
}

...
/**
 * The same link, asking a different question.
 *
 * A confirmation says "this account is this person" and nothing else: no
 * repository, no roster gate, no deadline. It rides the assignment's existing
 * keypair, nonce and broker on purpose - so it inherits the kill switch
 * (`INVITE_ENABLED`, flipped at finalize) and the revocation (rotate the nonce)
 * rather than inventing a second lifetime nobody would remember to end.
 *
 * THE PURPOSE IS SIGNED, NOT MERELY PREFIXED. The signature covers
 * `<kid>.<payload>` and the prefix sits outside it, so a title rewritten from
 * one prefix to the other would otherwise verify perfectly - and a student sent
 * a "confirm your email" link could turn it into an acceptance of an `open`
 * assignment they were never invited to. The purpose is folded into the SIGNED
 * SUBJECT instead (`confirm:<id>` rather than `<id>`), so a swapped prefix
 * makes the verifier expect a digest the payload does not carry and the title
 * is refused as `wrong-assignment`. That costs no wire-format change, which
 * matters more here than anywhere: every broker ever published checks the hub
 * out at `main` and runs this file, so a payload byte cannot be added without
 * breaking every live acceptance at once.
 */
export const CONFIRM_TITLE_PREFIX = "pxl-confirm:";

export const PURPOSE = Object.freeze({ ACCEPT: "accept", CONFIRM: "confirm" });

// ONE table. The prefix a title carries and the subject its signature covers
// are two halves of one decision, and splitting them across two lookups is how
// a check comes to verify a signature over the wrong question.
const PURPOSES = Object.freeze({
  [PURPOSE.ACCEPT]: { prefix: TITLE_PREFIX, subject: (id) => `${id}` },
  [PURPOSE.CONFIRM]: { prefix: CONFIRM_TITLE_PREFIX, subject: (id) => `confirm:${id}` },
});

/**
 * Which question a title is asking, or null if it is not one of ours.
 *
 * Longest prefix first would matter if one were a prefix of the other. They are
 * not, and this asserts nothing about that - it just checks both.
 */
export function purposeForTitle(title) {
  if (typeof title !== "string") return null;
  for (const [purpose, spec] of Object.entries(PURPOSES)) {
    if (title.startsWith(spec.prefix)) return purpose;
  }
  return null;
}

/** The title prefix for a purpose. Throws rather than guessing. */
export function titlePrefixFor(purpose) {
  const spec = PURPOSES[purpose];
  if (!spec) throw new Error(`unknown acceptance purpose ${JSON.stringify(purpose)}`);
  return spec.prefix;
}

/**
 * What the signature actually covers, for a purpose and an assignment.
 *
 * `accept` is the identity function, deliberately: every title ever signed is
 * an accept, and changing what they cover would invalidate all of them.
 */
export function signedSubjectFor(purpose, assignmentId) {
  const spec = PURPOSES[purpose];
  if (!spec) throw new Error(`unknown acceptance purpose ${JSON.stringify(purpose)}`);
  return spec.subject(assignmentId);
}
...
// instead of `bad-signature` (INSTALL.md §3.3 makes a mismatched INVITE_PUBKEY a
// real deployment fault, and telling the two apart is the whole value).

// Internal: the payload the encoder writes and the decoder refuses to read at
// any other value. Not exported - nothing outside this module can do anything
// useful with it, and a published constant invites a caller to branch on it.
 ...
 */
export async function signAcceptanceTitle({ privateKey, kid, subject, githubId, nonce, purpose = PURPOSE.ACCEPT }) {
  ...
  // Both derived from `purpose` in one place. The prefix and the signed subject
  // are the two halves of the anti-swap rule, and reading them from separate
  // arguments is how they would come to disagree.
  const prefix = titlePrefixFor(purpose);
  const nonceBytes = new Uint8Array(
    nonce.match(/../g).map((pair) => parseInt(pair, 16)),
  );
  const key = await importPrivate(privateKey);
  const payload = encodePayload({
    subjectBytes: await subjectDigest(signedSubjectFor(purpose, subject)),
    githubId,
    nonceBytes,
  });
  ...
  const title = `${prefix}${signed}.${toBase64Url(signature)}`;
...
export async function verifyAcceptanceTitle({ title, publicKey, expectedKid = null, expectedSubject = null }) {
  // The purpose comes from the title and the expected subject is DERIVED from
  // it - never taken from the caller. That is what makes rewriting the prefix
  // useless: swap it and the verifier looks for a different digest than the one
  // the signature covers, so the title fails as `wrong-assignment`. A caller
  // that passed a purpose of its own could be talked out of the check by the
  // very string it is checking.
  ...
  const signedPart = title.slice(titlePrefixFor(purpose).length).split(/\s/)[0];
  ...
  // holding the wrong INVITE_PUBKEY, which INSTALL.md §3.3 names as a real
  // deployment fault. Telling that apart from `bad-signature` is the point.
  //
  // It fires on a swapped prefix too, and that is not a side effect - it is the
  // anti-swap rule landing. `signedSubjectFor` folds the purpose into what the
  // digest covers, so a confirmation relabelled as an acceptance asks for a
  // digest of `<id>` while carrying one of `confirm:<id>`.
  if (expectedSubject !== null) {
    let want;
    try {
      want = await subjectDigest(signedSubjectFor(purpose, expectedSubject));
    } catch {
  ...
  return { ok: true, kid, payload, purpose };
}

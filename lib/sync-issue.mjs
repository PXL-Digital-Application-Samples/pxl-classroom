...
/**
 * One entry per REPOSITORY, the first member's, in the given order. A team is
 * several records naming one repository, and a sync planned per record in
 * parallel opened one pull request per member (the CLI, review 2026-09-26).
 * Records with no repository are kept - each is reported on its own.
 *
 * `rank`: which member plans it - the highest rank, the first on a tie. A
 * starting point is per LOGIN, so planning through a member with no sync
 * record while a teammate has one re-planned a synced team from scratch.
 *
 * @template T
 * @param {T[]} items
 * @param {(item: T) => string|null|undefined} repoOf
 * @param {(item: T) => number} [rank]
 * @returns {T[]}
 */
export function oneRecordPerRepo(items, repoOf, rank = () => 0) {
  const best = new Map();
  const out = [];
  for (const item of items || []) {
    const key = String(repoOf(item) || "").toLowerCase();
    if (!key) {
      out.push(item);
      continue;
    }
    ...
    if (!held) {
      best.set(key, { item, at: out.length });
      out.push(item);
    } else if (rank(item) > rank(held.item)) {
      out[held.at] = item;
      held.item = item;
    }
  }

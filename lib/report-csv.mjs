...
export const REPORT_ROW_COLUMNS = Object.freeze([
  ...
  "class_group",
  "acceptance_state",
  // Beside the acceptance state because it is one: this student accepted and
  // was handed a repository that already existed, so they never got this
  // assignment's starter code. It is the only surface that says so - the
  // acceptance JSON in the control repo is not somewhere a lecturer looks.
  "reused_existing_repo",
  "claimed_email",
  "claim_verified",
  "claim_domain_allowed",
  // Beside it: an address in the right domain that does not name the student
  // (`12345678@`) is the other half of "can I tell who this is".
  "claim_format_allowed",
  "submission_status",
  ...
  // Beside the instant writes stopped, because the pair is the answer: locked
  // then, let back in at this. A grade dispute months later asks exactly
  // whether this student could have pushed after the deadline, and a
  // spreadsheet is where it gets asked.
  "reopened_at",
  "preservation_status",
  "preserved_sha",
  "archive_repo",
...
export const RENDER_JOIN_COLUMNS = Object.freeze([
  ...
  "earned_points",
  "total_points",
  // Beside the number it qualifies, because a spreadsheet is where a grade
  // gets acted on. `conclusion` means nothing reported a score and all that is
  // known is whether the run went green - which for a job that died at setup
  // exports as a 0 indistinguishable from a measured one. The screen says so
  // (frontend/src/lib/status-labels.js); the export has to as well, or the
  // column a lecturer actually marks from is the one that cannot tell them.
  "score_source",
  // Which commit was graded, and whether a LECTURER decided it rather than the
  // rules (lib/grade-override.mjs) - a chosen commit or a score by hand, with
  // who, when and why. A grade dispute reads exactly this, and it was on the
  // screen and in the grading summary but not in the export (review
  // 2026-09-26). Empty where the rules graded.
  "graded_sha",
  "grade_decision",
  "grade_decision_by",
  "grade_decision_at",
  "grade_decision_reason",
  // Under a hand-in cap (submission_marker.max_hand_ins): hand-ins made on or
  // before the deadline, how many counted, how many were not graded - and the
  // per-student exception behind `allowed`, with who granted it, when and why,
  // because that is what a grade dispute reads. Counts from the grading
  // summary; the exception from `overrides/`, its record. Empty without a cap.
  "hand_ins_used",
  "hand_ins_allowed",
  "hand_ins_ignored",
  "hand_in_extra",

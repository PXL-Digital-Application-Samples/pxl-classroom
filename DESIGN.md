...
5. **The UI must not describe behaviour the system does not have.**
   ...
   * **The inverse costs as much: a control that renders only once its feature is already in use is invisible.** Hiding a distinction an organization has never made is right — it stops a lecturer hunting for groups that do not exist — but hiding the *whole field* means the only people who ever see it are the ones who did not need telling. The **Class groups** picker required a `class_group` on the roster before it would appear, and measured on 2026-09-05 not one student in any live organization had one: the control had never rendered anywhere, and a lecturer asking how to split their classes across assignments had nothing on screen to find. Show the field, keep the options honest, and let the empty state say what the field is for and where the input comes from.

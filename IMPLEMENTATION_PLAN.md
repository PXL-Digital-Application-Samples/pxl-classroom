...
assignments/<id>.yml         # definition (auto-generated by Admin UI)
...
overrides/<id>/<login>.json  # append-only lecturer overrides (auto-generated by Admin UI)
...
- [x] Finalize the provisioning App permissions (Admin/Contents/Metadata + Account/Starring + Device Flow); production secrets `PXL_APP_ID`/`PXL_APP_PRIVATE_KEY`; rotate the spike key.
- [x] Decide + create the **dispatcher App** (or scoped-secret model) for public brokers.
- [x] Create the **control repo template** (data dirs + schemas + thin caller workflows) and the per-org **archive repo**, both in PXLAutomation.
...
- [x] **Prove the `repository_dispatch` hop FIRST** (public broker → private control repo): the riskiest *untested* integration. Confirm a star fires the broker workflow, which mints a token and successfully dispatches an event that triggers the control repo's handler. (Spike 3 proved star→watch→actor→secret, **not** this hop.)
- [x] Configure the assignment's **open window** and **per-assignment cap** (`max_acceptances`) — the mandatory open-acceptance guardrails (§7).
- [x] Define one real assignment (e.g. from `template-automation-pe-1`); `publish-assignment` creates the public broker.
...
- [x] Lecturer dashboard: org picker + this assignment's per-student table + CSV export.
...
### Phase 2 — Harden
- [x] Notifications + tracking issue; recovery runbooks; retry/repair workflows; dry-run modes everywhere.

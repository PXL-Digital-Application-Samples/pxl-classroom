...
- **Stage 2:** the non-exam paths - `notify` and `report` (done 2026-09-17: the control repo, `issues: write`), `registry` (done 2026-09-17: every repository, `metadata: read` and `issues: write`), `collect` (done 2026-09-17: every repository, `contents: read` and `metadata: read`), then dashboard regeneration, starter sync, feedback PRs, org setup, the weekly report's repository list - one per change.
...
**How to tell it is closed:** no row is still pending (20 when stage 1 landed on 2026-09-17, 10 per stage; 16 after `notify`, `report`, `registry` and `collect`). A row that stays broad on purpose carries a `reason` instead.

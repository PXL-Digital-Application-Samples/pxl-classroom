...
### 2. Relayout of the new-assignment page — DONE 2026-09-07

**Shipped.** Basics and Template are one fieldset, in the order the values already derived: template repository, title, repository name pattern, slug, description. The merged block kept the name *Basics* rather than inventing one.

Three of the four questions had answers rather than choices. **The title prefill already existed** and was unreachable: `selectTemplate` fills the title only while it is empty, and the title sat above the picker, so anyone working down the form typed one first. **The slug** names `assignments/<id>.yml`, the public `broker-<id>` repository students open, and the lecturer's own dashboard link — but not the invitation link, which is a token; it is now a derived line with an Edit, and stays locked on an existing assignment. **The collision check already existed**, on `repository_name_pattern` and not the id, and the real gate runs at Save; it now also fires when a template is picked, because the fast path never blurs anything.

Side by side was declined: the picker holds `owner/repository` and the first-run wall is wide, and nothing else in the app is a two-column form. The width complaint was met by capping the controls instead. `DESIGN.md §1.8`, `tests/e2e/67`.
...
### 3. Class groups on the Roster tab, and the Number column — DONE 2026-09-07

**Shipped.** Tick rows, type a group, Apply — **one commit** for the whole selection, which was the actual complaint: the clicking was slow, the writing was slower, and a cell edit is a commit per student. Select-all takes the current filter and a selection survives changing it, so "3B plus these four" is one pass. Every column sorts, ungrouped students last in both directions, and sorting never touches the stored order. The Number column hides when nobody has a number and returns when somebody does.

The Group column deliberately does **not** hide when empty: `DESIGN.md §1` already records the Class groups picker rendering only once a `class_group` existed, so the feature was invisible to everyone who had not already used it.

**A UX review of the finished thing found two more, and one of them was a lie on screen.** Selecting students and then changing the filter showed *"2 selected"* over two **different** visible rows, so the rows on screen read as the ones about to change — Apply would have moved students the lecturer could not see. The bar now names how many are off-screen and a **Selected (n)** chip shows them. The header tick box was also never indeterminate, so one of six selected read as none.

The same review found the feature missing its own point: the chips were All / Has account / No account yet, so *"put everyone with no group into 3B"* — the flow it was built for — had no expression, and forty students still meant forty ticks. Group chips (one per class, plus **No group**) come from `classGroupChips` in `lib/class-groups.mjs`, shared with the cohort picker rather than copied into a second filter row. **Remove from group** is its own button, never an empty box plus Apply.

Two things were found on the way. The empty-state cell carried a hand-written `colspan="7"` over a **six**-column table — the exact defect `tests/table-colspan.test.mjs` was written for, which guarded only `AssignmentDetailView` and so never looked at the second table; it is derived now and the test covers both. And `SortIcon` was a render helper living inside `AssignmentDetailView`, which a second sortable table would have forked; it is a shared component. `tests/e2e/68`.

The request itself, as spoken:

...
- **Stage 3:** acceptance, provisioning, publishing, the sentinel's watch, finalize, lockdown and preservation, one job per change. Each change is followed within the hour by a deadline drill (below) instead of waiting for a gap in the calendar. Acceptance cannot be confined to one organization: any student accepting anywhere runs the changed job, so those changes land at a quiet hour.

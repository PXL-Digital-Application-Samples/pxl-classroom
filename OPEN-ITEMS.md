...
## 10. Most App tokens reach every repository with every permission

**Status: open - staged.** Found 2026-09-17 by zizmor's `github-app` audit: 23 of the 27 token steps mint with only `owner:`.

**What that means.** Such a token covers every repository the App can reach in that organization - student repositories, the control repo, brokers - with every permission the installation holds, including administration, members, secrets and workflows at write. It lives for up to an hour, is revoked when the job ends, and a control-repo checkout keeps it in `control/.git/config` in between. Anything that runs in that job after it is minted can use all of it: our scripts, the npm packages they load, a third-party action. Nothing has been misused; the bound is simply wider than any step needs.
...
- **Stage 2, done 2026-09-17:** the non-exam paths, one per change, each named in `lib/app-token-scopes.mjs` with the calls it makes. The control repo alone for `notify`, `report`, dashboard regeneration and the weekly report; every repository, with named permissions, for `registry`, `collect`, starter sync, feedback PRs and org setup, whose repositories are student repositories read from the records or one that does not exist yet.
- **Stage 3:** acceptance, provisioning, publishing, the sentinel's watch, finalize, lockdown and preservation, one job per change. Each change is followed within the hour by a deadline drill (below) instead of waiting for a gap in the calendar. Acceptance cannot be confined to one organization: any student accepting anywhere runs the changed job, so those changes land at a quiet hour. None of it changes the App's own permissions, which would email every org owner for approval: a token only asks for less of what the installation already holds.
  - `publish-assignment` (2026-09-17): every repository, because it creates the broker, with Administration, Contents, Secrets, Variables and Workflows at write and Organization administration read for the plan.
  - Acceptance (2026-09-17): `retry-acceptance` asks for the control repo alone; the acceptance handler for Contents and Issues (it labels the broker the payload names); `./acceptance` for Metadata read; `./provisioning` for Administration, Contents and Workflows at write, because it creates the repository.
  - The deadline (2026-09-17): the sentinel's watch and `migrate-org-lock` for Administration and Organization administration at write (rulesets, and demotion as the fallback) with Contents; `./lockdown` the same with Contents and Members read; `./preserve` for Administration, Contents and Workflows at write, because it creates the archive and pushes student commits; finalize for Contents, Variables and Secrets at write and Checks read. Grading at the deadline is the one path no drill reaches, because no drill assignment grades.
...
**The deadline drill** is `tests/live/drill.mjs`. `start` creates and publishes a fresh individual assignment with late work blocked and a deadline 45 minutes out, accepts it as both test students and pushes a commit for each. `verify <id> --wait` runs after the deadline. It checks that the sentinel fired, the lock method on every row, that a late push is refused, the preserved archive branches, the report, and that the broker is closed. A fresh assignment every time, because finalize locks and preserves an assignment once. The testbed is on **Team** (since 2026-09-17), and that is load-bearing. On a free organization a private repository cannot hold a ruleset, so lockdown falls back to demotion and still reports `locked`. A token that lost the ruleset permission falls back the same way on a Team organization, green, so only a Team drill that checks `lock_method` per row can catch it.
...
**How to tell it is closed:** no row is still pending (20 when stage 1 landed on 2026-09-17, 10 per stage; 10 once stage 2 was done the same day, all of them stage 3). A row that stays broad on purpose carries a `reason` instead.
...
| **Ad-hoc branch creation on the hub was unrestricted** | Ruleset `Block ad-hoc branch creation` | `gh api repos/PXL-Digital-Application-Samples/pxl-classroom/rulesets` returns it `active`, target `branch`, rule `creation`, `~ALL` excluding `refs/heads/participating-orgs`, bypass for OrganizationAdmin, the repository role and, since 2026-09-16, the Dependabot app (`Integration` 29110) - exactly as INSTALL.md §3.4 specifies. |

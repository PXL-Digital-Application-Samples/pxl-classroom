...
**Blocking ad-hoc branch creation** closes the other half of the branch-ref path: the environment stops a credential being *read* at another ref, and this stops the ref being created. A ruleset named `Block ad-hoc branch creation` does it, with target `branch`, enforcement `active`, rule `creation`, `conditions.ref_name.include` of `~ALL` excluding `refs/heads/participating-orgs` (which `setup-org.yml` creates on a fresh hub), and bypass actors for OrganizationAdmin, the repository admin role, and the Dependabot app (`Integration`, id `29110`). Dependabot delivers every update on a branch of its own (ADMIN.md §8); without its bypass the update runs report their pull requests as created and none appear. It is a bypass for that one app rather than an excluded `dependabot/**` pattern, which anyone with write access could create:

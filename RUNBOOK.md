...
3. Add starter code, `.github/workflows/` for the student's own CI, and assignment instructions. Anything you commit here becomes the student's starting point. **It needs at least one commit**, even when students are meant to start from nothing: GitHub cannot create a repository from an empty one (`422 Could not clone: <owner>/<repo> is empty.`), so every acceptance would fail. A README is enough. The form warns about an empty template and publishing refuses it.

**No starter code at all? Do none of the above.** In the assignment form, under the template box: *Nothing to start from?* -> **Create a blank starter**. It creates `starter-<slug>` in your organization with a README in it, already ticked as a template, and fills the field in - the three steps above in one click, with the commit and the checkbox both right. Students then begin from an empty project, which is what GitHub Classroom's "no starter code" did. If a repository of that name already exists it refuses rather than using it, because a repository called `starter-portfolio` may belong to last year's `portfolio` assignment; pick it in the list yourself if it really is the starter code you meant. Offered on a new assignment only, and a lecturer who is not allowed to create repositories in the organization gets the manual route above instead.
...
- A commit with no result also offers **Grade this commit now** when the student's grading workflow can grade a chosen commit. It runs the workflow on their submission branch against the code of that commit, now: no 30-day limit, and a hand-in message does not stop it. **Tests written in the workflow are the current ones; test files in the repository are the ones at that commit.** When the dialog says the workflow *cannot grade a chosen commit yet*, the repository has an older workflow file: send the current one with **Sync Starter Code** (repositories created from now on have it already). A template with a workflow of its own needs the three changes in AUTOGRADING.md, *Grading a commit you choose*. Where nothing can run, use a score by hand.

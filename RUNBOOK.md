...
| Who may accept | **`enforced` by default** - only logins in `students/roster.yml`. The form shows the live count underneath and links to the **Roster** tab: `No students imported yet - nobody can accept` or `213 students on the roster`. Switch to `open` only for a cohort you do not know up front, e.g. an exam - it removes the roster gate entirely and then requires a cap (§12.4). |

...
- **System Health now says so before they hit the 403.** The `hub-dispatch` check reads the viewer's own `permissions.push` on the hub repo and warns when it is false, naming the organization-membership trap above. It is a warning rather than a failure because editing still works, and an absent `permissions` object produces no verdict rather than a green one - GitHub omits it when nobody is authenticated, which is not the same answer as "no access". Point a lecturer at **System Health** first when they report that publishing fails.

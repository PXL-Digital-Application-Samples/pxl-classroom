...
test.describe('14 - Multi-Org Portal, Student Assignments Dashboard & Usage Billing', () => {
  ...
  test('Scenario 4 (Cross-Organization Billing & Quota Overview): Aggregates usage across all participating orgs and executes global trigger', async ({ page }) => {
    const cloudUsage = {
      schema_version: 1,
      week_start: '2026-08-17',
      week_end: '2026-08-23',
      ...
      over_count: 1,
      items: [
        { repo: 'cloud-exam-runner', sku: 'actions_minutes', used: 3100, limit: 2000, over: true },
      ],
    };
    ...
    const webUsage = {
      ...
      over_count: 0,
      items: [
        { repo: 'web-frameworks-repo', sku: 'actions_minutes', used: 600, limit: 2000, over: false },
      ],
    };
    ...
    // Verify all-orgs usage header
    await expect(page.getByRole('heading', { name: /Usage - all organizations/i })).toBeVisible();
    await expect(page.locator('.report-meta')).toContainText('Orgs scanned: 2 / 2');
    await expect(page.locator('.report-meta')).toContainText('1 repo/SKU pair(s) over threshold');

    // Verify table lists repos from both orgs
    const table = page.locator('table.usage-table');
    await expect(table).toBeVisible();
    await expect(table).toContainText('cloud-exam-runner');
    await expect(table).toContainText('web-frameworks-repo');

    // Test filter input
    const filterInput = page.getByPlaceholder(/Filter by org, repo, or SKU/i);
    await filterInput.fill('cloud-exam');
    await expect(table).toContainText('cloud-exam-runner');
    await expect(table).not.toContainText('web-frameworks-repo');

    // Test global Regenerate trigger
    const regenBtn = page.getByRole('button', { name: /Regenerate now/i });
    await expect(regenBtn).toBeVisible();
    await regenBtn.click();
    await expect(page.locator('.toast', { hasText: /Workflow triggered/i })).toBeVisible();
